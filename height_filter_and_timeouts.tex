...
It runs \processSlot once per slot from $\Sigma.\sigma[B.\parent].s + 1$ up to $B.\slot$ (so any height advance enabled by the accumulated tally fires before~$B$'s votes are folded in), then $\processBlock(\cdot, B)$; the resulting post-state $\sigma'$ becomes $\Sigma.\sigma[B]$.
...
The justification fired at the post-state of some processed block~$B$ whose \processSlot call triggered the justification branch with $(\sigma[B].J,\, \sigma[B].h_j) = (C,\, h)$, so $C \preceq B$ and $\sigma[B].h \geq h + 1$ (justification advances height).
...
This section extends the per-chain state machine of Section~\ref{sec:model} with a penalty counter and a new transition function \processInactivityPenalties, invoked each slot by an extended \processSlot (Figure~\ref{alg:leak-processslot}); \processBlock, \processHeight, and \processVote are unchanged.
...
Figure~\ref{alg:leak-processslot} replaces \processSlot from Section~\ref{sec:model} with a version that snapshots the pre-advance state and invokes \processInactivityPenalties after \processHeight.
...
Together: at $h_c = \Sigma.\hmax$ no validator has observed a justification at~$h_c$, so the lock branch is unreachable; at $h_c = \Sigma.\hmax - 1$ the lock branch forces the lock target to coincide with~$\Sigma.J$ via Lemma~\ref{lem:just-unique-height}, and $\Sigma.J$ is itself viable (its witnessing block sits at $\sigma$-height~$\Sigma.\hmax$ as a descendant of~$\Sigma.J$).
...
The fallback branch emits $v.\target = \bot$, satisfying the first disjunct.
The first-vote-at-$h_c$ branch sets $v.\target = T$ with $\sigma[T].h = h_c$ and $T \preceq T$, satisfying the second disjunct.

The remaining branch is the lock branch with $v.\target = T_{\mathrm{lock}}$ fixed by an earlier finalize-bearing vote~$v_0$ of~$i$ with $v_0.\finalizeHeight = h_c$ and $v_0.\finalizeTarget = T_{\mathrm{lock}}$.
Since $v_0$ is finalize-bearing, \getBaseVote populated its finalize fields from $i$'s then-canonical state, giving $(v_0.\finalizeHeight, v_0.\finalizeTarget) = (h_c, T_{\mathrm{lock}}) = (\sigma[T^0].h_j, \sigma[T^0].J)$ at $v_0$'s firing.
The justification branch of \processHeight thus fired with target $T_{\mathrm{lock}}$ at pre-state height~$h_c$ inside some \onBlock call on a processed block~$B^\star$, whose post-state has $\sigma[B^\star].h \geq h_c + 1$ (height-advance).
Hence $\Sigma.\hmax \geq h_c + 1$, i.e., $h_c \leq \Sigma.\hmax - 1$.
...
$\timeouts$ resets only at a height advance, which does not fire while state-height stays at~$h_c$, so $\timeouts[i] = \mathtt{true}$ persists and Layer~1 does not bump~$i$.
\end{proof}
...
\paragraph{No voting-rule change.}
Unlike the prefix-notarization design, no $R$-shift branch or stale-cert handling is needed inside \getFinalVote{}: the cascade gate $\Sigma.\hmax = \Sigma.h_j + 1$ in \getConfirmed{} (Figure~\ref{alg:store}) absorbs that role.
...
Assume $f < n/3$, all honest follow Definition~\ref{def:voting-rule},  slot~$k$ satisfies the standing assumptions (Definition~\ref{def:standing}), and, for any honest validator $v$ and time $t \in [\vote(k),\vote(k+1)]$, there exists a block $R' \in \V_v^t$ such that  $\getConfirmed_{v_\ell}^{\vote(k)} \succeq R'$. 
...
If
\begin{enumerate}
  \item\label{rev:hp:f} honest validators follow \Cref{def:voterecord};
...
\emph{Setup.}
Definition~\ref{def:healed} names some honest $v_\ell^0$ with conditions~\ref{rev:def:healed-state:cap} and~\ref{rev:def:healed-state:vote-h}.
...
\]
Since $v_\ell$ is $\preceq$-max, $\getConfirmed_{v_\ell^0}^{\vote(k)} \preceq \getConfirmed_{v_\ell}^{\vote(k)}$, so conditions~\ref{rev:def:healed-state:cap} and~\ref{rev:def:healed-state:vote-h} also hold with $v_\ell$ in place of $v_\ell^0$;
in particular every honest vote $u$ cast strictly before $\vote(k)$ has $u.\height \leq h^\ast$.

For honest votes cast at $\vote(k)$:
$\preceq$-maximality plus state-height monotonicity give $\sigma[\getConfirmed_v^{\vote(k)}].h \leq h^\ast$ for every honest $v$, and Lemma~\ref{lem:honest-vote-sets-timeout} identifies an honest vote's height with the casting validator's canonical state-height, so $u.\height \leq h^\ast$ at $\vote(k)$ as well.
...
Any justification at height~$h$ has a $\geq 2n/3$ quorum.
Under online-honest majority ($f < n/2$), no $2n/3$-quorum can be entirely dishonest, so each such quorum contains at least one honest non-$\bot$ contribution at height $h$.
By condition~\ref{rev:def:healed-state:vote-h}, that contribution has $h \leq h^\ast$, ruling out justifications at heights $> h^\ast$.
For $h \in [\comm{h}, h^\ast]$, condition~\ref{rev:def:healed-state:cap} places the honest contribution's target $\preceq \getConfirmed_{v_\ell}^{\vote(k)}$, and per-height uniqueness (Lemma~\ref{lem:just-unique-height}) extends this to the unique justified target.
...
Property~\ref{prop:getConfirmed-visible} places $\getConfirmed_{v_\ell}^{\vote(k)} \in \Sigma_v^{\vote(k)}.\T \subseteq \Sigma_v^t.\T$, with $\sigma$-height $h^\ast$ in $v$'s store (Theorem~\ref{thm:orderindep}).
...
\begin{itemize}
  ...
  If $h_F < h^\ast$, Lemma~\ref{lem:mainsafety} applied to $\getConfirmed_{v_\ell}^{\vote(k)}$ gives $\Sigma_v^t.F \preceq \getConfirmed_{v_\ell}^{\vote(k)}$ directly.
  If $h_F = h^\ast$, $\Sigma_v^t.F$ is the unique finalized block at $h^\ast$ (Lemma~\ref{lem:finchain}), which by Theorem~\ref{thm:fleqr} and Corollary~\ref{cor:getConfirmed-total} satisfies $\Sigma_{v_\ell}.F \preceq \Sigma_{v_\ell}.J \preceq \getConfirmed_{v_\ell}^{\vote(k)}$;
  per-height uniqueness then identifies $\Sigma_v^t.F$ with $\Sigma_{v_\ell}.F$, hence $\preceq \getConfirmed_{v_\ell}^{\vote(k)}$.
\end{itemize}

\medskip

\emph{Step 4: conclusions 2 and 3, and condition~\ref{rev:def:healed-state:compat} at $\vote(k{+}1)$.}
Step~3 instantiates the premise of Property~\ref{prop:getConfirmed-compat} with $R' = \getConfirmed_{v_\ell}^{\vote(k)}$ (which lies in $\V_v^t$ and is reflexively $\succeq$ itself).
...
\begin{itemize}
  ...
  \item if cast at $\vote(k{+}1)$, Lemma~\ref{lem:honest-vote-sets-timeout} gives $u.\height = \sigma[\getConfirmed_{v}^{\vote(k+1)}].h$ for the casting validator $v$, which is $\leq h^{\ast\prime}$ by maximality of $v_\ell^{(k+1)}$.
\end{itemize}
\end{proof}
...
Lemma~\ref{lem:healing-persistence} applies at slot~$k$ (its hypotheses follow from~\ref{rev:hu:f}, \ref{rev:hu:ohm} at $k$, and~\ref{rev:hu:hs}), yielding the system healed at $k+1$ (so~(i)) and conclusions~2 and~3 over $[\vote(k), \vote(k{+}1)]$ (so~(ii) and~(iii)).

\emph{Inductive step.}
Suppose $H(K)$ holds; we show $H(K{+}1)$.
By~(i) at slot $K{+}1$ together with hypothesis~\ref{rev:hu:f} and~\ref{rev:hu:ohm} at $K{+}1$, Lemma~\ref{lem:healing-persistence} applies at slot $K{+}1$, yielding the system healed at $K{+}2$ (extending~(i)) and conclusions~2 and~3 over $[\vote(K{+}1), \vote(K{+}2)]$.
