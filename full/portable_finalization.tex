...
\newcommand{\pnlty}{\mathsf{penalty}}
\newcommand{\parent}{\mathsf{parent}}
\newcommand{\fintarget}{\mathsf{fin\_target}}
\newcommand{\val}{\mathsf{val}}
\newcommand{\finh}{\mathsf{fin\_h}}
...
A \emph{vote} is a signed tuple $(\validator,\; \height,\; \target,\; \kind)$ where $\validator$ identifies the signer, $\height$ is a height, $\target$ is a block, and $\kind \in \{\prefix,\, \finalize\}$ distinguishes the vote type.
A vote included on a chain is verified to reference a target that actually exists on that chain.
\end{definition}

\begin{definition}[State]\label{def:state}
The \emph{state after block~$B$}, written $\sigma[B]$, is a tuple $(h,\; \targets,\; J,\; h_j,\; F,\; P)$.
A block~$B'$ is \emph{on the chain ending at~$B$} iff $B' \preceq B$.
\begin{itemize}
  \item $h$: the \emph{current height} of the finality gadget.
  \item $\targets[1..n]$: the \emph{target tracker}.
        For each validator~$i$, the highest block on the chain ending at~$B$ that~$i$ voted for at height~$h$
        ($\targets[i] = \bot$ if~$i$ has not voted).
  ...
  \item $P \subseteq \{1, \ldots, n\}$: the \emph{finality participation}, the set of validators who have cast a finalize vote ($\kind = \finalize$) with $\height = h_j$ and $\target \succeq J$.
\end{itemize}
...
\[
  \bigl|\{i \,:\, B' \preceq \targets[i]\}\bigr| \;\geq\; 2n/3.
\]
...
\begin{enumerate}
  \item \emph{Prefix vote} ($\kind = \prefix$, justification only).
        At each height~$h$, the validator casts a vote with $\height = h$, $\target = \textsf{getHead}()$, and $\kind = \prefix$.
  \item \emph{Finalize vote} ($\kind = \finalize$, finalization + portable justification).
        After block~$J$ is justified at height~$h_j$ and the validator has seen at least one additional block on the canonical chain at height $> h_j$ confirming that~$J$ is stable (not superseded), the validator casts a vote with $\height = h_j$, $\target = J$, and $\kind = \finalize$.
\end{enumerate}
...
When a vote~$v$ in block~$B$ is processed, the target tracker is updated if the vote is for the current height and the target is on the chain ending at~$B$ ($v.\target \preceq B$): $\targets[v.\validator]$ is set to $v.\target$ when it is higher than the current record (or the validator has not yet voted).
This applies to both prefix and finalize votes.
Independently, if the vote is a finalize vote ($v.\kind = \finalize$) with $v.\height = h_j$, $v.\target \succeq J$, and $v.\target \preceq B$, the validator is added to the finality participation~$P$.
...
\Function{$\textsf{processVote}$}{$(h,\, \targets,\, J,\, h_j,\, F,\, P),\; v,\; B$}
  \State $i \gets v.\validator$
  \If{$v.\height = h$ and $v.\target \preceq B$} \Comment{Both prefix and finalize votes}
    \If{$\targets[i] = \bot$ or $v.\target \succ \targets[i]$}
      \State $\targets[i] \gets v.\target$
    \EndIf
  \EndIf
  \If{$v.\kind = \finalize$ and $v.\height = h_j$ and $J \preceq v.\target \preceq B$} \Comment{Finalize votes only}
    \State $P \gets P \cup \{i\}$
  \EndIf
  \State \Return $(h,\, \targets,\, J,\, h_j,\, F,\, P)$
\EndFunction
\vspace{3pt}

\Function{$\textsf{processHeight}$}{$(h,\, \targets,\, J,\, h_j,\, F,\, P),\; B$}
  \If{$|P| \geq 2n/3$} \Comment{Finality}
    \State $F \gets J$
  \EndIf
  \State $\cnt \gets 0$ \Comment{Justification: walk from $B$ toward genesis}
  \For{$B'$ from $B$ down to $B_{\mathrm{genesis}}$}
    \State $\cnt \gets \cnt + |\{i : \targets[i] = B'\}|$
    \If{$\cnt \geq 2n/3$} \Comment{Height advances}
      ...
      \EndIf
      \State $h \gets h + 1$
      \For{all $i$} $\targets[i] \gets \bot$ \EndFor
      \State \Return $(h,\, \targets,\, J,\, h_j,\, F,\, P)$
    \EndIf
  \EndFor
  \State \Return $(h,\, \targets,\, J,\, h_j,\, F,\, P)$
...
\begin{proof}
By Definition~\ref{def:finality}, finality of~$C$ at height~$h$ on chain~$X$ means $|P| \geq 2n/3$: a set~$Q_F$ of at least $2n/3$ validators where each member~$i$ signed a finalize vote with $\height = h$ and $\target = D_i \succeq C$.
...
If $T_v \not\succeq D_v$, then $v$ is slashable (Definition~\ref{def:slashing}: the finalize vote has $\kind = \finalize$, $\height = h$, and $T_v \not\succeq D_v$).
...
The node computes $\sigma[B] \gets \textsf{processBlock}(\sigma[B.\parent],\; B)$, then updates the store's justified and finalized blocks.
...
\paragraph{Mechanism.}
After block~$J$ is prefix-justified at height~$h_j$ on chain~$X$ (and confirmed stable), honest validators send a finalize vote: $(\validator,\; h_j,\; J,\; \finalize)$.
This vote has $\height = h_j$, $\target = J$, and $\kind = \finalize$.
Consider how it is processed on different chains:

\emph{Original chain~$X$ (at height $> h_j$).}
The target part does not register ($v.\height = h_j \neq h$).
The finalize part registers ($v.\kind = \finalize$, $v.\height = h_j = h_j$, and $J \preceq v.\target = J \preceq B$), adding the validator to~$P$.
When $|P| \geq 2n/3$, $J$ is finalized.

\emph{Another chain~$Y$ at height~$h_j$, containing~$J$.}
The target part registers: $v.\height = h_j = h$ and $v.\target = J \preceq B$ (since $J$ is on chain~$Y$).
...
\begin{itemize}
  ...
  \item A competing chain that diverges before~$J$ cannot include the finalize votes (the on-chain verification $v.\target \preceq B$ fails), so it cannot benefit from them.
\end{itemize}

\begin{remark}[Contrast with pure timeouts]
In Simplex, a timeout vote carries no target ($\target = \bot$), so there is no block to ``replay'' on a competing chain.
...
On chain~$Y$, after~$J$ is justified (from the finalize votes' target part in one block), those votes' finalize parts register in subsequent blocks (once the chain's $h_j$ equals~$h_j$ and $J \preceq v.\target \preceq B$), growing~$P$ toward finalization.
...
For Layer~2: validator~$i$ voted ($\targets[i] \neq \bot$), so the participation check is satisfied.
For the finalize check: after one block of confirmation, $i$ sends a finalize vote with $\finalizeheight = h_j$ and $\finalizetarget = J$, so $i \in P$.
...
\midrule
Fields & $(\val,\, \height,\, \target,\, \finh,\, \fintarget)$ & $(\val,\, \height,\, \target,\, \kind)$ \\
Finalize commitment & Via $\finh$/$\fintarget$ fields on any vote & Via $\kind = \finalize$ (height and target are the commitment) \\
Vote kinds & One (finalize fields optional) & Two: $\prefix$ and $\finalize$ \\
\bottomrule
\end{tabularx}
\end{center}

The finalize fields ($\finalizeheight$, $\finalizetarget$) are replaced by a $\kind$ field.
For a finalize vote, the vote's own $\height$ and $\target$ serve as the finalize height and finalize target, eliminating redundancy.
...
& \textbf{Prefix-IC} & \textbf{Portable Finalization} \\
\midrule
Condition & $d_1.\target \neq d_2.\fintarget$ (equality) & $a.\target \not\succeq b.\target$ where $b.\kind = \finalize$ (compatibility) \\
Effect & Vote target must exactly match finalize target & Vote target must descend from finalize vote's target \\
\bottomrule
\end{tabularx}
\end{center}

Two changes: (1)~the relaxation from equality to compatibility, enabling validators to vote for head $\succeq J$ while finalizing~$J$; (2)~the condition references $\kind$ instead of separate finalize fields.
...
& \textbf{Prefix-IC} & \textbf{Portable Finalization} \\
\midrule
Prefix vote & target $=$ head, finalize fields optional & $\kind = \prefix$, target $=$ head \\
Finalize vote & --- (finalize fields on prefix vote) & $\kind = \finalize$, target $= J$, height $= h_j$ \\
