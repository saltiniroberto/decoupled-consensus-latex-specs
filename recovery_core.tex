...
Ordinary Goldfish proposer selection is receiver-local: at vote time
each validator takes the winning eligible proposal among the round
proposals in its own inclusive vote-time view, as the source's
algorithm prescribes, so two honest validators holding different
proposal sets may take different winners.  Recognition additionally
requires the winner to be timely --- received strictly before the vote
time, per Definition~\ref{def:stable-root} --- so a boundary-arriving
winner yields the failed-proposal branch, never acceptance.  The \emph{round
...
input received strictly before \(d_r\) --- a function of the record,
which stores each input's first-receipt time --- so the immutable
record fixes both the full inclusive input and the discarded boundary
receipts.  This is one of the paper's two deliberate strict thresholds
against the inclusive view convention of
Assumption~\ref{ass:recovery-network}, whose boundary-irrelevance
sentence is scoped to delivery deadlines, not to schedule cutoffs; the
other is the timeliness test of Definition~\ref{def:stable-root}, a
strict receipt condition evaluated inside the inclusive vote-time
view.  The selection exclusion keeps the round's own proposal,
...
\]
for every round, the relay point \(a_0\) included,
...
viability view, so equal counted vote sets give equal weights.  (Goldfish has
...
evidence at their round-\(r\) cutoffs.  Equality is meant statewise at
each compared cutoff: the receiver's grade-root selection state and its
first-slot vote-time state each agree with the proposer's selection
state on the store maximum, the active justified and finalized pairs,
and the processed finalized-evidence set.  Then the receiver's aged
...
received every one of them strictly before \(a_{r-1}+\Delta\leq d_r\);
by the store's dependency rule the proposer's deterministic replay
(Definition~\ref{def:total-raw-replay}) then accepts the same blocks
with the same derived states, and the witness is accepted in the
...
selection-root identifier agree; the relayed ancestry placed the
receiver's root block in the proposer's store, and, under
Assumption~\ref{ass:crypto}, distinct objects have distinct roots, so
both stores resolve the shared identifier to the same block and the
selection roots agree as blocks.  Since state-height is nondecreasing
along an accepted chain, the witness has an accepted leaf descendant
--- possibly itself --- of state-height at least the threshold, so the
witness's subtree is viable and the witness places the same blocks in
the proposer's selection tree.
\end{proof}
...
\(B'\)'s subtree.  Conflicting blocks have disjoint subtrees, and both
evaluators resolve any counted vote to the same target --- a counted
vote's required ancestry is relayed with it under
Assumption~\ref{ass:recovery-network}, and, under
Assumption~\ref{ass:crypto}, distinct objects have distinct roots ---
so one vote cannot lie in both, and these are two distinct votes by
\(i\); both were relayed by
...
\(\operatorname{ts}_u(B)\).  Hence \(X_w\) holds exactly one vote by \(i\), the one
counted at \(u\).  That vote was interpretable in \(V_u^-\), so the
store's dependency rule placed its target block and full ancestry ---
including \(B\) --- in \(u\)'s store by the support freeze
\(f=d+2\Delta\); the vote was relayed with that ancestry under
Assumption~\ref{ass:recovery-network},
and, under Assumption~\ref{ass:crypto}, distinct objects have distinct
roots, so \(w\) resolves it under \(B\) and
\(\operatorname{supp}_{X_w}(i,B)\) holds.
...
validity is positive-only --- signatures and the eligible round --- and
...
receiver (Definition~\ref{def:recovery-proposal}): it is the
receiver's local selection winner, it is timely --- received strictly
before the vote time, which puts it in the inclusive vote-time view ---
...
its vote.  Then the votes counted at every honest committee member's
vote time are exactly those of \(\mathcal V_{p,\rm GF}^r\), every
counted vote's target resolves at every honest committee member to the
same block as at the proposer, and every honest committee member's
GHOST walk from the proposer's root \(A\) votes for the exact block
\(B\).
...
recognized by everyone hands every honest voter its exact counted vote
set, and carries the blocks those votes name, so every honest vote
names its block.  Safety of confirmations does not rest on
...
throughout.  Target resolution agrees because the carried view is
consistent and, under Assumption~\ref{ass:crypto}, distinct objects
have distinct roots.  The rooted form follows because equal counted
vote sets with agreeing target resolutions make the walk from the
proposer's root replay the proposer's own choice sequence; that lift is
part of this paper's interface and is not attributed to the source.
\end{remark}
...
\begin{proof}
All counted vote sets are equal, and every counted vote's target
resolves to the same block everywhere, by
...
with equal counted vote sets the weight of every child is the same
...
proposer's exact counted vote set with agreeing target resolutions, and
Lemma~\ref{lem:forward-root} makes the
...
pair when \(Q_i^r\) is empty.  When local winners differ, a winner
is untimely, or a winner equivocated, recognition may differ between
honest validators, so this
...
\Function{recovery\_round}{$r$}
  ...
  \State select the winning eligible proposal in the inclusive
    vote-time view; recognize it only if timely; use none if that
    view holds two signed round proposals from its proposer
...
\(K\nmid H-1\), and the flag \(\nj\) is set only at heights with
...
\begin{enumerate}
 \item \(H-h'>D_{\rm debt}\).  Then every branch entering height \(H\)
   ...
   condition --- that is, before any later case-2 advance --- sets
...
transition's post-finality state, and the flag then stays fixed until the
...
up to \(h'\) --- satisfies the debt inequality, every entrant sets
\(\nj=\mathsf{true}\), and branches that entered earlier keep the
flag.  For case~2, the first subcase restates the advance itself; the
...
fixes its nonjustifiable flag at its height-\(H\) entry from its own
on-chain post-finality state (Definition~\ref{def:nonjustifiable}), and
every descendant still at height \(H\) inherits the flag.  A relevant
