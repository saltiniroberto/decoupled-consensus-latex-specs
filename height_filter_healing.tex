...
\newcommand{\cascadeRoot}{\mathsf{cascadeRoot}}
\newcommand{\Gone}{G_1}
\newcommand{\Gzero}{G_0}
\newcommand{\GST}{\mathrm{GST}}
\newcommand{\liveConfirmed}{\mathsf{live\_confirmed\_head}}
\newcommand{\roundEnd}{\mathsf{ends\_round}}
...
An FG message, also called an FG vote or attestation, is a signed object~\(v\)
with fields
\[
...
Figure~\ref{alg:state-machine} gives the same rules as pseudocode.  Write
\(\roundEnd(s)\) when slot~\(s\) is the final slot of a round under the
schedule.  A live validator invokes \(\mathsf{process\_slot}\) on every slot
tick; \(\mathsf{state\_transition}\) replays the same intervening slot
transitions before a later block.  A boundary sets
...
\(\sigma.L.\slot<\sigma.s\) proves that no block was processed in the
inclusion-opportunity slot; otherwise \(\mathsf{process\_block}\) would already
have cleared the pending outcome.  A block never supplies a separate close flag.  The
...
transition twice.  The function and field names otherwise follow the
executable specification.  In particular,
\(\mathsf{is\_viable\_attestation\_target}\),
\(\mathsf{is\_timeout\_vote}\),
\(\mathsf{has\_new\_finalization}\),
\(\mathsf{compute\_justified\_checkpoint}\), and
\(\mathsf{has\_timeout\_quorum}\) implement the checks stated above.
...
\Function{process\_slot}{$\sigma$}
  ...
  \If{\(\sigma.\pendingHeightOutcome\)}
    \State \textbf{assert} \(\sigma.L.\slot<\sigma.s\)
      \Comment{The inclusion-opportunity slot was empty}
    \State \(\sigma\gets
...
\Function{process\_block}{$\sigma,B$}
  ...
  \ForAll{attestations \(a\in B.\mathsf{body}.\mathsf{attestations}\)}
...
\Function{process\_attestation}{$\sigma,a$}
  \State \(d\gets a.\mathsf{data}\)
  \State \(\mathsf{validate\_attestation}(\sigma,a)\)
  \State \(\mathit{viable}\gets
    \mathsf{is\_viable\_attestation\_target}(\sigma,a)\)
  \State \(\mathit{timeout}\gets\mathsf{is\_timeout\_vote}(d)\)
  \ForAll{\(i\in\mathsf{get\_attesting\_indices}(\sigma,a)\)}
    \If{\(i\in V\), \(\sigma.h_j>\sigma.h_F\),
        \(\sigma.F\preceq\sigma.J\),
        \(d.\finalizeHeight=\sigma.h_j\), and
        \(d.\finalizeTarget=\sigma.J\)}
      \State \(\sigma.P\gets\sigma.P\cup\{i\}\)
    \EndIf
    \If{\(i\in V\) and \(\mathit{viable}\)}
...
\Function{process\_justification\_and\_finalization}{$\sigma,\mathit{height\_start\_slot}$}
  \If{\(\mathsf{has\_new\_finalization}(\sigma)\)}
    \State \((\sigma.F,\sigma.h_F)\gets(\sigma.J,\sigma.h_j)\)
  \EndIf
  \State \(T\gets\mathsf{compute\_justified\_checkpoint}(\sigma)\)
  \If{\(T\neq\bot\)}
    \State \Return \(\Call{advance\_height}{\sigma,T,\mathit{height\_start\_slot}}\)
  \EndIf
  \If{\(\mathsf{has\_timeout\_quorum}(\sigma)\)}
    \State \Return
...
The fresh-root pointer is not a separate store object.  As in the executable
specification, it is the \(\mathsf{anchor\_root}\) field of a round-start
block.  The \(\mathsf{on\_block}\) path retains that field with the signed
block.
Consequently, not observing a round-start block is different from observing
one with an explicit empty \(\mathsf{anchor\_root}\).
\end{definition}
...
\(\mathsf{anchor\_root}\) values signed by the round's proposer, the step uses
...
the anchor chosen for its round.  Every later \textsf{get\_head} call in that
round reads this anchor; later votes do not cause the pointer to be accepted or
rejected again.  A completed result is usable only when its predecessor chain
...
\Function{on\_block}{$S,\mathit{signed\_block}$}
  ...
  \ForAll{scheduled steps \(I\) whose input or validation uses
    ...
    \EndIf
  \EndFor
  \ForAll{\(a\in B.\mathsf{body}.\mathsf{attestations}\)}
    \State \(S\gets\Call{on\_attestation}{S,a}\)
  \EndFor
  \State \Return \(S\) \Comment{\(B.\mathsf{body}.\mathsf{anchor\_root}\) stays with \(B\)}
\EndFunction
\vspace{2pt}
...
\Function{derive\_fork\_choice}{$S$}
  ...
  \State \(R\gets J\) if \(\hmax=h_j+1\), else \(R\gets F\)
  \State \Return \(\Sigma=(\sigma,\T,F,h_F,J,h_j,\hmax,\viableTree,R)\)
\EndFunction
\vspace{2pt}

\Function{get\_head}{$S,I,\Omega,u$}
  \State \(\Sigma\gets\Call{derive\_fork\_choice}{S}\)
  \State \(A\gets\) the anchor in the unique history-compatible result for \(I\)
  \State \Return \(\mathsf{walk}(\Sigma,\Omega,u;A)\)
\EndFunction
...
\(\viableTree(\Sigma)\) for the viable subtree.  The cascade root is
\[
  \cascadeRoot(\Sigma)=
  \begin{cases}
...
\(\viableTree(\Sigma)\) and to descend from~\(\cascadeRoot(\Sigma)\).
\end{definition}
...
impossible.  When \(\Sigma.h_{\max}=H+2\), the cascade can instead select only
a height-\(H+1\) justification.  Lemma~\ref{lem:cascade-compatible} shows that any such root
...
The \emph{grade-1 fallback anchor} \(\Gone(\Sigma,u)\) starts at the cascade
root~\(\cascadeRoot(\Sigma)\) and repeatedly enters the unique viable child~\(c\) with
\(3\Supp_u(c)\geq2\Dvote_u\), stopping when no such child exists.  If
\(\Dvote_u=0\), it returns the cascade root.
...
\(\cascadeRoot(\Sigma)\preceq B\), and \(B\) is viable, set the anchor~\(A=B\).
Otherwise set \(A=\Gone(\Sigma,u)\).  From~\(A\), follow
...
\(\mathsf{walk}(\Sigma,\Omega,u;A)\).  The available-vote step stores~\(A\);
...
\(R=\cascadeRoot(\Sigma)\), and Goldfish available-confirmed head
\(\liveConfirmed_u\).  The cascade root~\(R\) is always safe-confirmed.  If
...
Assumption~\ref{ass:goldfish} holds.  Under \(b<2q-W\), every cascade root
...
For a pointed anchor, let~\(B_C\) be the highest common ancestor of the
...
\(h_{\max}=H+1\), no height-\(H\) justification exists and the cascade root
is~\(F\), so FG
