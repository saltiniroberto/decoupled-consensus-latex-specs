...
Unless $\geq n/3$ validators are slashable: if~$F$ is finalized at height~$h_F$ and some block~$B^\star$ whose post-state has $(F, h_F)$ as its justified checkpoint has been processed by the node, then $R_s \succeq F$ at all future times.
\end{lemma}

\begin{proof}
Fix the moment when $\textsf{onBlock}(B^\star)$ offered the descriptor $(h_F,\, 1,\, F)$ to $\textsf{updateForkChoiceRoot}$, and let $F_s^0$ be the value of $\Sigma.F_s$ at that moment.
