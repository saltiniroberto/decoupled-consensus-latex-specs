...
A \notar vote~$v$ is processed on a chain with head state $\sigma[L]$ only if $v.\height = \sigma[L].h$: the chain is at~$v$'s height; $v.\target$ plays no role in the freshness check.
...
\]
Notarization does not require a common target; a \notar vote is fresh on this chain whenever $v.\height = h$, regardless of $v.\target$. The notarized block is the chain head~$L$.
...
Any vote with $v.\height = \sigma.h$ updates $\voteSlots[i] \gets \max(\voteSlots[i],\, v.\slot)$ (treating~$\bot$ as $-\infty$); a \just vote additionally passes the target-freshness check and, if it does, sets $\jtargets[i] \gets v.\target$.
...
\Function{\processVote}{$\sigma,\; v$}
  ...
  \If{$v.\height = \sigma.h$} \Comment{height freshness --- enough for \notar}
    \State $\sigma.\voteSlots[i] \gets \max(\sigma.\voteSlots[i],\, v.\slot)$
    \If{$v.\kind = \just$ \textbf{and} $v.\target \preceq \sigma.L$ \textbf{and} $v.\target.\slot \geq \sigma.s_h$}
...
Only \emph{justified} cert events (height-advances fired by \processHeight's justification branch) are compared against $\key_R$ in lex order; whenever such an event's key strictly dominates, $(R_s, \key_R)$ is updated.
The key packages height, the justified-bit (always~$1$ for events that reach~$R_s$), the cert event's slot component, and a block hash, so that among justified events at the same height the more recent (higher vote-slot) one wins.
...
\begin{itemize}
  ...
  \item a \emph{slot component} $s \in \mathbb{N}$: the highest slot for which a $2n/3$-quorum of contributing votes all have $\slot \geq s$ (\Cref{def:justification}, \Cref{def:notarization});
...
\[
  \key(X, h, s, b) \;\coloneqq\; \bigl(h,\; b,\; s,\; \hash(X)\bigr).
\]
\end{definition}

A justification also notarizes its chain head (the justification branch sets $(J, N) \gets (T, L)$ with shared slot component), but we classify the combined event as a justification with key $\key(T, h, s, 1)$.
A given chain may therefore fire a bit-$0$ cert event on one occasion and a bit-$1$ cert event on another, with the bit-$1$ key strictly dominating at the same $(h, s)$.
...
\end{itemize}
The genesis store has $\T = \{\genesis\}$, $F_s = R_s = \genesis$, and $\key_R = (0,\, 1,\, 0,\, \hash(\genesis))$, treating genesis as a bit-$1$ cert event at height~$0$ with slot component~$0$.
\end{definition}
...
The acceptance routine \onBlock (\Cref{alg:store}) admits $B$ only if $B.\parent \in \Sigma.\T$ and $\Sigma.F_s \preceq B$.
...
The post-state then offers the justified-checkpoint descriptor $(\sigma[B].h_j,\, 1,\, \sigma[B].s_j,\, \sigma[B].J)$ to \updateForkChoiceRoot; notarization-only events do not update~$R_s$.
...
\State \textbf{Genesis store} $\Sigma$:\ $\Sigma.\T \gets \{\genesis\},\ \Sigma.F_s \gets \genesis,\ \Sigma.R_s \gets \genesis,\ \Sigma.\key_R \gets (0,\, 1,\, 0,\, \hash(\genesis)),$
...
\Function{\onBlock}{$\Sigma,\; B$}
  ...
  \State $\Sigma \gets \updateForkChoiceRoot(\Sigma,\; (\sigma'.h_j,\; 1,\; \sigma'.s_j,\; \sigma'.J))$ \Comment{Only justified checkpoints update $R_s$}
  \State $\Sigma \gets \updateFinalized(\Sigma,\; \sigma'.F)$
  \State \Return $\Sigma$
\EndFunction
\vspace{3pt}

\Function{\updateForkChoiceRoot}{$\Sigma,\; (h,\, b,\, s,\, C)$} \Comment{Filter by $\Sigma.F_s \preceq C$, then running max}
  \State $\newKey \gets (h,\; b,\; s,\; \hash(C))$
...
The $F_s$-filter never discards a key-dominant event: \Cref{thm:fleqr} below shows that $F_s$ advances only to blocks $\preceq R_s$, so every cert event whose key could beat $R_s$ remains on the $F_s$-chain.
...
At all times $\Sigma.F_s$ is either~$\genesis$ or a block that was finalized on some chain; moreover, whenever a justification $(J, h)$ with slot component $s_j$ has fired on some processed chain, the descriptor $(h,\, 1,\, s_j,\, J)$ was offered to \updateForkChoiceRoot, so $\Sigma.\key_R \geq (h,\, 1,\, s_j,\, \hash(J))$ for every such $(J, h, s_j)$ with $J \succeq \Sigma.F_s$.
...
Since only justified (bit-$1$) descriptors reach \updateForkChoiceRoot, $\Sigma.\key_R.b = 1$ at all times.
\end{remark}
...
\begin{proof}
By \Cref{thm:finperm} and~\Cref{thm:fleqr}, $F \preceq F_s \preceq R_s$, and \getHead returns a descendant of~$R_s$.
\end{proof}
...
\begin{proof}
Fix the moment when $\onBlock(B)$ offered the descriptor $(h_F,\, 1,\, s_F,\, F)$ to \updateForkChoiceRoot, and let $F_s^0 = \Sigma.F_s$ at that moment.
Both $F_s^0$ and $F$ are finalized or genesis (\Cref{rem:fs-invariant}), so \Cref{lem:finchain} gives $F_s^0 \preceq F$ or $F \preceq F_s^0$.

\emph{Case $F \preceq F_s^0$.}
$R_s \succeq F_s^0 \succeq F$ already holds by \Cref{thm:fleqr}; \Cref{thm:finperm} preserves $F_s \succeq F_s^0$ and \Cref{thm:fleqr} preserves $R_s \succeq F_s$ at all future times, so $R_s \succeq F$ persists.

\emph{Case $F_s^0 \preceq F$.}
The filter passes, so after the call $\Sigma.\key_R \geq (h_F,\, 1,\, s_F,\, \hash(F))$; \Cref{lem:Rs-key-monotone} preserves this bound thereafter.
Write $h_R \coloneqq \Sigma.\key_R.h$; the bound gives $h_R \geq h_F$, and \Cref{rem:fs-invariant} gives $\Sigma.\key_R.b = 1$.
If $h_R > h_F$, \Cref{lem:certchain} applied to the cert event $(R_s,\, h_R,\, \cdot,\, 1)$ gives $F \prec R_s$.
If $h_R = h_F$, $R_s$ was justified at height~$h_F$ on some processed chain, giving a \just-quorum~$Q'$ of size $\geq 2n/3$ with target~$R_s$ at height~$h_F$.
Finality of~$F$ at~$h_F$ gives a quorum~$Q_F$ with $\finalizeHeight = h_F$ and $\finalizeTarget = F$.
By quorum intersection, $|Q' \cap Q_F| \geq n/3$; pick $v \in Q' \cap Q_F$ not slashable.
E1 (\Cref{def:slashing}) applied to $v$'s votes in~$Q', Q_F$ forces $R_s = F$.
\end{proof}
...
Otherwise $F_s$ is genesis or finalized (\Cref{rem:fs-invariant}), so \Cref{lem:finchain} gives $F_s \sim F_B$ and hence $F_s \prec F_B$.
$F_B$ being finalized in~$B$'s post-state requires some ancestor~$B' \preceq B$ to have post-state $(J_{B'},\, h_j^{B'}) = (F_B,\, h_{F_B})$; $B'$ was processed before~$B$ (parent-first), so \Cref{lem:upgrade} gives $R_s \succeq F_B$, and the \updateFinalized guard passes.
\end{proof}
...
\begin{proof}
\Cref{lem:upgrade} gives $R_s \succeq F$ directly.
...
\emph{$F_s$.} By \Cref{lem:finchain}, $\{\sigma[B].F : B \in \T\}$ is a chain with maximum $F_{\max}$, attained at some processed~$B^\ast$.
\Cref{rem:fs-invariant} forces $F_s$ into this chain, so $F_s \preceq F_{\max}$; once $B^\ast$ has been processed, \Cref{thm:finlive} gives $F_s \succeq F_{\max}$.

\emph{$R_s$.} The multiset~$D$ of descriptors offered to \updateForkChoiceRoot is a deterministic function of the processed set: each block contributes one justified-checkpoint descriptor exactly once.
Write $K_{\max} \coloneqq (h_{F_{\max}},\, 1,\, s_{F_{\max}},\, \hash(F_{\max}))$, where $s_{F_{\max}}$ is the slot component attached to $F_{\max}$'s justification.
For any $(h, 1, s, C) \in D$ with $h \geq h_{F_{\max}}$, \Cref{lem:certchain} gives $C \succeq F_{\max}$ (strict when $h > h_{F_{\max}}$); at $h = h_{F_{\max}}$, a competing descriptor has bit~$1$, hence a \just-quorum~$Q_C$ of size $\geq 2n/3$ targeting~$C$ at height~$h_{F_{\max}}$, while finality of~$F_{\max}$ gives a quorum~$Q_F$ with $\finalizeHeight = h_{F_{\max}}$ and $\finalizeTarget = F_{\max}$; quorum intersection and E1 (as in \Cref{lem:upgrade}) force $C = F_{\max}$.
...
Hence $R_s$'s final value is the lex-max over $\{(h, 1, s, C) \in D : h \geq h_{F_{\max}}\}$, a deterministic function of the processed set.
\end{proof}
...
At each voting opportunity, $i$ constructs its vote by $\constructVote(V,\, \Sigma)$, defined in \Cref{alg:voting-rule}.
...
Unless $\geq n/3$ validators are slashable, and assuming $f < n/3$ and all honest validators follow \Cref{def:voting-rule}: let~$i$ be honest with local store~$\Sigma$, $X = \getHead(\Sigma)$, and $h_c = \sigma[X].h$.
...
\begin{proof}
Write $h_R \coloneqq \Sigma.\key_R.h$.
By \Cref{rem:fs-invariant}, $\Sigma.\key_R.b = 1$, i.e., $R_s$ is justified at height~$h_R$ on some processed chain.
$X$ descends from~$R_s$ (\Cref{alg:store}), so state-height monotonicity (\Cref{lem:slotmono}) gives $h_c \geq h_R$ and $\sigma[R_s].h = h_R$.
...
\Cref{lem:no-finalize-without-just} gives the descriptor $(h_c, 1, s^\star, L)$ offered to \updateForkChoiceRoot at that time.
The filter $\Sigma.F_s \preceq L$ passes under $f < n/3$ (\Cref{rem:fs-invariant} makes $\Sigma.F_s$ finalized or genesis; \Cref{lem:certchain} with $h_c \geq h_{F_s}$ gives $\Sigma.F_s \preceq L$), so \updateForkChoiceRoot raised $\Sigma.\key_R.h$ to $\geq h_c$; \Cref{lem:Rs-key-monotone} preserves this bound.
But $\Sigma.\key_R.h = h_R < h_c$, contradiction.
Hence the locked case does not arise when $h_c > h_R$, and $T_c \preceq X$ with $\sigma[T_c].h = h_c$ from the unlocked branch.

\emph{Locked case, $h_c = h_R$.}
\Cref{lem:no-finalize-without-just} gives $L$ justified at~$h_c$ on some processed chain; $\Sigma.\key_R.b = 1$ gives $R_s$ justified at~$h_R = h_c$ on some processed chain.
\Cref{lem:just-unique-height} then forces $L = R_s$, so $T_c = L = R_s \preceq X$ with $\sigma[T_c].h = \sigma[R_s].h = h_c$.
\end{proof}

\begin{theorem}[Layer~1 leak fairness]\label{thm:fairness}
Unless $\geq n/3$ validators are slashable, and assuming $f < n/3$ and all honest validators follow \Cref{def:voting-rule}: let~$i$ be an honest validator, $\Sigma$ its local store, $X = \getHead(\Sigma)$ its canonical chain, and $h_c = \Sigma.\sigma[X].h$ the current canonical state-height.
Let $v$ be the vote~$i$ produces at the current slot.
...
The $f < n/3$ hypothesis is used only in \Cref{lem:canonical-fresh-honest-vote} via \Cref{lem:just-unique-height}, to collapse the lock target~$L$ onto $R_s$ in the $h_c = h_R$ case.
