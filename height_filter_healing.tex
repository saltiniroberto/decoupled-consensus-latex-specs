...
A target vote is valid on a chain at height \(h\) only when its target is
...
height to contribute to height advancement without violating E1.  A timeout
vote is valid only at the chain's current height.  A finalize pair equal to
...
a valid vote for the stored current-height target~\(T_h\).  The bit
\(\timeouts[i]\) records whether it has made either such a target vote or a
valid current-height timeout vote.  Thus
\[
...
set \(\targetParticipation[i]\) is the stored target~\(T_h\).  The signed messages are kept so
...
The attestations in a block \(B\) are treated as a set.  A valid vote for
the current-height target sets both
\(\targetParticipation[i]\) and \(\timeouts[i]\); a valid timeout sets
only \(\timeouts[i]\).  A valid finalize commitment to the prestate's current
...
Since a chain stores one current-height target, all valid target votes
increment the same bitlist.

A target vote therefore has two independent effects: it contributes to
...
\Function{process\_attestation}{$\sigma,a$}
  ...
  \State \(\mathit{timeout\_vote}\gets
    (a.\height=\sigma.h)\wedge(a.\target=\bot)\)
  ...
  \If{\(\mathit{target\_vote}\)}
    \State \(\sigma.\targetParticipation[i]\gets\mathsf{true};\quad
      \sigma.\timeouts[i]\gets\mathsf{true}\)
  \ElsIf{\(\mathit{timeout\_vote}\)}
...
current-height target votes, timeout votes, or both, from distinct
validators with weight at least \(q\).  When no target quorum is selected first, a
...
strict ancestor of the block containing the vote.  The target is initialized
...
current-height target is also~\(T\), and it accepts the vote.  Distinct targets
can be accepted only on conflicting branches.
\end{lemma}
...
state and has already recorded~\(T\).  The target-validity rule then accepts
the vote.
\end{proof}
...
signed either a valid target or a valid timeout is
...
non-slashable signer in the relevant intersection.  Its height vote cannot
...
the retained \(\mathsf{JC}(k,T_L)\), give \(T_L=T\) under \(b<2q-W\).
...
votes can be processed.  Thus the exact-target validity check accepts them on
the common chain.
