...
Section~\ref{sec:healing} extends $\Sigma$ with a vote pool $\Sigma.V$.
\end{definition}
...
We further extend $\Sigma$ with a vote pool~$\Sigma.V$ populated by view-merge.
...
\subsection{Adversary card inventory}\label{sec:card-inventory}

The adversary may withhold and selectively release justification certificates to disrupt canonical inside \getConfirmed{}.
The next lemma bounds this disruption to two cards under $f < n/3$, which underpins the staggered consumption argument in Lemma~\ref{lem:healing-3slot}.

\begin{lemma}[Adversary card inventory under $f < n/3$]\label{lem:adversary-cards}
Assume $f < n/3$ and all honest validators follow Definition~\ref{def:voting-rule}.
Fix a slot~$k$ with an honest syncer, and let $h^0 \coloneqq \comm{\Sigma}^{\vote(k)}.\hmax$, the post-merge frontier height (uniform across honest stores at $\vote(k)$ by Assumption~\ref{ass:view-merge}).
The adversary holds at most two distinct justification certificates that, when delivered to~$\Sigma$, both pass the lex test inside \updateJustified{} \emph{and} satisfy the cascade gate $\Sigma.\hmax = \Sigma.h_j + 1$ inside \getConfirmed{}:
\begin{itemize}
\item[(i)] a unique justification $J^*$ at height~$h^0 - 1$, carried by a processed block $B^*$ with $\sigma[B^*].h = h^0$;
\item[(ii)] a unique justification $J^\dagger$ at height~$h^0$, carried by a processed block $B^\dagger$ with $\sigma[B^\dagger].h = h^0 + 1$.
\end{itemize}
Justifications at heights $< h^0 - 1$ may be released but never satisfy the gate after release; justifications at heights $> h^0$ do not exist; and a timeout-only chain advancing $\sigma$-height to~$h^0 + 1$ on a sibling of card~(ii)'s chain cannot coexist with~(ii).
\end{lemma}

\begin{proof}
\emph{Cards (i), (ii) when present are unique.}
Lemma~\ref{lem:just-unique-height} forces at most one justified block per height; carrying-block heights follow from the height-advance rule (Lemma~\ref{lem:slotmono}: a justification at~$h$ produces a containing block of $\sigma$-height $h + 1$).
On release of card~(i), $\Sigma.\hmax \gets \max(\Sigma.\hmax, h^0) = h^0$ and $\Sigma.h_j \gets h^0 - 1$ (when the lex test passes), leaving $\Sigma.\hmax = h^0 = \Sigma.h_j + 1$ and the gate fires.
On release of card~(ii), $\Sigma.\hmax \gets h^0 + 1$ and $\Sigma.h_j \gets h^0$, again with $\Sigma.\hmax = \Sigma.h_j + 1$.

\emph{Heights below $h^0 - 1$ fail the gate.}
A justification at height $h < h^0 - 1$ on any chain, when released, sets $\Sigma.h_j \gets h$ and bumps $\Sigma.\hmax$ to at most $\max(\Sigma.\hmax, h + 1) \leq h^0$.
Since $\Sigma.\hmax \geq h^0 > h + 1 = \Sigma.h_j + 1$ after the release, the gate fails, $R = \Sigma.F$ inside \getConfirmed{}, and the released card does not shift canonical.

\emph{Heights above $h^0$ do not exist.}
Any quorum of size $\geq 2n/3$ requires at least $2n/3 - f > n/3$ honest contributions, but no honest validator has cast a non-$\bot$ vote at height $> h^0$ pre-slot-$k$: each honest's voting height $h_c = \sigma[\getConfirmed_v].h$ at any prior voting time satisfies $h_c \leq \Sigma_v.\hmax$, and $\Sigma_v.\hmax \leq h^0$ in every honest history before $\start(k)$ by definition of~$h^0$.
Hence no justification at height $> h^0$ exists.

\emph{Timeout-only chain at $\sigma$-height $h^0 + 1$ excluded jointly with~(ii).}
Suppose card~(ii) exists on chain $Y^\dagger$ and the adversary also presents a chain $Y^\circ$ disjoint from $Y^\dagger$ that reaches $\sigma$-height $h^0 + 1$ via a timeout cert at height~$h^0$ (per-height uniqueness rules out a separate justification at $h^0$ on $Y^\circ$).
Card~(ii) on $Y^\dagger$ provides a justification quorum~$Q^\dagger$ of size $\geq 2n/3$ at height~$h^0$ on $Y^\dagger$; the timeout cert on $Y^\circ$ provides a timeout quorum~$Q^\circ$ of size $\geq 2n/3$ at height~$h^0$ on~$Y^\circ$.
By Definition~\ref{def:voting-rule}, an honest validator casts at most one vote per height, and the target of a non-$\bot$ vote on~$Y^\dagger$ at height~$h^0$ excludes a $\bot$-target timeout vote at the same height (and conversely).
Hence no honest validator contributes to both quorums, so the joint honest contributions sum to at most $n - f$, while individually they sum to at least $2(2n/3 - f) = 4n/3 - 2f$.
The combined inequality $4n/3 - 2f \leq n - f$ rearranges to $f \geq n/3$, which together with the standing $f < n/3$ rules out the joint configuration.
\end{proof}
...
\subsection{Advance}\label{sec:healing-advance}

\begin{lemma}[Three-slot advance from arbitrary state]\label{lem:healing-3slot}
Assume $f < n/3$ and that all honest validators follow Definition~\ref{def:voting-rule}.
Let~$k$ be a slot.
If slots~$k+1, k+2, k+3$ each satisfy the standing assumptions and have honest syncers, then there exists $k_h \in \{k+1, k+2, k+3\}$ such that the system is in a healed state at slot~$k_h$ with some frontier height $\comm{h} \geq h^0 - 1$, where $h^0$ is the post-async frontier height of Lemma~\ref{lem:adversary-cards}.
\end{lemma}

\begin{proof}[Proof sketch]
By Lemma~\ref{lem:adversary-cards}, the adversary holds at most two disruptive cards: $J^*$ at height~$h^0 - 1$ and $J^\dagger$ at height~$h^0$, carried by blocks of $\sigma$-heights~$h^0$ and~$h^0 + 1$ respectively.
For each slot~$\ell \in \{k+1, k+2, k+3\}$, view-merge under the honest syncer (Assumption~\ref{ass:view-merge}) makes $\Sigma_v^{\vote(\ell)}$ uniform across honest~$v$, and Definition~\ref{def:standing}(d) ensures slot-$\ell$ honest votes are aggregated into the slot-$\ell$ block by $\vote(\ell+1) - \Delta$, firing the slot-$\ell$ honest virtual justification at canonical's $h_c^{(\ell)} \coloneqq \sigma[\getConfirmed^{\vote(\ell)}].h$ via \updateJFromVotes{}.

\emph{Card consumption.}
Each release of $J^*$ or $J^\dagger$ pre-vote at slot~$\ell$ updates $\comm{\Sigma}^{(\ell)}.J$ via the lex test, with the cascade gate firing at the moment of release; canonical reorgs onto the released chain, $h_c^{(\ell)}$ matches or exceeds the released card's height, and the slot-$\ell$ honest virtual cert at $h_c^{(\ell)}$ raises $\Sigma.h_j$ accordingly.
A second card released within the same slot is dominated on lex by the first ($J^\dagger > J^*$ in lex height): if $J^\dagger$ has fired first, the next $J^*$ release is rejected by the lex test; if $J^*$ has fired first, $J^\dagger$ dominates and absorbs $J^*$.
At most one card is consumed per slot in the worst-case stagger.

\emph{Case analysis} (which cards released in which slots, up to symmetry):
\begin{itemize}
  \item \emph{No release.} Slot-$(k+1)$ honest virtual cert fires at $h_c^{(k+1)} \geq h^0 - 1$; $k_h = k+1$.
  \item \emph{Single card released at slot~$\ell_B \in \{k+1, k+2\}$.} The slot-$\ell_B$ honest virtual cert at $h_c^{(\ell_B)} \geq h^0 - 1$ absorbs the card; from slot~$\ell_B + 1$ no further disruption occurs; $k_h = \ell_B + 1$.
  \item \emph{Both cards released at the same slot~$\ell_C$.} $J^\dagger$ dominates $J^*$ via lex; the slot-$\ell_C$ honest virtual cert at $h_c^{(\ell_C)} \geq h^0$ then absorbs $J^\dagger$; $k_h = \ell_C + 1$.
  \item \emph{Forward stagger ($J^*$ at $k+1$, $J^\dagger$ at $k+2$).} Slot-$(k+1)$ virtual cert fires at $h_c^{(k+1)} \geq h^0 - 1$; slot-$(k+2)$ release of $J^\dagger$ dominates by lex, canonical reorgs to $J^\dagger$'s chain, slot-$(k+2)$ virtual cert at $h_c^{(k+2)} \geq h^0$ absorbs $J^\dagger$; $k_h = k+3$.
  \item \emph{Reverse stagger ($J^\dagger$ at $k+1$, $J^*$ at $k+2$).} $J^*$'s lex height $h^0 - 1$ falls below the current $\Sigma.h_j = h^0$, so the lex test rejects it; $J^*$ is wasted, $k_h = k+2$.
\end{itemize}
The worst case is forward stagger, giving $k_h = k+3$; in every case $k_h \leq k+3$.

\emph{Healed-state conditions at $\vote(k_h)$.}
Set $\comm{h} \coloneqq h_c^{(k_h - 1)} \geq h^0 - 1$.
\begin{enumerate}
  \item View-merge gives $\comm{\Sigma}^{(k_h)}.h_j \geq \comm{h}$ from the slot-$(k_h-1)$ virtual cert; the gate $\comm{\Sigma}^{(k_h)}.\hmax = \comm{\Sigma}^{(k_h)}.h_j + 1$ holds (no further card released after slot~$k_h - 1$ in the relevant case); $R_v^{\vote(k_h)} = \comm{\Sigma}^{(k_h)}.J$ has $\sigma$-height $\geq \comm{h}$.
  \item View-merge under the honest slot-$k_h$ syncer makes $\Sigma_v^{\vote(k_h)} = \comm{\Sigma}^{(k_h)}$ uniform across honest, hence $\getConfirmed_v^{\vote(k_h)} \sim \getConfirmed_{v'}^{\vote(k_h)}$.
  \item Pick $v_\ell$ honest with $\preceq$-maximal $\getConfirmed^{\vote(k_h)}$ on the common chain.
  Honest votes~$u$ cast strictly before~$\vote(k_h)$ with $u.\height \geq \comm{h}$ split into:
  \begin{itemize}
    \item votes contributing to a justification at height $\geq \comm{h}$ (already absorbed by some virtual cert in $\{k+1, \ldots, k_h - 1\}$): per Lemma~\ref{lem:just-unique-height}, the unique justified target at each such height lies on canonical at $\vote(k_h)$, and freshness of the contributing vote on its chain forces $u.\target$ onto canonical;
    \item slot-$\ell$ honest votes for $\ell \in \{k+1, \ldots, k_h - 1\}$: by Lemma~\ref{lem:honest-vote-sets-timeout} each such vote targets the then-canonical, and Lemma~\ref{lem:getConfirmed-monotone} together with chain compatibility through the absorbed cards gives $u.\target \preceq \getConfirmed_{v_\ell}^{\vote(k_h)}$.
  \end{itemize}
\end{enumerate}
\end{proof}
