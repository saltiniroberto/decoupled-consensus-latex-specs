...
Each chain maintains a \emph{state} $(L,\; h,\; \mathsf{targets},\; \mathsf{secondaries},\; J,\; h_j,\; s_n,\; F,\; P)$.
The \emph{head}~$L$ is the leaf block of the chain; a block~$B$ is on the chain iff $B \preceq L$.
The \emph{current height}~$h$ is the finality gadget's counter.
\begin{itemize}
  \item $\mathsf{targets}[1..n]$: the \emph{primary target tracker}.
        For each validator~$i$, the block that~$i$ voted for with a primary vote at the current height ($\mathsf{targets}[i] = \bot$ if~$i$ has not cast a primary vote).
  \item $\mathsf{secondaries}[1..n]$: the \emph{secondary target tracker}.
        For each validator~$i$, the block that~$i$ voted for with a secondary vote at the current height ($\mathsf{secondaries}[i] = \bot$ if~$i$ has not cast a secondary vote).
  \item $J$: the \emph{justified block}, the highest justified block on this chain, justified at \emph{justified height}~$h_j$.
...
The genesis state has $L = B_{\mathrm{genesis}}$, $h = 0$, all $\mathsf{targets}[i] = \bot$, all $\mathsf{secondaries}[i] = \bot$, $J = F = B_{\mathrm{genesis}}$, $h_j = 0$, $s_n = 0$, and $P = \emptyset$.
\end{definition}
...
The state evolves by processing blocks.
Each block extends the chain (updating the head~$L$), then its votes are processed, and finally finality, justification, and notarization are evaluated.

\emph{Processing a vote.}
When a vote~$v$ is processed:
if $v.\mathsf{kind} = \mathsf{primary}$, $v.\mathsf{height} = h$, and $v.\mathsf{target} \preceq L$, then $\mathsf{targets}[v.\mathsf{validator}] \gets v.\mathsf{target}$.
If $v.\mathsf{kind} = \mathsf{secondary}$, $v.\mathsf{height} = h$, and $v.\mathsf{target} \preceq L$, then $\mathsf{secondaries}[v.\mathsf{validator}] \gets v.\mathsf{target}$.
Independently, if $v.\mathsf{finalize\_height} = h_j$, then the validator is added to~$P$.
...
If justification does not fire, notarization is checked: the chain is walked from~$L$ toward genesis, accumulating the count of secondary votes.
...
\State \textbf{State:} $L \gets B_{\mathrm{genesis}}$, \; $h \gets 0$, \; $\mathsf{targets}[1..n] \gets \bot$, \; $\mathsf{secondaries}[1..n] \gets \bot$, \; $J \gets B_{\mathrm{genesis}}$, \; $h_j \gets 0$, \; $s_n \gets 0$, \; $F \gets B_{\mathrm{genesis}}$, \; $P \gets \emptyset$
\vspace{3pt}

\Function{$\textsf{processBlock}$}{$B$}
  \State \textbf{assert} $B.\mathsf{parent} = L$ \Comment{$B$ extends the chain}
  \State $L \gets B$
  \For{each vote $v$ in $B$} $\textsf{processVote}(v)$ \EndFor
  \State $\textsf{processHeight}()$
\EndFunction
\vspace{3pt}

\Function{$\textsf{processVote}$}{$v$}
  \State $i \gets v.\mathsf{validator}$
  \If{$v.\mathsf{height} = h$ and $v.\mathsf{kind} = \mathsf{primary}$ and $v.\mathsf{target} \preceq L$}
    \State $\mathsf{targets}[i] \gets v.\mathsf{target}$
  \EndIf
  \If{$v.\mathsf{height} = h$ and $v.\mathsf{kind} = \mathsf{secondary}$ and $v.\mathsf{target} \preceq L$}
...
\Function{$\textsf{processHeight}$}{}
  ...
  \If{$|\{i : \mathsf{targets}[i] = T^*\}| \geq 2n/3$} \Comment{Justification}
    ...
    \State \Return
  \EndIf
  \State $\mathsf{count} \gets 0$ \Comment{Notarization: walk from $L$ toward genesis}
  \For{$B'$ from $L$ down to $B_{\mathrm{genesis}}$}
...
A node maintains a \emph{store} $(J_s,\; h_s,\; F_s,\; \mathcal{T})$ where:
...
The node runs the per-chain state transition to obtain the block's post-state, then updates the store's justified and finalized blocks.
...
After running the state transition, the block's post-state yields a candidate justified pair~$(J_B, h_B)$ and a candidate finalized block~$F_B$.
...
\State \textbf{Store:} $J_s \gets B_{\mathrm{genesis}}$, \; $h_s \gets 0$, \; $F_s \gets B_{\mathrm{genesis}}$, \; $\mathcal{T} \gets \{B_{\mathrm{genesis}}\}$
\vspace{3pt}
...
\Function{$\textsf{onBlock}$}{$B$}
  ...
  \State run state transition on~$B$; let $(J_B, h_B, F_B)$ be the post-state checkpoints
  \State $\textsf{updateJustified}(J_B, h_B)$
  \State $\textsf{updateFinalized}(F_B)$
\EndFunction
\vspace{3pt}
...
The primary votes register ($C \preceq L$ on chain~$A$), and justification fires at height~$h'$, advancing $A$ to $h' + 1$.
