...
Condition~(3) (for votes cast strictly before~$\vote(k)$) gives $u.\target \preceq \getConfirmed_{v_\ell}^{\vote(k)}$; for votes cast at $\vote(k)$, Property~\ref{prop:vote-inclusion} (under the hypotheses inherited from conditions~(1)--(2) and Step~1's prefix) places $u.\target$ on~$D$ at $\sigma$-height $\leq \comm{h}$.
...
Step~1 verifies the hypothesis of Property~\ref{prop:getConfirmed-compat}, yielding $\getConfirmed_v^{\vote(k+1)} \sim \getConfirmed_{v'}^{\vote(k+1)}$ for honest $v, v'$ --- condition~(2) at slot~$k+1$.
...
\begin{itemize}
  \item if $u$ was cast strictly before~$\vote(k)$, condition~(3) at slot~$k$ gives $u.\target \preceq \getConfirmed_{v_\ell}^{\vote(k)}$; Property~\ref{prop:target-dominance} gives $\getConfirmed_v^{\vote(k+1)} \succeq \getConfirmed_{v_\ell}^{\vote(k)}$ for honest~$v$, so $u.\target \preceq \getConfirmed_{v_\ell^{(k+1)}}^{\vote(k+1)}$;
  \item if $u$ was cast at $\vote(k)$, Property~\ref{prop:vote-inclusion} places $u.\target$ on the chain at $\getConfirmed_v^{\vote(k+1)}$ for honest~$v$, so $u.\target \preceq \getConfirmed_{v_\ell^{(k+1)}}^{\vote(k+1)}$.
\end{itemize}
