...
For \(h>1\), \(s_h\) is the first slot after the transition that enters
height~\(h\).  The \emph{current-height target} \(T_h\) is a state field.  It
is initially~\(\bot\).  After the first block processed at or after~\(s_h\),
the following slot transition records that block as~\(T_h\), and the field
...
checkpoint-sync state carries \(T_h\) even when its trusted block is in the
...
\(P=\varnothing\).  Write \(\sigma_{\rm gen}\) for this initial state.  A
height advance at the end of slot~\(s\) sets
\(s_{h+1}=s+1\) and \(T_{h+1}=\bot\).  Slot processing records the first
later block as~\(T_{h+1}\) once its root is available.
...
The height rules are evaluated exactly once at each scheduled round boundary,
after all timely votes for that close have been processed.  They are applied
in this order:
\begin{enumerate}
  ...
  \item if \(\nj=\mathsf{false}\) and
    ...
    \((s_h,T_h)\gets(s+1,\bot)\), set the new height's
  ...
  \item otherwise, if \(w(Q_{\rm timeout})\geq q\), advance \(h\) and
    ...
    \((s_h,T_h)\gets(s+1,\bot)\) and setting the new height's
...
\end{enumerate}
Finality is processed before a justification or timeout enabled at the same
round boundary.  Since a chain stores one current-height target, all valid target votes
...
transitions before a later block.  The round outcome is therefore evaluated
even when the final slot is empty.  A block never supplies a separate close
flag.  All timely attestations for the close have already been included.  The
...
\Function{process\_slot}{$\sigma$}
  ...
  \If{\(\roundEnd(\sigma.s)\)}
    \State \(\sigma\gets
      \Call{process\_justification\_and\_finalization}{\sigma}\)
  \EndIf
...
\Function{process\_justification\_and\_finalization}{$\sigma$}
  ...
  \If{\(T\neq\bot\)}
    \State \Return \(\Call{advance\_height}{\sigma,T}\)
  \EndIf
  \If{\(\mathsf{has\_timeout\_quorum}(\sigma)\)}
    \State \Return \(\Call{advance\_height}{\sigma,\bot}\)
  \EndIf
  \State \Return \(\sigma\)
\EndFunction
\vspace{3pt}

\Function{advance\_height}{$\sigma,\mathit{justify\_target}=\bot$}
  ...
  \State \(\sigma.\currentHeightStartSlot\gets\sigma.s+1;\quad
...
has occurred before a vote naming the block can be counted.  That block's state
is already at height~\(h\), and no height transition occurs between it and its
use as~\(T_h\).
\end{proof}
...
therefore has at most one first block after the start of height~\(h\), which
Definition~\ref{def:height} stores after its root becomes available and until
the next transition.  If another height-\(h\) branch has processed beyond that
block~\(T\), it has crossed the same height start and has already recorded
\(T\) as its first block.  The target-validity rule then accepts the vote.
\end{proof}
...
only the attestations included in that block.  When two stores are merged, each
...
height has just begun with an empty stored target, the common first-slot
proposal is its first block; slot processing records that block before the
delayed confirmation and SG actions.
\end{proof}
...
\(k=\sigma_r.h\) and
\(T=\sigma_r.\currentHeightTarget=T_k\).  When \(T\neq\bot\), it is the first block of
height~\(k\) on that chain and supplies the candidate target vote~\((k,T)\),
which is not necessarily about the pointed root itself.  Validators first choose their
walk, then derive its action-state height and target.
\end{definition}
...
\(k_i^r=\sigma_i^r.h\), and let \(T_i^r\) be its current-height target.  The round
...
responsive honest votes of weight at least~\(q\) are included before the round
closes, then~\(U\) is justified and the chain advances to~\(h^\ast+1\).
\end{lemma}
...
\((h^\ast,U)\).  If those messages are included and processed at round close,
\(U\) finalizes there.  The round's first-slot proposer need not
be honest.
\end{lemma}
...
justification before the finalize pairs are collected, so
...
\texttt{current\_height\_target} together with the
current height and finalized state.  Figure~\ref{alg:state-machine} enters a
new height at a slot boundary with an empty target, then records the first
block in that height once its root is available.  The executable profile
currently marks the outcome due at the round boundary and consumes it after
the next slot's block operations, or after that slot's empty operation phase.
That one-slot grace lets final-slot attestations satisfy their inclusion delay;
it does not make closing a block-supplied choice.  Both models derive the
outcome from the slot schedule and require no \(\mathsf{closes\_round}(B)\)
flag.  A valid vote that refers to an unknown block must be queued and replayed
