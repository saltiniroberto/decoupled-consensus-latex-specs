...
\begin{lemma}[Cross-time, cross-validator target compatibility]\label{lem:getConfirmed-compat}
Assume $f < n/3$, all honest follow Definition~\ref{def:voting-rule}, and slot~$k$ satisfies the standing assumptions (Definition~\ref{def:standing}).
Suppose $R_v^{\vote(k)} \sim R_{v'}^{\vote(k)}$ for every pair of honest~$v, v'$, and there exists an honest~$v_\ell$ with $\getConfirmed_{v_\ell}^{\vote(k)} \succeq R_v^t$ for every honest~$v$ and every $t \in [\vote(k),\, \vote(k{+}1)]$.
Then $\getConfirmed_v^t \sim \getConfirmed_{v'}^{t'}$ for every honest pair $v, v'$ and times $t, t' \in [\vote(k),\, \vote(k{+}1)]$.
\end{lemma}

\begin{proof}
Let $\comm{h} \coloneqq \sigma[\getConfirmed_{v_\ell}^{\vote(k)}].h$ and $D$ the chain ending at $\getConfirmed_{v_\ell}^{\vote(k)}$.
By the premise, every honest $R_v^t$ lies on~$D$ at $\sigma$-height $\leq \comm{h}$.

\emph{Every $B \in \Sigma_v^t.\T$ at $\sigma$-height $\geq \comm{h}$ lies on~$D$.}
Any such~$B$ is reached on its chain via a height advance at some $h^* \leq \sigma[B].h$, fired by a justification or timeout cert.
A justification at $h^* \leq \comm{h}$ off~$D$ would, by Lemma~\ref{lem:just-unique-height}, coincide with the unique on-$D$ justification at the same height.
A timeout cert at any $h^* \leq \comm{h}$ off~$D$ would form a $\geq 2n/3$ quorum disjoint from on-$D$ contributors at the same height, exceeding the $n - f$ honest budget under $f < n/3$.
Hence every $B \in \Sigma_v^t.\T$ at $\sigma$-height $\geq \comm{h}$ lies on~$D$, and $\Sigma_v^t.\hmax \leq \comm{h} + 1$.

\emph{$\getConfirmed_v^t \in D$.}
$\getConfirmed_v^t$ lies in $\viableTree(\Sigma_v^t)$ above $R_v^t \in D$ at $\sigma$-height $\geq \Sigma_v^t.\hmax - 1$, hence on~$D$ by the chain restriction.
Two blocks both on~$D$ are pairwise $\sim$-comparable, so $\getConfirmed_v^t \sim \getConfirmed_{v'}^{t'}$.
\end{proof}

\begin{lemma}[Intra-slot target monotonicity]\label{lem:getConfirmed-monotone}
Under the hypotheses of Lemma~\ref{lem:getConfirmed-compat}, for every honest~$v$ and times $t \leq t' \in [\vote(k),\, \vote(k{+}1)]$, $\getConfirmed_v^{t'} \succeq \getConfirmed_v^t$.
\end{lemma}

\begin{proof}
Lemma~\ref{lem:getConfirmed-compat} places $\getConfirmed_v^t,\, \getConfirmed_v^{t'} \in D$ at $\sigma$-heights in $\{\comm{h} - 1,\, \comm{h},\, \comm{h} + 1\}$.
No honest voting occurs in $(\vote(k),\, \vote(k{+}1))$, so $\Sigma_v$ evolves only via \onBlock{} and view-merge processing of newly received blocks: $\Sigma_v.\T$ grows, $\Sigma_v.\hmax$ is non-decreasing, the lex key of $\Sigma_v.J$ is non-decreasing (Lemma~\ref{lem:Rs-key-monotone}), and $\Sigma_v.F$ is non-decreasing (Theorem~\ref{thm:finperm}).
Hence $R_v^{t'} \succeq R_v^t$ on~$D$ by state-height monotonicity (Lemma~\ref{lem:slotmono}); the deterministic disambiguation~$\Omega_v$ extends $\getConfirmed_v$ along~$D$ as $\Sigma_v.\T$ grows, giving $\getConfirmed_v^{t'} \succeq \getConfirmed_v^t$.
\end{proof}

\begin{lemma}[Honest-vote inclusion in slot-$(k{+}1)$ canonical]\label{lem:vote-inclusion}
Under the hypotheses of Lemma~\ref{lem:getConfirmed-compat}, for every honest~$v$, the chain ending at $\getConfirmed_v^{\vote(k+1)}$ contains every vote cast by an honest validator at~$\vote(k)$.
\end{lemma}

\begin{proof}
Lemma~\ref{lem:getConfirmed-compat} gives $\getConfirmed_v^{\vote(k+1)} \in D$.
Each honest~$i$'s slot-$k$ vote~$a^k_i$ has, by Lemma~\ref{lem:honest-vote-sets-timeout}, either $a^k_i.\target = \bot$ or $a^k_i.\target \preceq \getConfirmed_i^{\vote(k)}$ at $\sigma[a^k_i.\target].h = \sigma[\getConfirmed_i^{\vote(k)}].h$; Lemma~\ref{lem:getConfirmed-compat} (at $t = \vote(k)$) gives $\getConfirmed_i^{\vote(k)} \in D$, so $a^k_i.\target \in D \cup \{\bot\}$.

By Definition~\ref{def:standing}(d), every slot-$k$ honest vote reaches an honest available-chain proposer that builds a slot-$k$ block~$B^\sharp$ before $\vote(k{+}1) - \delta_c$.
The proposer's canonical at proposal time lies on~$D$ (Lemma~\ref{lem:getConfirmed-compat} applied to the proposer), and $B^\sharp$ extends it on~$D$.
Synchrony (Definition~\ref{def:standing}(a)) delivers $B^\sharp$ to every honest~$v$ by $\vote(k{+}1) - \Delta$, and \onBlock{} folds it into $\Sigma_v.\T$ on~$D$, so $\Sigma_v^{\vote(k+1)}.\hmax \geq \sigma[B^\sharp].h$.
Lemma~\ref{lem:slotmono} along~$D$ together with $\getConfirmed_v^{\vote(k+1)} \in D$ at $\sigma$-height $\geq \Sigma_v^{\vote(k+1)}.\hmax - 1$ forces $B^\sharp \preceq \getConfirmed_v^{\vote(k+1)}$, placing every slot-$k$ honest vote on~$B^\sharp$ on the chain at $\getConfirmed_v^{\vote(k+1)}$.
\end{proof}

\begin{lemma}[Slot-$(k{+}1)$ canonical extends $v_\ell$'s slot-$k$ canonical]\label{lem:target-dominance}
Under the hypotheses of Lemma~\ref{lem:getConfirmed-compat}, for every honest~$v$, $\getConfirmed_v^{\vote(k+1)} \succeq \getConfirmed_{v_\ell}^{\vote(k)}$.
\end{lemma}

\begin{proof}[Proof sketch]
Choose $v_\ell$ to be the $\preceq$-max honest at $\vote(k)$ on~$D$, well-defined by Lemma~\ref{lem:getConfirmed-compat}.
Let $B^\sharp$ be the slot-$k$ honest available-chain block from Lemma~\ref{lem:vote-inclusion}; $B^\sharp \preceq \getConfirmed_v^{\vote(k+1)}$ on~$D$ by that lemma.

By Definition~\ref{def:standing}(b,c,d), $\geq n - f > 2n/3$ honest validators cast a slot-$k$ vote and contribute to~$B^\sharp$; each sets $\timeouts[i] = \mathtt{true}$ on~$B^\sharp$'s chain at state-height $\sigma[\getConfirmed_i^{\vote(k)}].h$ (Definition~\ref{def:timeout}), with non-$\bot$ targets pinned onto~$D$ by Lemma~\ref{lem:honest-vote-sets-timeout} together with maximality of~$v_\ell$ and Lemma~\ref{lem:just-unique-height}.
Iterating \processHeight{} on~$B^\sharp$'s chain advances state-height to~$\comm{h} + 1$ on~$D$: at each height $\leq \comm{h}$, either the unique on-$D$ justification target attains a $\geq 2n/3$ quorum or the $\geq 2n/3$ $\timeouts$-quorum fires.
Hence $\Sigma_v^{\vote(k+1)}.\hmax \geq \comm{h} + 1$.

Lemma~\ref{lem:vote-inclusion} places $v_\ell$'s slot-$k$ vote on the chain at $\getConfirmed_v^{\vote(k+1)}$.
In the first-vote and lock branches of \getFinalVote{}, the target equals $\getConfirmed_{v_\ell}^{\vote(k)}$ (by Lemma~\ref{lem:honest-vote-sets-timeout}'s cascade-gate analysis combined with maximality on~$D$); height-fresh inclusion of the vote on~$B^\sharp$'s chain places $\getConfirmed_{v_\ell}^{\vote(k)} \preceq \getConfirmed_v^{\vote(k+1)}$.
\end{proof}
...
\begin{enumerate}
  \item for every honest~$v$, $\sigma[R_v^{\vote(k)}].h \geq \comm{h}$;
  \item for every honest pair $v, v'$, $\getConfirmed_v^{\vote(k)} \sim \getConfirmed_{v'}^{\vote(k)}$;
  \item there exists an honest~$v_\ell$ such that every honest vote~$u$ cast at any time strictly before~$\vote(k)$ with $u.\height \geq \comm{h}$ has $u.\target \preceq \getConfirmed_{v_\ell}^{\vote(k)}$.
\end{enumerate}
\end{definition}
...
If the system is in a healed state at slot~$k$ with frontier height~$\comm{h}$, slot~$k+1$ satisfies the standing assumptions, and $\syncer(k+1)$ is honest, then the system is in a healed state at slot~$k+1$ with frontier height $\comm{h}' \geq \comm{h}$.
\end{lemma}

\begin{proof}
Write $\comm{\Sigma} \coloneqq \comm{\Sigma}^{\vote(k+1)}$ for the post-merge store guaranteed by Assumption~\ref{ass:view-merge} under the honest slot-$(k+1)$ syncer; $\Sigma_v^{\vote(k+1)} = \comm{\Sigma}$ for every honest~$v$.
Let $D$ be the chain ending at $\getConfirmed_{v_\ell}^{\vote(k)}$, where $v_\ell$ is the witness from condition~(3) at slot~$k$.

\emph{Step 1: cascade root stable over $[\vote(k),\, \vote(k{+}1)]$.}
Fix honest~$v$ and $t \in [\vote(k),\, \vote(k{+}1)]$.
At $t = \vote(k)$, condition~(1) gives $\sigma[R_v^{\vote(k)}].h \geq \comm{h}$, while $R_v^{\vote(k)} \preceq \getConfirmed_v^{\vote(k)}$ by the algorithm spec (Corollary~\ref{cor:getConfirmed-total}); condition~(2) and chain comparability give $R_v^{\vote(k)} \preceq \getConfirmed_{v_\ell}^{\vote(k)}$.
For $t > \vote(k)$, $\Sigma_v.h_j$ is non-decreasing (Lemma~\ref{lem:Rs-key-monotone}) and $\Sigma_v.F$ is non-decreasing (Theorem~\ref{thm:finperm}), so $\sigma[R_v^t].h \geq \comm{h}$ persists.

For chain dominance, any update to $\Sigma_v.J$ during $(\vote(k),\, \vote(k{+}1)]$ comes from a justification at some height~$h^* \geq \comm{h}$ offered to \updateJustified{}.
The witnessing quorum has size $\geq 2n/3$ and contains an honest~$u$; $u$'s contributing vote was cast at height~$h^* \geq \comm{h}$ strictly before~$t$.
Condition~(3) (for votes cast strictly before~$\vote(k)$) gives $u.\target \preceq \getConfirmed_{v_\ell}^{\vote(k)}$; for votes cast at $\vote(k)$, Lemma~\ref{lem:vote-inclusion} (under the hypotheses inherited from conditions~(1)--(2) and Step~1's prefix) places $u.\target$ on~$D$ at $\sigma$-height $\leq \comm{h}$.
Per-height uniqueness (Lemma~\ref{lem:just-unique-height}) forces the new justification's target onto~$D$ at $\sigma$-height $\leq \sigma[\getConfirmed_{v_\ell}^{\vote(k)}].h$; the new $\Sigma_v^t.J$ stays $\preceq \getConfirmed_{v_\ell}^{\vote(k)}$, and $\Sigma_v^t.F$ stays $\preceq \Sigma_v^t.J$ by Theorem~\ref{thm:fleqr}.
Hence $R_v^t \preceq \getConfirmed_{v_\ell}^{\vote(k)}$ throughout the interval.

\emph{Step 2: pairwise compatibility at $\vote(k+1)$.}
Step~1 verifies the hypothesis of Lemma~\ref{lem:getConfirmed-compat}, yielding $\getConfirmed_v^{\vote(k+1)} \sim \getConfirmed_{v'}^{\vote(k+1)}$ for honest $v, v'$ --- condition~(2) at slot~$k+1$.

\emph{Step 3: frontier non-decrease.}
By Definition~\ref{def:standing}(d) at slot~$k+1$, the slot-$k$ honest votes are aggregated by an honest available-chain proposer into a slot-$k$ block whose post-state is processed before $\vote(k+1) - \Delta$.
Folding through \onBlock{} and \updateJFromVotes{} fires the slot-$k$ honest virtual justification at canonical's $h_c^{(k)} \coloneqq \sigma[\getConfirmed_{v_\ell}^{\vote(k)}].h \geq \comm{h}$, raising $\Sigma.h_j$ uniformly.
Hence $\comm{\Sigma}.h_j \geq h_c^{(k)} \geq \comm{h}$; set $\comm{h}' \coloneqq h_c^{(k)}$ when the gate fires at $\vote(k+1)$, and $\comm{h}' \coloneqq \comm{h}$ otherwise.

\emph{Step 4: condition~(1) at slot~$k+1$.}
If the gate fires at $\vote(k+1)$ ($\comm{\Sigma}.\hmax = \comm{\Sigma}.h_j + 1$), $R_v^{\vote(k+1)} = \comm{\Sigma}.J$ has $\sigma$-height $\comm{\Sigma}.h_j \geq \comm{h}'$.
If the gate fails, $R_v^{\vote(k+1)} = \comm{\Sigma}.F$ has $\sigma$-height $\geq \comm{h} = \comm{h}'$ by Theorem~\ref{thm:finperm} and condition~(1) at slot~$k$.

\emph{Step 5: target-cap at slot~$k+1$.}
Choose $v_\ell^{(k+1)}$ honest with $\preceq$-maximal $\getConfirmed^{\vote(k+1)}$ on the common chain (well-defined by Step~2).
For an honest vote~$u$ cast strictly before~$\vote(k+1)$ with $u.\height \geq \comm{h}'$:
\begin{itemize}
  \item if $u$ was cast strictly before~$\vote(k)$, condition~(3) at slot~$k$ gives $u.\target \preceq \getConfirmed_{v_\ell}^{\vote(k)}$; Lemma~\ref{lem:target-dominance} gives $\getConfirmed_v^{\vote(k+1)} \succeq \getConfirmed_{v_\ell}^{\vote(k)}$ for honest~$v$, so $u.\target \preceq \getConfirmed_{v_\ell^{(k+1)}}^{\vote(k+1)}$;
  \item if $u$ was cast at $\vote(k)$, Lemma~\ref{lem:vote-inclusion} places $u.\target$ on the chain at $\getConfirmed_v^{\vote(k+1)}$ for honest~$v$, so $u.\target \preceq \getConfirmed_{v_\ell^{(k+1)}}^{\vote(k+1)}$.
\end{itemize}
This is condition~(3) at slot~$k+1$ with $v_\ell^{(k+1)}$ and frontier height $\comm{h}'$.
\end{proof}
...
View-merge under the honest slot-$k_h$ syncer makes $\Sigma_v^{\vote(k_h)} = \comm{\Sigma}^{(k_h)}$ uniform across honest, hence $\getConfirmed_v^{\vote(k_h)} \sim \getConfirmed_{v'}^{\vote(k_h)}$ (condition~(2) of Definition~\ref{def:healed}).
Set $\comm{h} \coloneqq \comm{\Sigma}^{(k_h)}.h_j \geq h^0 - 1$ and $R_v^{\vote(k_h)} = \comm{\Sigma}^{(k_h)}.J$ at $\sigma$-height $\comm{h}$ (condition~(1)).

\emph{Condition~(3) at $\vote(k_h)$.}
Pick $v_\ell$ honest with $\preceq$-maximal $\getConfirmed^{\vote(k_h)}$ on the common chain (well-defined by condition~(2)).
...
\end{itemize}
This is condition~(3) at slot~$k_h$ with frontier height $\comm{h} \geq h^0 - 1$.
\end{proof}
