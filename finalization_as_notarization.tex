...
\Function{$\textsf{processHeight}$}{$(h,\, \jtargets,\, \ntargets,\, T^\mathsf{min},\, J,\, h_j,\, F,\, P,\, T^\mathsf{max}),\; B$}
  ...
  \State $T^\mathsf{max\_temp}\gets \bot$
  \For{$B'$ from $B$ down to $T^\mathsf{max}$}
    \State $\cnt \gets \cnt + |\{i : \jtargets[h][i] = B'\}|$
    \If{$T^\mathsf{max\_temp}=\bot$ and $\cnt > 0$}
...
\begin{lemma}\label{lem:upgrade-helper}
  ...
  Let $(J_j, h_j) = \arg\max_{(J',h')\in C }(h', J')$.
...
\begin{proof}
  Assume it sent a finalize vote for $(F,h_F)$ with $h_F > j_j$.
  It would be that $(F,h_F)\in C$.
  If $F \succeq F_s$, then it is obvious that we reach a contradiction as we would have $h_j \geq h_F$.

  $F$ must have been $\succeq F'_s$ where $F'_s$ was $F_s$ at the time the vote was cast.
  From a previous lemma we know $F'_s \sim F_s$.
  Then $(F, h_F) \in C$.
