...
Over any non-finality period of length $T \geq 1$ on the canonical chain, the total penalty increment across all validators assessed by $\textsf{processInactivityPenalties}$ is strictly greater than $(T - 1) \cdot n/3$.
\end{theorem}
...
After such a slot, $\sigma_{\mathrm{post}}^{(t)}.J \succ \sigma_{\mathrm{pre}}^{(t)}.F$; since $F$ does not advance anywhere in the period, $\sigma_{\mathrm{pre}}^{(t')}.F = \sigma_{\mathrm{pre}}^{(t)}.F \prec \sigma_{\mathrm{pre}}^{(t')}.J$ at every later slot $t' > t$ of the period (justification is monotone across slots by Lemma~\ref{lem:slotmono}).
...
Summing over the period, the total penalty increment across all validators is strictly greater than $(T - 1) \cdot n/3$.

The bound is purely state-logic: the proof reads $\sigma_{\mathrm{pre}}^{(t)}$ and $\sigma_{\mathrm{post}}^{(t)}$ on the canonical chain and counts charges directly from Figure~\ref{alg:leak-processslot}, with no hypothesis on synchrony, on the honest voting rule, or on the size of the slashable set.
\end{proof}
