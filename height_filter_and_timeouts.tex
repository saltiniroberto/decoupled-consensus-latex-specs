...
\begin{enumerate}
  ...
  \item\label{rev:def:healed-state:vote-h} every honest vote~$u$ cast at any time $\leq \vote(k)$ has $u.\height \leq \sigma[\getConfirmed_{v_\ell}^{\vote(k)}].h$.
\end{enumerate}
\end{definition}
...
\emph{Setup.}
Let $v_\ell$ be the honest validator named by Definition~\ref{def:healed}, and write
\[
  h^\ast \;\coloneqq\; \sigma[\getConfirmed_{v_\ell}^{\vote(k)}].h.
\]
By condition~\ref{rev:def:healed-state:vote-h}, every honest vote $u$ cast at any time $\leq \vote(k)$ has $u.\height \leq h^\ast$.
