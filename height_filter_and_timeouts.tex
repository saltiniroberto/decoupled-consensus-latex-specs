...
\end{itemize}
The genesis store has $\T = \{\genesis\}$, $V = \emptyset$, $F = J = \genesis$, $h_j = 0$, $\hmax = 0$.
\end{definition}
...
\begin{proof}
At genesis, $\Sigma.F = \genesis$ and $\Sigma.\hmax = 0$, so $\genesis$ is a viable leaf.
...
for the cascade root selected inside $\getConfirmed_v^t$.
...
\begin{enumerate}
  \item slot~$k$ satisfies the standing assumptions (Definition~\ref{def:standing});
...
\subsection{Persistence and advance}

\begin{lemma}[Healing persistence over one slot]\label{lem:healing-persistence}
If $f < n/3$, all honest follow Definition~\ref{def:voting-rule}, slot~$k$ satisfies the standing assumptions, and the system is in a healed state at slot~$k$ with frontier height~$\comm{h}$, then it is in a healed state at slot~$k+1$ with frontier height~$\comm{h}$ (or a higher one, if a fresh justification fires).
\end{lemma}

\begin{proof}[Proof sketch]
View-merge under the slot-$k$ honest syncer (Assumption~\ref{ass:view-merge}) makes $\Sigma_v^{\vote(k)}$ uniform across honest~$v$, and condition~(3) is the post-merge \getConfirmed equality.

\emph{Slot-$k$ honest vote targets uniform.}
Each honest~$v$ casts via $\getFinalVote(\Delta_v,\, \getBaseVote(\Sigma_v^{\vote(k)}, \Omega_v^{\vote(k)}))$.
$\getBaseVote$ produces $(h_c, T, h_f, T_f)$ uniformly across honest at $\vote(k)$ (since $\Sigma$ is common and $h_c, h_f, T_f$ depend only on $\Sigma$; $T = \getConfirmed$ is also $\sim$-compatible across honest by~(3), with the deterministic-disambiguation $\Omega$ resolving up to the same canonical block when given uniform $\Sigma$).
$\getFinalVote$ then either reproduces $T$ (first-vote), substitutes a lock target $\Delta_v.T(h_c)$ (lock), or emits $\bot$ (timeout).
By the healed-state target-cap~(4), every prior $i$-vote that pinned $\Delta_v.T(h_c)$ targeted some prefix of $\getConfirmed_{v_\ell}^{\vote(k)}$, so each lock-branch vote also targets a chain $\preceq \getConfirmed_{v_\ell}^{\vote(k)}$ on the common chain.
A timeout vote has $\target = \bot$ and contributes universally.
Hence at least $n - f > 2n/3$ honest cast slot-$k$ votes whose contributions to a virtual height-fresh quorum on canonical agree on the canonical target.

\emph{Slot-$(k+1)$ uniform $\Sigma$ post-virtual.}
By honest-relay (standing assumption~(d)), the slot-$k$ honest votes reach an honest available-chain proposer that builds a slot-$k$ block before $\vote(k+1) - \delta_c$, and synchrony delivers that block to every honest by $\vote(k+1) - \Delta$.
Folding the block through \onBlock and \updateJFromVotes fires the virtual justification $J^*$ at $h_c = \sigma[\getConfirmed^{\vote(k)}].h$ on canonical, raising $\Sigma.h_j$ uniformly to $h_c$.
By view-merge under the slot-$(k+1)$ honest syncer, $\Sigma_v^{\vote(k+1)}$ is again uniform.

\emph{Re-verifying the four conditions at slot~$k+1$.}
(1)~slot~$k+1$ standing assumptions are inherited.
(2)~$\sigma[R_v^{\vote(k+1)}].h \geq h_c \geq \comm{h}$: either $\Sigma.h_j = h_c$ keeps the gate at $\Sigma.\hmax = \Sigma.h_j + 1 = h_c + 1$, or a higher cert event has fired and dominates.
(3)~uniformity from view-merge at $\vote(k+1)$.
(4)~target-cap inherits from~(4) at slot~$k$ together with the slot-$k$ vote targets bounded above.
\end{proof}

\begin{lemma}[Three-slot advance]\label{lem:healing-3slot}
If $f < n/3$, all honest follow Definition~\ref{def:voting-rule}, slots~$k+1, k+2, k+3$ all satisfy the standing assumptions and have honest syncers, and the system is in any state at slot~$k$ (no healed-state assumption needed) with $\Sigma.\hmax^{(k)} = h^0$, then the system is in a healed state at slot~$k+3$ with some frontier height~$\comm{h} \geq h^0$.
\end{lemma}

\begin{proof}[Proof sketch]
\emph{Adversary card inventory.}
The adversary holds at most two disruptive cards usable post-async:
\begin{itemize}
\item[(i)] a unique justification $J^*$ at height~$h^0 - 1$ on a sibling chain~$Y^*$;
\item[(ii)] a unique justification $J^\dagger$ at height~$h^0$ on a chain~$Y^\dagger$ whose containing block has $\sigma$-height~$h^0 + 1$.
\end{itemize}
Justifications at heights below $h^0 - 1$, when released, update $\Sigma.h_j$ to that older height, but $\Sigma.\hmax \geq h^0$ throughout (state-height monotonicity + adversary releases never decrease $\Sigma.\hmax$), so the cascade gate fails and the released chain is silently demoted to $R = \Sigma.F$ without reorg.
Justifications at heights above $h^0$ would require honest contributions at $\sigma$-heights $> h^0$, which cannot fire before slot~$k+1$.
A timeout-only chain reaching $\sigma$-height $h^0 + 1$ via a timeout cert at $h^0$ is incompatible with~(ii) under $f < n/3$: both events require disjoint $2n/3$-honest quorums at height~$h^0$, exceeding the $n - f$ honest budget.
So no third card exists.

\emph{Per-slot mechanics.}
At each slot~$\ell \in \{k+1, k+2, k+3\}$: view-merge under the honest syncer makes $\Sigma_v^{\vote(\ell)}$ uniform across honest, and the standing assumptions (synchrony, AHA, HAR) ensure honest slot-$\ell$ votes are aggregated into the slot-$\ell$ block via \updateJFromVotes by $\vote(\ell+1) - \Delta$.
At least $2n/3$ honest votes converge on the common canonical's target (via $\getFinalVote$'s first-vote branch on common $\Sigma$, modulo lock-validators whose targets are bounded by the prior healed-state target-cap or by the prior-justification quorums witnessing them), forming a virtual justification at canonical's $h_c$.

\emph{Card consumption.}
Each release of (i) or (ii) updates $\Sigma.J$ via the lex test, satisfies the cascade gate at the moment of release ($\Sigma.\hmax = \Sigma.h_j + 1$ holds with $\Sigma.h_j$ now equal to the new card's height), and reorgs canonical onto the released chain.
The slot-$\ell$ honest virtual justification at canonical's $h_c$ then dominates the released card on the lex test in turn (since $h_c$ matches or exceeds the released height), absorbing the card.
The adversary's optimal strategy staggers releases one per slot pre-vote, so by slot~$k+2$ both cards are spent.

\emph{Slot~$k+3$ produces a healed state.}
At slot~$k+3$ no card remains. View-merge gives uniform $\Sigma$, and slot-$(k+3)$ honest votes form a uniform virtual justification at canonical's $h_c$, which is the new frontier $\comm{h}$.
The four healed-state conditions are verified at $\vote(k+3)$:~(1) standing assumptions are hypothesized;~(2) $\sigma[R_v^{\vote(k+3)}].h \geq \comm{h}$ from the slot-$(k+3)$ virtual cert;~(3) view-merge uniformity gives pairwise compatibility;~(4) the union of pre-async target-bounds and slots~$k+1, k+2, k+3$ honest votes are all bounded by the maximal honest $\getConfirmed^{\vote(k+3)}$.
Finally, $\comm{h} \geq h^0$: slots~$k+1, k+2, k+3$ each form a virtual justification at canonical's $h_c \geq h^0 - 1$, and the lex monotonicity together with the staggered card consumption forces $\Sigma.h_j \geq h^0$ by slot~$k+3$.
\end{proof}
...
Let~$k$ be a slot such that slots~$k+1, k+2, k+3$ all satisfy the standing assumptions and have honest syncers.
Then the system is in a healed state at some slot $k_h \leq k + 3$, and remains healed at every subsequent slot satisfying the standing assumptions.
\end{theorem}

\begin{proof}
Lemma~\ref{lem:healing-3slot} gives the healed state at $k_h \leq k + 3$, and Lemma~\ref{lem:healing-persistence} propagates the healed state forward.
\end{proof}
