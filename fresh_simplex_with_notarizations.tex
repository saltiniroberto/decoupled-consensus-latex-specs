...
The store maintains a single \emph{root} $R_s$, updated incrementally whenever $\textsf{processHeight}$ fires a height advance (a \emph{cert event}) whose \emph{cert key} strictly exceeds the key currently recorded for~$R_s$.
...
\begin{itemize}
  ...
  \item $R_s = (R_s.h,\; R_s.\mathsf{bit},\; R_s.\mathsf{block})$ is the \emph{store root}, a triple recording the height, the justification bit, and the block of the highest-key cert event ever observed by the node.
\end{itemize}
The capital~$\Sigma$ distinguishes the store from the per-chain state~$\sigma$: $\sigma[B]$ is a single post-state, while $\Sigma$ aggregates the state map together with the fork-choice bookkeeping $(\T, F_s, R_s)$.
Where unambiguous we write $\Sigma.\sigma$, $\Sigma.\T$, $\Sigma.F_s$, $\Sigma.R_s$ for the corresponding fields, $\Sigma.R_s.h$, $\Sigma.R_s.\mathsf{bit}$, $\Sigma.R_s.\mathsf{block}$ for the three coordinates of the root triple, and $\mathsf{key}(R_s) \coloneqq \mathsf{key}(R_s.\mathsf{block},\, R_s.h,\, R_s.\mathsf{bit})$ for its full cert key.
The genesis store has $F_s = \genesis$, $\T = \{\genesis\}$, and $R_s = (0,\, 1,\, \genesis)$ (genesis acts as a bit-$1$ cert event at height~$0$; its full cert key is $(0,\, 1,\, 0,\, \mathsf{hash}(\genesis))$).
As shorthand for ancestor/descendant relations involving the root, we write $R_s \succeq B$ for $R_s.\mathsf{block} \succeq B$, and similarly for $\preceq$ and $\succ$.
\end{definition}
...
$\textsf{updateForkChoiceRoot}$ first filters out any candidate~$X'$ with $X' \not\succeq \Sigma.F_s$, and then updates $\Sigma.R_s$ iff the candidate's cert key strictly exceeds the key currently recorded for $R_s$.
...
\State \textbf{Genesis store} $\Sigma$: \;$\Sigma.\T \gets \{\genesis\}$, \; $\Sigma.F_s \gets \genesis$, \; $\Sigma.R_s \gets (0,\, 1,\, \genesis)$,
...
\Function{$\textsf{updateForkChoiceRoot}$}{$\Sigma,\; (h',\, b',\, X')$} \Comment{Filter by $\Sigma.F_s \preceq X'$, then running max}
  \If{$X' \not\succeq \Sigma.F_s$} \Return $\Sigma$ \EndIf
  \State $\mathsf{newKey} \gets (h',\; b',\; X'.\slot,\; \mathsf{hash}(X'))$
  \State $\mathsf{curKey} \gets (\Sigma.R_s.h,\; \Sigma.R_s.\mathsf{bit},\; \Sigma.R_s.\mathsf{block}.\slot,\; \mathsf{hash}(\Sigma.R_s.\mathsf{block}))$
  \If{$\mathsf{newKey} > \mathsf{curKey}$ (lex)}
    \State $\Sigma.R_s \gets (h',\; b',\; X')$
  \EndIf
  \State \Return $\Sigma$
\EndFunction
\vspace{3pt}
...
Each call is gated by the filter $X' \succeq \Sigma.F_s$, so a candidate on a chain no longer consistent with $F_s$ is dropped; the remaining candidate replaces $\Sigma.R_s$ iff its key strictly exceeds the current root's.
...
The cert key $\mathsf{key}(R_s)$ is non-decreasing over time; in particular, $R_s.h$ is non-decreasing.
\end{lemma}

\begin{proof}
$R_s$ changes only inside $\textsf{updateForkChoiceRoot}$, whose guard $\mathsf{newKey} > \mathsf{curKey}$ (lex) allows only strict increases; the height coordinate $h$ is the leading component of the key.
\end{proof}
...
At genesis, $F_s = \genesis$ and $R_s.\mathsf{block} = \genesis$, so $F_s \preceq R_s$.

Assume $F_s \preceq R_s$ holds before a store operation, and consider the two mutators in Figure~\ref{alg:store}.

\emph{$\textsf{updateForkChoiceRoot}(\Sigma,\, (h',\, b',\, X'))$.}
If the filter $X' \succeq \Sigma.F_s$ fails, $\Sigma$ is returned unchanged and the invariant is preserved; otherwise the lex-strict guard either rejects the candidate (leaving $R_s$ unchanged) or replaces $R_s.\mathsf{block}$ by $X' \succeq \Sigma.F_s$.
Either way $F_s$ is untouched and the new $R_s.\mathsf{block}$ descends from $F_s$.
...
The filter $F \succeq F_s^0$ passes and $\textsf{updateForkChoiceRoot}$ proceeds: either it raises $R_s$ to $(h_F,\, 1,\, F)$, or the current $R_s$ already has strictly greater key.
By Lemma~\ref{lem:Rs-key-monotone}, $\mathsf{key}(R_s) \geq (h_F,\, 1,\, F.\slot,\, \mathsf{hash}(F))$ from that moment on; in particular $R_s.h \geq h_F$.
If $R_s.h > h_F$, Lemma~\ref{lem:certchain} applied to the cert event $(R_s.\mathsf{block},\, R_s.h,\, R_s.\mathsf{bit})$ gives $F \prec R_s.\mathsf{block}$.
If $R_s.h = h_F$, the key lower bound forces $R_s.\mathsf{bit} = 1$ and $R_s.\mathsf{block}.\slot \geq F.\slot$; so $R_s.\mathsf{block}$ was justified at height $h_F$, and Lemma~\ref{lem:just-unique-height} gives $R_s.\mathsf{block} = F$.
\end{proof}
...
Every descriptor $(h',\, b',\, X') \in D$ with $h' \geq h_{F_{\max}}$ satisfies $X' \succeq F_{\max}$ by Lemma~\ref{lem:certchain} (compatibility, then strict descent when $h' > h_{F_{\max}}$; for $h' = h_{F_{\max}}$, the key-order lower bound $\mathsf{key}(R_s) \geq (h_{F_{\max}},\, 1,\, F_{\max}.\slot,\, \mathsf{hash}(F_{\max}))$ from Remark~\ref{rem:fs-invariant} and Lemma~\ref{lem:just-unique-height} force $X' = F_{\max}$).
...
Hence $R_s$'s final value is determined by the lex-max over $\{(h',\, b',\, X') \in D : h' \geq h_{F_{\max}}\}$, which is invariant under the processing order.
\end{proof}

\begin{corollary}[Cross-node agreement]\label{cor:agreement}
Unless $\geq n/3$ validators are slashable: two nodes that have processed the same set of blocks have the same $(\sigma,\, \T,\, F_s,\, R_s)$.
\end{corollary}
...
The argument reads $\Sigma.R_s$ directly as the running-max cert event, bounds $h_c$ in terms of $R_s.h$, and handles the locked branch of $T_c$ by matching its target against $R_s.\mathsf{block}$.

Let $R_s = (h,\, b,\, N)$ with $N \coloneqq R_s.\mathsf{block}$.

\emph{Step 1: $N \preceq X$ and $h_c \geq h$.}
$X$ descends from~$N$ (Figure~\ref{alg:store}), so $N \preceq X$.
$R_s$ is either the genesis triple (with $N = \genesis$, $h = 0$) or was set by a cert event firing at height $h$ on some processed chain; in the latter case Remark~\ref{rem:fresh-equiv} gives $\sigma[N].h = h$, and in the former trivially $\sigma[N].h = 0 = h$.
State-height monotonicity along~$\preceq$ (Lemma~\ref{lem:slotmono}) gives $h_c = \sigma[X].h \geq h$.

\emph{Step 2: $h_c \leq h + 1$.}
...
If some such event had $h' \geq h + 1$, its key's leading coordinate would beat $R_s$'s $h$, so $\textsf{updateForkChoiceRoot}$ would have raised $R_s.h$ to at least $h + 1$, contradicting the current $R_s.h = h$.
Hence no advance from $h + 1$ to~$h + 2$ has fired on~$X$, and $h_c \in \{h,\, h + 1\}$.
...
That event was passed through $\textsf{updateForkChoiceRoot}$ and raised $\mathsf{key}(R_s)$ to at least $(h',\, 1,\, J_\ast.\slot,\, \mathsf{hash}(J_\ast))$; by Lemma~\ref{lem:Rs-key-monotone} this bound persists.
So a lock at height $h'$ implies the current $\mathsf{key}(R_s) \geq (h',\, 1,\, J_\ast.\slot,\, \mathsf{hash}(J_\ast))$, and in particular $R_s.h \geq h'$, with the bit coordinate of $R_s$ being $1$ whenever $R_s.h = h'$.

\emph{Step 3: case $h_c = h + 1$.}
A lock at $h_c = h + 1$ would force $R_s.h \geq h + 1$ by the characterization, contradicting $R_s.h = h$.
...
\emph{Step 4: case $h_c = h$.}
If $i$ is not locked, the non-locked branch gives $T_c = T \preceq X$ with $\sigma[T].h = h_c$ directly.

Suppose instead~$i$ is locked at $h_c = h$ with lock-target~$L$.
The lock characterization gives $\mathsf{key}(R_s) \geq (h, 1, L.\slot, \mathsf{hash}(L))$; with $R_s.h = h$ this forces $R_s.\mathsf{bit} = 1$.
Hence $N = R_s.\mathsf{block}$ was justified at height~$h$ on some processed chain; the locking (R1) also justified~$L$ at height~$h$, so Lemma~\ref{lem:just-unique-height} gives $L = N$.
With Step~1, $T_c = L = N \preceq X$ and $\sigma[T_c].h = h = h_c$.
...
The hypothesis is used in Step~4 (Lemma~\ref{lem:just-unique-height} for same-height justification uniqueness) and implicitly in Step~1 (via the fact that $R_s.\mathsf{block}$ coincides with the latest-observed cert target, which is on canonical because $X \succeq R_s$); together these force the locked-branch target~$L$ to coincide with $R_s.\mathsf{block}$, hence to lie on canonical.
