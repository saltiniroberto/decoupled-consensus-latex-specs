...
A \emph{vote} is a signed tuple $(\validator,\; \height,\; \target,\; \kind,\; \mathsf{finalize\_height})$ where $\validator$ identifies the signer, $\height$ is a height, $\target$ is a block, $\kind \in \{\notarize, \justify\}$ distinguishes the vote type, and $\mathsf{finalize\_height}$ is either a height or~$\bot$.
...
A block~$T$ is \emph{justified at height~$h$} if at least $2n/3$ validators cast $\justify$ votes with $\height = h$ and $\target = T$:
\[
  \bigl|\{i \,:\, \mathsf{jtargets}[i] = T\}\bigr| \;\geq\; 2n/3.
\]
Only votes targeting~$T$ itself contribute; votes for ancestors or descendants do not count.
\end{definition}

\begin{definition}[Fixed-target notarization]\label{def:notarization}
A block~$T$ is \emph{notarized at height~$h$} if at least $2n/3$ validators cast $\notarize$ votes with $\height = h$ and $\target = T$:
\[
  \bigl|\{i \,:\, \mathsf{ntargets}[i] = T\}\bigr| \;\geq\; 2n/3.
\]
...
\Function{$\textsf{processVote}$}{$(h,\, \mathsf{jtargets},\, \mathsf{ntargets},\, J,\, h_j,\, F,\, P,\, N^\mathsf{max}),\; v,\; B$}
  \State $i \gets v.\validator$
  \If{$v.\height = h$ and $v.\target \preceq B$}
    \If{$v.\kind = \justify$ and $\mathsf{jtargets}[i] = \bot$}
      \State $\mathsf{jtargets}[i] \gets v.\target$
    \EndIf
    \If{$v.\kind = \notarize$} \Comment{Only notarize votes}
      \State $\mathsf{ntargets}[i] \gets v.\target$
    \EndIf
  \EndIf
  \If{$v.\mathsf{finalize\_height} = h_j$}
    \State $P \gets P \cup \{i\}$
  \EndIf
  \State \Return $(h,\, \mathsf{jtargets},\, \mathsf{ntargets},\, J,\, h_j,\, F,\, P,\, N^\mathsf{max})$
\EndFunction
\vspace{3pt}

\Function{$\textsf{processHeight}$}{$(h,\, \mathsf{jtargets},\, \mathsf{ntargets},\, J,\, h_j,\, F,\, P,\, N^\mathsf{max}),\; B$}
  \If{$|P| \geq 2n/3$} \Comment{Finality}
    \State $F \gets J$
  \EndIf
  \If{$\exists\, T$ s.t.\ $|\{i : \mathsf{jtargets}[i] = T\}| \geq 2n/3$} \Comment{Justification}
    \If{$T \succeq N^\mathsf{max}$} \Comment{Justified block advances}
      \State $J \gets T$, \; $h_j \gets h$, \; $P \gets \emptyset$, \; $N^\mathsf{max} \gets T$
    \EndIf
    \State $h \gets h + 1$
    \For{all $i$} $\mathsf{jtargets}[i] \gets \bot$, \; $\mathsf{ntargets}[i] \gets \bot$ \EndFor
    \State \Return $(h,\, \mathsf{jtargets},\, \mathsf{ntargets},\, J,\, h_j,\, F,\, P,\, N^\mathsf{max})$
  \EndIf
  \If{$\exists\, N$ s.t.\ $|\{i : \mathsf{ntargets}[i] = N\}| \geq 2n/3$} \Comment{Notarization}
    \If{$N \succeq N^\mathsf{max}$}
      \State $N^\mathsf{max} \gets N$
    \EndIf
    \State $h \gets h + 1$
    \For{all $i$} $\mathsf{jtargets}[i] \gets \bot$, \; $\mathsf{ntargets}[i] \gets \bot$ \EndFor
    \State \Return $(h,\, \mathsf{jtargets},\, \mathsf{ntargets},\, J,\, h_j,\, F,\, P,\, N^\mathsf{max})$
...
The state after block~$B$ is written $\sigma[B]$; the genesis state is $\sigma[B_{\mathrm{genesis}}] = (0,\, \bot^n,\, \bot^n,\, B_{\mathrm{genesis}},\, 0,\, B_{\mathrm{genesis}},\, \emptyset,\, \bot)$.
...
\State $\sigma[B_{\mathrm{genesis}}] \gets (0,\; \bot^n,\; \bot^n,\; B_{\mathrm{genesis}},\; 0,\; B_{\mathrm{genesis}},\; \emptyset,\; \bot)$
...
\Function{$\textsf{onVote}$}{}
  ...
  \If{$\lnot\, \sentjustify[h^\mathsf{can}]$}
    \State \textbf{send} $(i, h^\mathsf{can}, \getConfirmed(), \justify, \mathsf{fh})$
