...
Moreover $J$ and $h_j$ advance only on justification events, and $s_h$ advances only on height-advances, each time being set to the slot of the advancing block.
...
\begin{proof}
Since $\sigma[B].h > h$, Lemma~\ref{lem:heightprog} gives a block $B^* \preceq B$ at which the advance from height~$h$ to~$h+1$ fired, via a height-$h$ justification or prefix-notarization quorum~$Q$: each $i \in Q$ contributed a vote~$a_i$ with $a_i.\height = h$ processed freshly on the chain ending at~$B^*$ (directly for $\just$; for prefix $\notar$, the $\mathsf{ntargets}$-update rule records only fresh votes).
...
Applying E1 to $v$'s votes~$a \in Q$ and $b \in Q_F$: $b$ carries a finalize commitment to~$C$ at height~$h$, and $a.\height = h$, so $a.\target \neq C$ would be slashable.
Hence $a.\target = C$, and freshness of~$a$ on~$B^*$'s chain gives $C = a.\target \preceq B^* \preceq B$.
\end{proof}
...
By quorum intersection $|Q_F \cap Q'| \geq n/3$; for any shared signer~$v$, E1 applied to $v$'s votes in~$Q_F$ and~$Q'$ forces $C = C'$ or renders~$v$ slashable.
Under the hypothesis, some such~$v$ is not slashable, so $C = C'$.
...
Two results follow.
Tightness (Theorem~\ref{thm:tightness}) is purely state-logic: it reads $\sigma.\mathsf{penalties}$ on the canonical chain and bounds its cumulative increment over periods of non-finality, assuming nothing about message delivery, honesty, or the size of the slashable set.
...
By the period hypothesis, $\sigma_{\mathrm{pre}}^{(t)}.F = \sigma_{\mathrm{post}}^{(t)}.F$ at every slot~$t$ in the period.
Partition the slots of the period by whether height advances and, if so, whether~$J$ advances.

\emph{Case~1: stall slot} ($\sigma_{\mathrm{pre}}.h = \sigma_{\mathrm{post}}.h$).
Neither height-advance branch of $\textsf{processHeight}$ fired, so the prefix-notarization walk summed to $< 2n/3$ on~$X$ from $\sigma.L$ down to genesis.
By freshness (Remark~\ref{rem:fresh-equiv}), every~$i$ with $\sigma_{\mathrm{post}}.\mathsf{ntargets}[i] \neq \bot$ has its target on~$X$, so the walk counts it exactly once; hence $|\{i : \sigma_{\mathrm{post}}.\mathsf{ntargets}[i] \neq \bot\}| < 2n/3$, and Layer~1 charges $> n/3$ validators.

\emph{Case~2: advance slot, $J$ unchanged} ($\sigma_{\mathrm{pre}}.h < \sigma_{\mathrm{post}}.h$ and $\sigma_{\mathrm{pre}}.J = \sigma_{\mathrm{post}}.J$).
The Layer~2 target guard $\sigma_{\mathrm{pre}}.J = \sigma_{\mathrm{post}}.J$ holds, so Layer~2 target fires with plurality target $T^*$ (possibly~$\bot$).
The justification branch of $\textsf{processHeight}$ strictly advances~$J$ whenever it fires (Lemma~\ref{lem:slotmono}), so it did not fire this slot, giving $|\{i : \sigma_{\mathrm{pre}}.\mathsf{jtargets}[i] = T^*\}| < 2n/3$ (counting $T^* = \bot$ as~$0$).
...
\emph{Case~3: advance slot, $J$ updates} ($\sigma_{\mathrm{pre}}.h < \sigma_{\mathrm{post}}.h$ and $\sigma_{\mathrm{pre}}.J \neq \sigma_{\mathrm{post}}.J$).
Call such a slot a \emph{checkpoint-update slot}.
Layer~2 target may charge~$0$ (if $\geq 2n/3$ validators had $\mathsf{jtargets}[i] = \sigma_{\mathrm{post}}.J$).
But finality does not advance inside the period, so $\sigma_{\mathrm{post}}.F \prec \sigma_{\mathrm{post}}.J$ --- a pending-finality window is open at the end of the slot.
We amortize this slot against a distinct Layer~2 finalize slot later in the period.

\emph{Attribution.}
Let $t_1 < t_2 < \cdots < t_K$ be the checkpoint-update slots of the period.
Fix a $t_j$ and let $t' > t_j$ be any later slot of the period:
$\sigma_{\mathrm{pre}}^{(t')}.J = \sigma_{\mathrm{post}}^{(t_j)}.J$ if no checkpoint-update intervened;
$\sigma_{\mathrm{pre}}^{(t')}.F = \sigma_{\mathrm{post}}^{(t_j)}.F$ since finality does not advance in the period;
and $\sigma_{\mathrm{post}}^{(t_j)}.F \prec \sigma_{\mathrm{post}}^{(t_j)}.J$ since $J$ strictly advanced at~$t_j$ while~$F$ did not.
In particular, taking $t' = t_j + 1$ (which lies in the period whenever $t_j$ is not the last slot, and never has an intervening checkpoint-update), the Layer~2 finalize guard holds at $t_j + 1$; since finality did not fire (else it would advance), $|\sigma_{\mathrm{pre}}^{(t_j + 1)}.P| < 2n/3$ and Layer~2 finalize charges $> n/3$ validators there.

For each $j$ such that $t_j$ is not the last slot of the period, attribute $t_j$ to~$t_j + 1$.
The targets $t_j + 1$ are distinct across~$j$, so the attribution is injective.
The charge it ``takes'' from $t_j + 1$ is the Layer~2 finalize charge ($> n/3$), which is independent of any Case~1 or Case~2 charge that may also apply at $t_j + 1$; the two are summed by $\textsf{processInactivityPenalties}$.
At most one checkpoint-update ($t_K$, when $t_K$ is the period's last slot) remains unattributed; dropping it contributes an $O(1)$ correction, negligible in the per-slot average.

Summing over the period:
every Case~1 or Case~2 slot contributes $> n/3$ via its case-direct charge;
every attributed Case~3 slot is covered by an injectively-assigned Layer~2 finalize charge at $t_j + 1$, also $> n/3$, disjoint from the case-direct charges.
The total is $\geq (T - 1) \cdot n/3$ where~$T$ is the period length (at most one slot is excluded), so the average is $\geq (T-1)/T \cdot n/3$, which is~$\geq n/3$ up to an $O(1/T)$ correction.

The bound is purely state-logic: the proof reads $\sigma_{\mathrm{pre}}$ and $\sigma_{\mathrm{post}}$ on the canonical chain and counts penalty increments directly from $\textsf{processInactivityPenalties}$, with no hypothesis on synchrony, on the honest voting rule, or on the size of the slashable set.
\end{proof}
...
Under the hypotheses, every canonical height-advance is witnessed by the $> 2n/3$ honest (R1) quorum agreeing on~$T$ (Corollary~\ref{cor:agreement}), so canonical state-height is monotone over time.
Since $v_2$ was cast at canonical height $h_c > h_*$, canonical cannot return to height~$h_*$ for any later~$v_1$; the case is vacuous.
