...
The genesis height is~$0$; height advances by exactly~$1$ via fixed-target justification (Definition~\ref{def:justification}) or a timeout (Definition~\ref{def:timeout}).
\end{definition}
...
\[
  \sigma_{\mathrm{gen}} \coloneqq (\genesis,\; 0,\; 0,\; 0,\; \bot^n,\; \mathtt{false}^n,\; \genesis,\; 0,\; \genesis,\; \emptyset),
\]
...
Blocks at $\sigma$-height $\hmax - 1$ may still conflict with~$F$, since mainsafety constrains only strictly higher heights --- this is why $\Sigma.J$ remains the primary walk-from block in the cascade whenever it has not itself been filtered out.
...
The protocol fixes the walk-from rule via a two-way cascade: walk from $\Sigma.J$ when $\Sigma.J \in \viableTree(\Sigma)$, and otherwise walk from $\Sigma.F$ (always viable; Lemma~\ref{lem:F-viable}).
...
\State \textbf{Genesis store} $\Sigma$:\ $\Sigma.\T \gets \{\genesis\},\ \Sigma.F \gets \genesis,\ \Sigma.J \gets \genesis,\ \Sigma.h_j \gets 0,\ \Sigma.\hmax \gets 0,$
...
\Function{\getConfirmed}{$\Sigma,\; \Omega$}
  \State $R \gets \Sigma.J$ \textbf{if} $\Sigma.J \in \viableTree(\Sigma)$ \textbf{else} $\Sigma.F$ \Comment{$\Sigma.F$ always viable; Lemma~\ref{lem:F-viable}}
  \State \Return a block in $\viableTree(\Sigma)$ with $\sigma$-height $\geq \Sigma.\hmax - 1$ that descends from $R$, depending on~$\Omega$
\EndFunction
...
\begin{proof}
$\Sigma.J \succeq F$ by Lemma~\ref{lem:upgrade}; $F \in \viableTree(\Sigma)$ by Lemma~\ref{lem:viable-finalized}.
If $\Sigma.J \in \viableTree(\Sigma)$, $\getConfirmed$ enters its first branch and returns a descendant of $\Sigma.J \succeq F$.
Otherwise every leaf in $\Sigma.J$'s subtree has $\sigma$-height $< \Sigma.\hmax - 1$, so $\sigma[\Sigma.J].h \leq \Sigma.\hmax - 2$ by state-height monotonicity (Lemma~\ref{lem:slotmono}); combined with $\sigma[\Sigma.J].h \geq h_F$ (state-height monotonicity along $F \preceq \Sigma.J$, using $\sigma[F].h = h_F$ from Remark~\ref{rem:fresh-equiv}), this gives $\Sigma.\hmax \geq h_F + 2$.
\getConfirmed's second branch returns a block~$T$ with $\sigma[T].h \geq \Sigma.\hmax - 1 \geq h_F + 1 > h_F$, so Lemma~\ref{lem:mainsafety} gives $F \preceq T$.
\end{proof}
...
$\Delta$ may safely drop entries at heights below $\Sigma.\hmax - 1$; \getFinalVote then conservatively clears the finalize commitment at older heights (because the gate $\Delta.T(h_f) = T_f$ fails on a missing entry), but every emitted vote remains E1-safe.
...
Only the first-vote-at-$h_c$ branch produces finalize-bearing votes, so $v_0$'s firing read $(h_j^0, J^0) = (h_c, T_{\mathrm{lock}})$ from~$i$'s then-canonical state, meaning the justification branch of \processHeight fired with target~$T_{\mathrm{lock}}$ at pre-state height~$h_c$ inside some \onBlock call on a processed block~$B^\star$, whose post-state has $\sigma[B^\star].h \geq h_c + 1$ (height-advance).
...
$B^\star$ descends from $\Sigma.J = T_{\mathrm{lock}}$ (it carries the justification with target~$T_{\mathrm{lock}}$) at $\sigma$-height~$\Sigma.\hmax$.
Any leaf $L \succeq B^\star$ in $\Sigma.\T$ has $\sigma[L].h \geq \sigma[B^\star].h = \Sigma.\hmax$ by state-height monotonicity, so $L$ is a viable leaf descendant of~$\Sigma.J$, placing $\Sigma.J \in \viableTree(\Sigma)$.
\getConfirmed therefore walks from~$\Sigma.J$, so $T \succeq \Sigma.J = T_{\mathrm{lock}}$, with $\sigma[T_{\mathrm{lock}}].h = h_c$ by Remark~\ref{rem:fresh-equiv} on the chain witnessing $T_{\mathrm{lock}}$'s justification.
\end{proof}
