...
% height_filter_prefix_justification_lite.tex with \litemode defined (lite version).
...
Justification certifies the deepest block with a quorum of supporting justification votes; finalization sets $F \gets J$ (the justified block) once a quorum has committed to finalize it, exactly as in \emph{Finalization as Notarization}, subject to the \emph{finalize gate} --- a commitment counts only once the caster's justification vote at that height has been processed, and only up to that justification's target --- so a finalize quorum for~$J$ is also a justification quorum for~$J$.
...
\[
  (L,\; s,\; h,\; s_h,\; \targets,\; \timeouts,\; \jtargets,\; J,\; h_j,\; F,\; P),
\]
with components:
\begin{itemize}
  ...
  \item $\jtargets[1..n]$: for each~$i$, the target of~$i$'s justification vote \emph{at the justified height~$h_j$} ($\bot$ if none), snapshotted from $\targets$ when height~$h_j$ was justified. This is what the finalize gate consults.
  \item $J$, $h_j$: the most recently justified block and its height.
  \item $F$: the most recently finalized block.
  \item $P \subseteq \{1, \ldots, n\}$: the validators whose finalize commitment for the current justified height~$h_j$ has passed the finalize gate (Definition~\ref{def:finalize-gate}); finality fires when $|P| \geq 2n/3$.
\end{itemize}
The genesis state is
\[
  \sigma_{\mathrm{gen}} \coloneqq (\genesis,\; 0,\; 1,\; 0,\; \bot^n,\; \mathtt{false}^n,\; \bot^n,\; \genesis,\; 0,\; \genesis,\; \emptyset),
\]
...
A finalize commitment $(\finalizeHeight,\, \finalizeTarget) = (h_j,\, D)$ from validator~$i$ is \emph{admitted} on a chain with head state $\sigma[L]$ (placing $i \in P$) iff
\[
  \jtargets[i] \neq \bot \;\wedge\; J \preceq D \preceq \jtargets[i] :
\]
$i$'s justification vote at the justified height~$h_j$ has already been processed on this chain, and~$i$'s finalize target lies between the justified block~$J$ and that justification target.
An admitted commitment therefore witnesses that~$i$ justified a descendant of~$J$ (namely $\jtargets[i] \succeq D \succeq J$) at height~$h_j$.
\end{definition}

\begin{definition}[Finality]\label{def:finality}
On a chain with head state $\sigma[L]$ whose justified height is~$h_j$, the justified block~$J$ is \emph{finalized at height~$h_j$} once
\[
  |P| \;\geq\; 2n/3 ,
\]
i.e., once at least $2n/3$ validators have an admitted finalize commitment (Definition~\ref{def:finalize-gate}) for~$h_j$.
The finalized block~$F$ is then set to~$J$ (and never regresses); as in \emph{Finalization as Notarization}, finalization assigns the justified block rather than walking a separate longest-common-prefix.
\end{definition}

\begin{remark}[The gate makes the finalize quorum a justification quorum]\label{rem:gate-prefix-justified}
Finalizing $F \gets J$ keeps the finalized block equal to the justified block, so it is prefix-justified (Definition~\ref{def:justification}) by construction.
The role of the finalize gate (Definition~\ref{def:finalize-gate}) is to make each admitted commitment \emph{witness a justification vote for~$J$}: every $i \in P$ has $\jtargets[i] \succeq J$, so $P$ is itself a $\geq 2n/3$ justification quorum for~$J$.
This is what lets accountable safety tie each finalize commitment back to a justification vote for~$J$ (Section~\ref{sec:safety}).
\end{remark}
...
\processHeight runs the height-closing logic for the current slot or post-block state by invoking \processHeightEvents, whose branches are checked in order: (i) finality, $F \gets J$ if $|P| \geq 2n/3$ (Definition~\ref{def:finality}); (ii) prefix justification, if the deepest prefix-justified block exists---which also snapshots the current $\targets$ into $\jtargets$ and clears $P$; (iii) timeout cert, if $\geq 2n/3$ entries of $\timeouts$ are \texttt{true}.
...
The finalize gate (Definition~\ref{def:finalize-gate}) adds~$i$ to~$P$ iff $v.\finalizeHeight = \sigma.h_j$, $\jtargets[i] \neq \bot$, and $\sigma.J \preceq v.\finalizeTarget \preceq \jtargets[i]$ (so~$i$'s justification vote at~$h_j$ is already recorded and~$i$'s finalize target lies between~$J$ and that vote's target); this is independent of the freshness of~$v$'s own justification part.
...
\Function{\stateTransition}{$\sigma,\; B$} \Comment{Per-block entry point}
  ...
  \State \Return $\processHeight(\sigma)$ \Comment{close events enabled by $B$'s votes}
\EndFunction
\vspace{3pt}
...
\Function{\processVote}{$\sigma,\; v$}
  ...
  \State \textbf{if} $v.\finalizeHeight = \sigma.h_j \,\land\, \sigma.\jtargets[i] \neq \bot \,\land\, \sigma.J \preceq v.\finalizeTarget \preceq \sigma.\jtargets[i]$ \textbf{then} $\sigma.P \gets \sigma.P \cup \{i\}$ \Comment{finalize gate}
  \State \Return $\sigma$
\EndFunction
\vspace{3pt}
...
\Function{\processHeightEvents}{$\sigma$}
  \If{$|\sigma.P| \geq 2n/3$} \State $\sigma.F \gets \sigma.J$ \EndIf \Comment{finality}
  ...
  \If{$J^* \neq \bot$} \Comment{prefix justification}
    \State $\sigma.\jtargets \gets \sigma.\targets$ \Comment{snapshot justify targets at $h_j$}
    \State $\sigma.J \gets J^*$, \; $\sigma.h_j \gets \sigma.h$, \; $\sigma.h \gets \sigma.h + 1$, \; $\sigma.s_h \gets \sigma.s$
    \State reset $\targets, \timeouts$; \quad $\sigma.P \gets \emptyset$
    \State \Return $\sigma$
  \EndIf
...
\begin{proof}
Finality of~$C$ gives $|P| \geq 2n/3$, with $C = J$ (finality sets $F \gets J$).
Each $i \in P$ was admitted by the finalize gate (Definition~\ref{def:finalize-gate}), so $\jtargets[i] \succeq J = C$, i.e.,~$i$'s justification vote at height~$h$ has target $\jtargets[i] \succeq C$.
Thus $P$ witnesses $|\{i : \jtargets[i] \succeq C\}| \geq 2n/3$, which is prefix justification of~$C$ at~$h$.
\end{proof}
...
Finality of~$C$ at~$h$ gives the finalize quorum~$P$ with $|P| \geq 2n/3$ and $C = J$.
Fix a non-slashable $v \in Q \cap P$ (quorum intersection, $|Q \cap P| \geq n/3$).
By the finalize gate (Definition~\ref{def:finalize-gate}), $v$ cast a finalize commitment~$b_v$ with $b_v.\finalizeHeight = h$ and $C \preceq b_v.\finalizeTarget \eqqcolon D \preceq \jtargets[v]$, where $\jtargets[v] \neq \bot$ is~$v$'s justification target at height~$h$.
...
\emph{$a$ is a fresh justification vote} (so either~$Q$ is a justification quorum, or~$a$ set~$v$'s timeout marker as a fresh justification, Definition~\ref{def:timeout}). Then $a.\height = h$ and $a.\target \preceq B^*$ by freshness. By E1, all of~$v$'s justification votes at height~$h$ carry a single target, so $a.\target = \jtargets[v] \succeq D \succeq C$. Hence $C \preceq a.\target \preceq B^*$.
...
\emph{Case $h < h'$.}
Let~$Z$ be the chain on which~$C'$ was finalized, at the post-state where its finalize quorum closed; then $\sigma[Z].h \geq h' > h$ and $C' \preceq Z$.
...
Each vote carries a \emph{protective prefix} $W_v \preceq T$: a block at the current height layer (or~$\bot$) whose presence on a chain makes~$v$ set~$i$'s timeout marker there.
