...
If $h_R = h_F$, the bound forces $\mathsf{bit}_R = 1$, so $R_s$ is justified at height~$h_F$, and Lemma~\ref{lem:just-unique-height} gives $R_s = F$.
\end{proof}
...
For any $(h, b, C) \in D$ with $h \geq h_{F_{\max}}$, Lemma~\ref{lem:certchain} gives $C \succeq F_{\max}$ (strict when $h > h_{F_{\max}}$); at $h = h_{F_{\max}}$, Remark~\ref{rem:fs-invariant} with $F_{\max}$ justified gives $\Sigma.\mathsf{key}_R \geq K_{\max}$, forcing any competitive descriptor to have bit~$1$, and Lemma~\ref{lem:just-unique-height} then gives $C = F_{\max}$.
...
Let $X = \textsf{getHead}(\Sigma)$ at the voting opportunity, with $h_c = \sigma[X].h$, $J = \sigma[X].J$, $h_j = \sigma[X].h_j$.
Write $\mathsf{currentSlot}$ for the node's protocol time (a slot counter shared across chains, with $\mathsf{currentSlot} = \sigma[X].s \geq \sigma[X].s_h$), and let $T$ be any ancestor of~$X$ with $\sigma[T].h = h_c$ (e.g.~$X$ itself).

\emph{Target selection.} Validator~$i$ is \emph{locked at~$h_c$} if it previously cast a vote with $\mathsf{finalize\_height} = h_c$ and $\mathsf{finalize\_target} = L \neq \bot$; by E1, any future height-$h_c$ vote must then have target~$L$.
The target of~$i$'s vote at height~$h_c$ is
\[
  T_c = \begin{cases} L & \text{if~$i$ is locked at $h_c$,} \\ T & \text{otherwise.} \end{cases}
\]

\emph{Voting rules.} At each voting opportunity at canonical height~$h_c$:
\begin{enumerate}
  \item[(R1)] If $i$ has not yet cast any vote at height~$h_c$, cast a $\just$ vote
  \[ v = (i,\; h_c,\; \mathsf{currentSlot},\; T_c,\; \just,\; f_h,\; f_t) \]
  with finalize-fields $(f_h, f_t) = (h_j,\, J)$ if every prior vote of~$i$ at height~$h_j$ had target~$J$, else $(\bot,\bot)$.
  \item[(R2)] Otherwise, cast a $\notar$ vote
  \[ v = (i,\; h_c,\; \mathsf{currentSlot},\; T_c,\; \notar,\; \bot,\; \bot). \]
\end{enumerate}
Both rules read $T, J, h_j$ from the current $\sigma[X]$; if~$i$'s canonical view shifted between firings, $T_c$ may shift accordingly.
Voting at~$h_c$ stops once canonical advances past~$h_c$.
\end{definition}

Only (R1) produces a $\just$ vote and it fires at most once per~$h_c$, so~$i$ casts at most one $\just$ vote per height, ensuring per-height $\just$-uniqueness.
Rule~(R2) is the \emph{resubmission} fallback: when the (R1) $\just$ quorum did not form on canonical (heterogeneous targets across views), the $\notar$ vote's prefix counting may still notarize a common ancestor.
Re-reading canonical at each firing ensures each vote tracks~$i$'s current view; the lock clause preserves E1-safety under view shifts.

Checking \emph{all} prior votes at~$h_j$ (not just the (R1) one) is required because a (R2) at~$h_j$ may have chosen a target differing from the (R1) target if~$i$'s view shifted; committing $f_t = J$ at height~$h_j$ requires (by E1) that every $h_j$-vote of~$i$ have target~$J$, so in case of a split the conditional fires $(\bot, \bot)$: $i$ misses the finalization at~$h_j$ but remains E1-safe.
...
\begin{proof}
It suffices to show $T_c \preceq X$ with $\sigma[T_c].h = h_c$: freshness (Remark~\ref{rem:fresh-equiv}) then makes~$v$ canonical-fresh on any $X' \succeq X$ at state-height $h_c$, processing~$v$ populates $\mathsf{ntargets}[i]$, and Layer~1 does not bump a validator with $\mathsf{ntargets}[i] \neq \bot$.

\emph{Step 1: $R_s \preceq X$ and $\sigma[X].h \geq \Sigma.\mathsf{key}_R.h$.}
$X$ descends from~$R_s$ (Figure~\ref{alg:store}), so $R_s \preceq X$.
$R_s$ is either genesis or was set by a cert event firing at height $\Sigma.\mathsf{key}_R.h$ on a processed chain, so $\sigma[R_s].h = \Sigma.\mathsf{key}_R.h$ (Remark~\ref{rem:fresh-equiv}, with genesis trivial).
State-height monotonicity (Lemma~\ref{lem:slotmono}) then gives $h_c \geq \Sigma.\mathsf{key}_R.h$.

\emph{Step 2: $h_c \leq \Sigma.\mathsf{key}_R.h + 1$.}
Every state-height advance on~$X$ from $h'$ to $h'+1$ fires a cert event at height $h'$ (Lemma~\ref{lem:heightprog}) and is passed through $\textsf{updateForkChoiceRoot}$; the key $(h',\, \cdot,\, \cdot,\, \cdot)$ raises $\Sigma.\mathsf{key}_R.h$ to at least $h'$ whenever $h' > \Sigma.\mathsf{key}_R.h$.
Hence no advance from $\Sigma.\mathsf{key}_R.h + 1$ to $\Sigma.\mathsf{key}_R.h + 2$ has fired on~$X$, and $h_c \in \{\Sigma.\mathsf{key}_R.h,\; \Sigma.\mathsf{key}_R.h + 1\}$.

\emph{Step 3: lock characterization.}
A lock at height~$h'$ requires a prior (R1) firing of~$i$ with finalize-fields $(h',\, J_\ast)$, meaning~$J_\ast$ was justified at height~$h'$ on some processed chain.
That justification fired a cert event with key $(h',\, 1,\, J_\ast.\slot,\, \mathsf{hash}(J_\ast))$, which $\textsf{updateForkChoiceRoot}$ passed into $\Sigma.\mathsf{key}_R$; Lemma~\ref{lem:Rs-key-monotone} preserves this bound, so $\Sigma.\mathsf{key}_R \geq (h',\, 1,\, J_\ast.\slot,\, \mathsf{hash}(J_\ast))$ thereafter.

\emph{Step 4: $h_c = \Sigma.\mathsf{key}_R.h + 1$.}
By Step~3, a lock at $h_c$ would force $\Sigma.\mathsf{key}_R.h \geq h_c = \Sigma.\mathsf{key}_R.h + 1$ --- absurd.
So $i$ is not locked, and $T_c = T \preceq X$ with $\sigma[T].h = h_c$.

\emph{Step 5: $h_c = \Sigma.\mathsf{key}_R.h$.}
If $i$ is not locked, $T_c = T \preceq X$ directly.
If~$i$ is locked at $h_c$ with target~$L$, Step~3 gives $\Sigma.\mathsf{key}_R \geq (h_c,\, 1,\, L.\slot,\, \mathsf{hash}(L))$; matching leading coordinates forces $\Sigma.\mathsf{key}_R.b = 1$, so $R_s$ was itself justified at height~$h_c$ on some processed chain.
The locking (R1) also justified~$L$ at height~$h_c$, so Lemma~\ref{lem:just-unique-height} gives $L = R_s$, and Step~1 yields $T_c = L = R_s \preceq X$ with $\sigma[T_c].h = h_c$.

\emph{Conclusion.} Each case gives $T_c \preceq X \preceq X'$ and $\sigma[T_c].h = h_c$.
...
The slashable-set hypothesis is used only in Step~5, via Lemma~\ref{lem:just-unique-height}, to collapse the locked-branch target~$L$ onto $R_s$.
