...
\newcommand{\Wvote}{\ensuremath{W}}         % latest-head-vote expiry window (128 slots)
...
\newcommand{\anchor}{\textsf{anchor}\xspace}
% --- Procedure names ---
...
\begin{itemize}
  ...
  \item \textbf{L2 --- stabilization gadget (SG), latest messages.}  The
    ...
    \emph{point to} a \emph{fresh quorum} of the previous round's
    attestations, whose anchor every validator then adopts for the whole
    round; absent a valid pointed quorum, the anchor is the deepest block
...
(pointed fresh quorum, or the grade-1 descent as fallback), follows Goldfish
...
another delivery path and creates no distinct object.  The attestation is
\emph{live} while $v.\slot + \Wvote > s$, where $\Wvote=128$ slots.
...
\subsection{Fresh quorums, anchors, and the walk}\label{sec:walk}
...
\subsubsection*{Fresh quorums and anchors}

The walk's starting point below the cascade root is an \emph{anchor}, fixed
in one round-atomic step from FG attestations rather than descended through a
sequence of local grade thresholds.  FG voting is round-atomic: each validator
emits exactly one FG attestation per round (Section~\ref{sec:timing},
Definition~\ref{def:round-atomic}), whose head field is its walk output.

\begin{definition}[Round-$r$ quorum, fresh quorum, and anchor]\label{def:fresh-quorum}
A \emph{round-$r$ quorum} is a set~$Q$ of round-$r$ FG attestations from
\emph{distinct} validators whose effective balances sum to at least
$\tfrac23$ of the \emph{total} stake (an \emph{absolute} threshold, not a
fraction of any local live-head-vote weight).  The \emph{anchor} of~$Q$,
\[
  \anchor(Q) \;\coloneqq\; \text{the highest common ancestor of }
  \{\, v.\headf : v \in Q \,\},
\]
is the deepest block whose subtree contains every head field of~$Q$; it is a
function of~$Q$, not a free choice.  We say $Q$ is a round-$r$ quorum
\emph{for} a block~$B$ iff $B \preceq \anchor(Q)$, equivalently
$B \preceq v.\headf$ for every $v \in Q$.

A \emph{fresh quorum} for the current round is a round-$r$ quorum with~$r$ the
\emph{immediately preceding} round --- the latest round whose attestations are
complete.  (``Fresh'' here means \emph{from the latest round}; it is unrelated
to the \emph{height} freshness of a justification vote in
Section~\ref{sec:model}, which pins a vote to a chain's current height.  A
fresh quorum is a cross-validator object of the anchor layer; height
freshness is a per-vote predicate of the FG state machine.)
\end{definition}

Two facts make the anchor a sound synchronization object: conflicting anchors
are impossible without mass equivocation (the safety backstop, mirroring the
FG's own quorum intersection), and the honest validators alone always form a
fresh quorum (so an anchor is always available under the standing
hypotheses).

\begin{lemma}[Anchor intersection]\label{lem:quorum-intersection-sg}
If two fresh quorums~$Q, Q'$ have conflicting anchors
($\anchor(Q) \nsim \anchor(Q')$), then at least $n/3$ of the stake belongs to
validators that equivocated in the shared round --- signed two distinct
round-$r$ attestations --- and are therefore slashable.  Hence, unless
$\geq n/3$ of the stake is slashable, all fresh quorums of a round have
pairwise compatible anchors.  The conclusion holds even when the equivocations
are never exhibited to a verifier: it is forced by the counting, not by any
on-chain proof.
\end{lemma}

\begin{proof}
Conflicting anchors have disjoint subtrees: a common descendant of
$\anchor(Q)$ and $\anchor(Q')$ would make them comparable
(Definition~\ref{def:chain}).  Every $v \in Q$ has $v.\headf \succeq \anchor(Q)$
and every $v' \in Q'$ has $v'.\headf \succeq \anchor(Q')$, so no single
round-$r$ attestation lies in both~$Q$ and~$Q'$.  A validator that contributes
a (necessarily non-equivocating, single) round-$r$ attestation to both quorums
would thus have contributed \emph{two distinct} round-$r$ attestations --- an
equivocation.  Now count by stake: $w(Q) + w(Q') \geq \tfrac23 + \tfrac23 =
\tfrac43$ of the total, while the union of their signers has stake at most~$1$
(the total).  Hence the stake of validators appearing in \emph{both} quorums is
at least $\tfrac43 - 1 = \tfrac13$ of the total, i.e.\ $\geq n/3$, and every
such validator equivocated in round~$r$.  The count uses only the two
$\tfrac23$-absolute weights and the total, so it stands whether or not the two
offending attestations are ever placed side by side for a verifier; this
robustness is exactly why the threshold is $\tfrac23$ of the \emph{total}
rather than a fraction of a view-dependent denominator.
\end{proof}

\begin{lemma}[Honest fresh quorum]\label{lem:honest-fresh-quorum}
Fix a round~$r$ after \GST{} satisfying the standing assumptions
(Definition~\ref{def:standing}): $\beta < 1/3$ and every honest validator
online.  Then the honest round-$r$ attestations form a fresh quorum, whose
anchor is the highest common ancestor of the honest head fields and is a
descendant of the cascade root of every honest view that has processed
round~$r$'s finalized prefix.
\end{lemma}

\begin{proof}
Honest validators hold more than $\tfrac23$ of the stake ($\beta < 1/3$) and
each emits one round-$r$ attestation (Definition~\ref{def:round-atomic}), so
the set~$Q_H$ of honest round-$r$ attestations has $w(Q_H) > \tfrac23$ of the
total and distinct signers: a round-$r$ quorum, hence a fresh quorum.  Its
anchor $A_H \coloneqq \anchor(Q_H)$ is the highest common ancestor of the
honest heads by definition.  Each honest head is the honest caster's own walk
output, a descendant of that caster's cascade root
(Definition~\ref{def:walk}); after \GST{} honest views share the same
store-finalized block and the same cascade root up to the one-height cascade
slack (Theorem~\ref{thm:orderindep}), so every honest head descends from that
common cascade root, and therefore so does their highest common
ancestor~$A_H$.
\end{proof}

\paragraph{Pointing.}
A round's first-slot ($\rzero$) proposer \emph{may} include in its proposal a
reference to one fresh quorum of the previous round.  The reference is an
aggregate over round-$r$ attestations plus a marker identifying them as an
anchor claim.  A validator \emph{verifies} the reference from the aggregate
alone (aggregate signature over distinct round-$r$ signers, total effective
balance $\geq \tfrac23$ of the stake, and the common-subtree condition that
pins $\anchor$); the aggregate riding the proposal makes its votes available
on its own and does not mutate the receiver's latest-message store.  If the
reference verifies, its
anchor~$\anchor(Q)$ is \emph{adopted by every validator for the whole round},
even if other or deeper fresh quorums exist or later become visible.  Anchors
carry \emph{no cross-round state}: each round's anchor is computed from that
round's pointed quorum alone, with no required relation to earlier anchors.
...
Absent a valid pointed quorum, the anchor is fixed by a latest-head-vote
...
used when the round's $\rzero$ proposal does not point to a valid fresh quorum
...
\begin{enumerate}
  \item \emph{Anchor.}  If the current round's $\rzero$ proposal points to a
    valid fresh quorum~$Q$ (Definition~\ref{def:fresh-quorum}) whose anchor
    satisfies $\cascadeRoot(\Sigma) \preceq \anchor(Q)$ and
    $\anchor(Q) \in \viableTree(\Sigma)$, set the anchor $A \gets \anchor(Q)$
    --- the deeper of the cascade root and the pointed anchor, which are
    comparable because both lie on the chain of every head of a fresh quorum.
    (If the pointed anchor is not a descendant of the cascade root --- shallower
    than it, or on a conflicting branch --- or if it has slipped behind the
    viability frontier, ignore the pointed quorum.)  Otherwise set
    ...
    children from the always-viable cascade root, and the pointed anchor is
...
\Function{\walk}{$\Sigma,\; \Omega$}
  ...
  \If{the current round's $\rzero$ proposal points to a valid fresh quorum $Q$, \ $R \preceq \anchor(Q)$, \ \textbf{and} $\anchor(Q) \in \viableTree(\Sigma)$}
    \State $A \gets \anchor(Q)$ \Comment{pointed anchor, adopted for the whole round (Def.~\ref{def:fresh-quorum})}
  \Else
...
$\anchor(Q) \in \viableTree(\Sigma)$ (Definition~\ref{def:walk}).  Phase~2 then
...
or inclusion path.  The fresh quorum a round's $\rzero$ proposal points to
(Definition~\ref{def:fresh-quorum}) is the only separate round-atomic anchor
certificate; it rides the proposal and fixes one common anchor per round.
...
anchor is \emph{not} covered by this step --- the pointed aggregate is a threshold
certificate, not a write to the latest-message store --- and is protected separately by anchor
intersection (Lemma~\ref{lem:quorum-intersection-sg}), as
...
point to a valid fresh quorum (Definition~\ref{def:walk}), evaluated at~\(u\)'s
round freeze after \GST.  Then either \(A = \cascadeRoot(\Sigma_u)\), or
\(\Supp_u(A) \geq \tfrac{2}{3}\Dvote_u\) in~\(u\)'s own view.  A \emph{pointed}
anchor \(A = \anchor(Q)\) carries \emph{no} such latest-head-vote-support guarantee:
\(\Supp_u(\anchor(Q)) \geq \tfrac{2}{3}\Dvote_u\) need not hold.
\end{lemma}
...
The pointed case carries no latest-head-vote-support guarantee: verification of a pointed
quorum~\(Q\) reads only the aggregate signature, distinct-signer count,
\(\tfrac23\)-\emph{absolute} weight, and common-subtree condition
(Definition~\ref{def:fresh-quorum}); the aggregate rides the proposal but is not
thereby inserted into the receiver's latest-message store
(Definition~\ref{def:head-vote}).  Hence no lower bound on
\(\Supp_u(\anchor(Q))\) follows --- before the honest head votes have
concentrated, \(\anchor(Q)\) may have support well below
\(\tfrac23\Dvote_u\), or below \(\tfrac13\Dvote_u\).  Pointed anchors are therefore
protected not by local grade support but by anchor intersection
...
attestations form a fresh quorum~\(Q_C\) whose anchor descends from~\(C\),
\(\anchor(Q_C) \succeq C\) (Lemma~\ref{lem:honest-fresh-quorum} once concentration
has placed honest heads on~\(C\)'s chain, Lemma~\ref{lem:vote-concentration}).  Then no
...
grade-1 anchor by graded delivery, the pointed anchor by anchor intersection.
\end{corollary}
...
\emph{Pointed anchor \(A = \anchor(Q_A)\).}  Here~\(A\) is backed by a
\(\tfrac23\)-absolute quorum~\(Q_A\) but need not carry grade-1 support
(Lemma~\ref{lem:anchor-support}), so the graded-delivery route does not apply.
Instead use the honest fresh quorum~\(Q_C\): its anchor satisfies
\(\anchor(Q_C) \succeq C\), so \(\anchor(Q_C)\) is compatible with~\(C\).  If~\(A\)
conflicted with~\(C\) then~\(A \nsim \anchor(Q_C)\) as well (a block conflicting
with~\(C\) conflicts with every descendant of~\(C\), and \(\anchor(Q_C) \succeq C\)),
so~\(Q_A\) and~\(Q_C\) would be two fresh quorums with conflicting anchors,
forcing \(\geq n/3\) of the stake to be slashable
...
the participation-dependent premise \(\anchor(Q_C) \succeq C\) (honest heads
...
attestations form a fresh quorum~\(Q_C\) with \(\anchor(Q_C) \succeq C\)
...
grade-1 anchors by graded delivery, pointed anchors by anchor intersection --- and
...
anchor \(\Gone\) conflicts with~\(C\); and a fresh quorum for an anchor
conflicting with an honest anchor above~\(C\) would require \(\geq n/3\) slashable
...
round's first slot~\(\rzero\) has an honest proposer, it assembles a fresh quorum
from the previous round's honest attestations (Lemma~\ref{lem:honest-fresh-quorum})
and points to it (Definition~\ref{def:fresh-quorum}); every honest validator
adopts the same anchor~\(A=\anchor(Q)\) for that round.  Above the common anchor,
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item \emph{Behavior for \(\beta \in (1/3, 1/2)\).}
    Above \(\beta = 1/3\) honest stake alone cannot form the
    \(\tfrac23\)-absolute fresh quorum, and the single-step graded-delivery
  ...
  \item \emph{Pointer equivocation by the \(\rzero\) proposer.}
    A first-slot proposer that points to two conflicting fresh quorums in
    ...
    round.  Honest validators adopt the anchor of whichever proposal they
    process on their chain; the anchor intersection lemma
    (Lemma~\ref{lem:quorum-intersection-sg}) guarantees the two anchors cannot be
    both valid and conflicting without \(\geq n/3\) slashable, so healing safety
    is unaffected.
