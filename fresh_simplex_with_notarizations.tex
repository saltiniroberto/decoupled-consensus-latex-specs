...
The store maintains a single \emph{root} $R_s$ --- a block --- together with a cached height $h_R$ and justification-bit $\mathsf{bit}_R$ for the cert event that last set $R_s$; the triple $(R_s, h_R, \mathsf{bit}_R)$ is updated incrementally whenever $\textsf{processHeight}$ fires a height advance (a \emph{cert event}) whose \emph{cert key} strictly exceeds the key currently recorded for the root.
...
\[
  \Sigma \;=\; (\sigma,\; \T,\; F_s,\; R_s,\; h_R,\; \mathsf{bit}_R),
\]
where:
\begin{itemize}
  ...
  \item $h_R$ and $\mathsf{bit}_R$ are the cached height and justification-bit of the cert event that set $R_s$; together with $R_s$'s slot and hash they form the full cert key $(h_R,\, \mathsf{bit}_R,\, R_s.\slot,\, \mathsf{hash}(R_s))$.
\end{itemize}
The capital~$\Sigma$ distinguishes the store from the per-chain state~$\sigma$: $\sigma[B]$ is a single post-state, while $\Sigma$ aggregates the state map together with the fork-choice bookkeeping $(\T, F_s, R_s, h_R, \mathsf{bit}_R)$.
Where unambiguous we write $\Sigma.\sigma$, $\Sigma.\T$, $\Sigma.F_s$, $\Sigma.R_s$, $\Sigma.h_R$, $\Sigma.\mathsf{bit}_R$ for the corresponding fields.
The genesis store has $F_s = \genesis$, $\T = \{\genesis\}$, $R_s = \genesis$, $h_R = 0$, $\mathsf{bit}_R = 1$ (genesis acts as a bit-$1$ cert event at height~$0$).
\end{definition}

\begin{remark}[Store-finalized invariant]\label{rem:fs-invariant}
At all times, $F_s$ is either~$\genesis$ or a block that was finalized on some chain.
Indeed, $F_s$ is only updated by $\textsf{updateFinalized}$ (Figure~\ref{alg:store}) to a value~$F_B = \sigma[B].F$, where $B$ was processed by the node; $\sigma[B].F$ is set only by the finality branch of $\textsf{processHeight}$, which requires prior justification of $F_B$ at its finalization height~$h_{F_s}$ on $B$'s chain.
Consequently, some processed ancestor $B' \preceq B$ has post-state $\sigma[B'].J = F_s$ and $\sigma[B'].h_j = h_{F_s}$; by $\textsf{onBlock}(B')$'s post-$\textsf{processBlock}$ step, the cert-event descriptor $(h_{F_s},\, 1,\, F_s)$ was offered to $\textsf{updateForkChoiceRoot}$ of this node (and was either accepted or dropped by the $F_s$-filter or the lex-strict guard against a higher-key state).
\end{remark}
...
$\textsf{updateForkChoiceRoot}$ first filters out any candidate block~$C$ with $C \not\succeq \Sigma.F_s$, and then updates $\Sigma.R_s$ (together with the cached $\Sigma.h_R$ and $\Sigma.\mathsf{bit}_R$) iff the candidate's cert key strictly exceeds the key currently recorded for the root.
...
\State \textbf{Genesis store} $\Sigma$: \;$\Sigma.\T \gets \{\genesis\}$, \; $\Sigma.F_s \gets \genesis$, \; $\Sigma.R_s \gets \genesis$, \; $\Sigma.h_R \gets 0$, \; $\Sigma.\mathsf{bit}_R \gets 1$,
...
\Function{$\textsf{updateForkChoiceRoot}$}{$\Sigma,\; (h,\, b,\, C)$} \Comment{Filter by $\Sigma.F_s \preceq C$, then running max}
  ...
  \State $\mathsf{curKey} \gets (\Sigma.h_R,\; \Sigma.\mathsf{bit}_R,\; \Sigma.R_s.\slot,\; \mathsf{hash}(\Sigma.R_s))$
  \If{$\mathsf{newKey} > \mathsf{curKey}$ and $F_s \preceq C$}
    \State $\Sigma.R_s \gets C$; \;\; $\Sigma.h_R \gets h$; \;\; $\Sigma.\mathsf{bit}_R \gets b$
  \EndIf
  \State \Return $\Sigma$
\EndFunction
\vspace{3pt}
...
Each call is gated by the filter $C \succeq \Sigma.F_s$, so a candidate on a chain no longer consistent with $F_s$ is dropped; the remaining candidate replaces $\Sigma.R_s$ (and the cached $\Sigma.h_R$, $\Sigma.\mathsf{bit}_R$) iff its key strictly exceeds the current root's.
...
The cached cert key $(\Sigma.h_R,\, \Sigma.\mathsf{bit}_R,\, \Sigma.R_s.\slot,\, \mathsf{hash}(\Sigma.R_s))$ is non-decreasing over time; in particular, its height component $\Sigma.h_R$ is non-decreasing.
\end{lemma}

\begin{proof}
The cached key changes only inside $\textsf{updateForkChoiceRoot}$, whose guard $\mathsf{newKey} > \mathsf{curKey}$ (lex) allows only strict increases; the height coordinate $h$ is the leading component of the key.
\end{proof}
...
The key tool for the remaining store theorems is Lemma~\ref{lem:certchain}: every cert event ever observed by the node, whose height is at least that of a finalized block~$C$, has a block compatible with~$C$, and strictly descending when its height strictly exceeds that of~$C$.

\begin{lemma}[Checkpoint chain]\label{lem:certchain}
Unless $\geq n/3$ validators are slashable: if~$C$ is finalized at height~$h_F$ and a cert event $(X,\, h',\, b')$ has fired on any chain processed by the node with $h' \geq h_F$, then $C$ and~$X$ are compatible; moreover, if $h' > h_F$ then $C \prec X$.
\end{lemma}

\begin{proof}
If $h_F = 0$ then $C = \genesis$ and compatibility is trivial; assume $h_F \geq 1$.
The cert event fired at the post-state of some processed block~$B$ whose $\textsf{processSlot}$ call triggered $\textsf{processHeight}$ with $(\sigma[B].N,\, \sigma[B].h_n) = (X,\, h')$, so $X \preceq B$.
...
The filter $F \succeq F_s^0$ passes and $\textsf{updateForkChoiceRoot}$ proceeds: either it raises $R_s$ to $F$ (with $h_R \gets h_F$ and $\mathsf{bit}_R \gets 1$), or the current $R_s$ already has strictly greater key.
By Lemma~\ref{lem:Rs-key-monotone}, the cached key satisfies $(h_R,\, \mathsf{bit}_R,\, R_s.\slot,\, \mathsf{hash}(R_s)) \geq (h_F,\, 1,\, F.\slot,\, \mathsf{hash}(F))$ from that moment on; in particular $h_R \geq h_F$.
...
Every descriptor $(h,\, b,\, C) \in D$ with $h \geq h_{F_{\max}}$ satisfies $C \succeq F_{\max}$: by Lemma~\ref{lem:certchain}, compatibility, then strict descent when $h > h_{F_{\max}}$; for $h = h_{F_{\max}}$, the key-order lower bound $(h_R,\, \mathsf{bit}_R,\, R_s.\slot,\, \mathsf{hash}(R_s)) \geq (h_{F_{\max}},\, 1,\, F_{\max}.\slot,\, \mathsf{hash}(F_{\max}))$ from Remark~\ref{rem:fs-invariant} and Lemma~\ref{lem:just-unique-height} force $C = F_{\max}$.
...
Unless $\geq n/3$ validators are slashable: two nodes that have processed the same set of blocks have the same $(\sigma,\, \T,\, F_s,\, R_s,\, h_R,\, \mathsf{bit}_R)$.
\end{corollary}
...
The argument reads $\Sigma.R_s$ directly as the running-max cert event, bounds $h_c$ in terms of the cached cert-key height $\Sigma.h_R$, and handles the locked branch of $T_c$ by matching its target against $R_s$.

\emph{Step 1: $R_s \preceq X$ and $h_c \geq h_R$.}
$X$ descends from~$R_s$ (Figure~\ref{alg:store}), so $R_s \preceq X$.
$R_s$ is either the genesis block (with $R_s = \genesis$, $h_R = 0$) or was set by a cert event firing at height $h_R$ on some processed chain; in the latter case Remark~\ref{rem:fresh-equiv} gives $\sigma[R_s].h = h_R$, and in the former trivially $\sigma[R_s].h = 0 = h_R$.
...
If some such event had $h' \geq h_R + 1$, its key's leading coordinate would beat $h_R$, so $\textsf{updateForkChoiceRoot}$ would have raised $h_R$ to at least $h' \geq h_R + 1$, contradicting the current value of $h_R$.
...
That event was passed through $\textsf{updateForkChoiceRoot}$ and raised the cached cert key $(h_R,\, \mathsf{bit}_R,\, R_s.\slot,\, \mathsf{hash}(R_s))$ to at least $(h',\, 1,\, J_\ast.\slot,\, \mathsf{hash}(J_\ast))$; by Lemma~\ref{lem:Rs-key-monotone} this bound persists.
So a lock at height $h'$ implies the current cached key $\geq (h',\, 1,\, J_\ast.\slot,\, \mathsf{hash}(J_\ast))$, and in particular $h_R \geq h'$, with $\mathsf{bit}_R = 1$ whenever $h_R = h'$.
...
The lock characterization gives $(h_R,\, \mathsf{bit}_R,\, R_s.\slot,\, \mathsf{hash}(R_s)) \geq (h_R, 1, L.\slot, \mathsf{hash}(L))$; with the leading coordinate matching, this forces $\mathsf{bit}_R = 1$.
