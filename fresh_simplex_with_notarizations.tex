...
The pair $(\mathsf{finalize\_height},\, \mathsf{finalize\_target})$ is either both~$\bot$ or a commitment to finalize a block at a height.
...
\begin{itemize}
  ...
  \item $\mathsf{jtargets}[1..n]$: for each validator~$i$, the target of~$i$'s $\just$ vote at the current height ($\bot$ if none processed).
  \item $\mathsf{ntargets}[1..n]$: for each~$i$, the $\preceq$-max target among~$i$'s fresh votes at the current height, across both kinds ($\bot$ if none processed). A $\just$ vote for~$T$ thus also contributes to prefix-notarizing~$T$ and its ancestors.
  ...
  \item $P \subseteq \{1, \ldots, n\}$: validators who have cast a vote with $\mathsf{finalize\_height} = h_j$ and $\mathsf{finalize\_target} = J$.
\end{itemize}
...
On a chain with head state $\sigma[L]$ at height~$h$, walk from~$L$ toward genesis summing $|\{i : \mathsf{ntargets}[i] = B\}|$ at each~$B$.
...
\[
  \bigl|\{i : T \preceq \mathsf{ntargets}[i] \preceq L\}\bigr| \;\geq\; 2n/3.
\]
...
\[
  b.\mathsf{finalize\_target} \neq \bot \;\wedge\; a.\height = b.\mathsf{finalize\_height} \;\wedge\; a.\target \neq b.\mathsf{finalize\_target}.
\]
That is, a validator committing $(\mathsf{finalize\_height},\, \mathsf{finalize\_target}) = (h_f, T_f)$ must not sign any vote at height~$h_f$ with target $\neq T_f$.
\end{definition}
...
The state machine (Figure~\ref{alg:state-machine}) exposes two per-chain routines: $\textsf{processSlot}$, invoked once per slot on every maintained chain, and $\textsf{processBlock}$, invoked additionally whenever a block~$B$ is processed.

$\textsf{processSlot}$ increments~$s$ and calls $\textsf{processHeight}$, which in turn checks three branches in order: (i) finality, $F \gets J$ if $|P| \geq 2n/3$; (ii) justification, if some~$T$ has $\geq 2n/3$ entries in $\mathsf{jtargets}$; (iii) prefix notarization, at the firing block~$T^*$ located by the walk of Definition~\ref{def:notarization}.
Branches (ii) and (iii) both advance height and reset $\mathsf{jtargets}, \mathsf{ntargets}$, so at most one height-advance fires per invocation.

$\textsf{processBlock}(\sigma, B)$ has precondition $\sigma.s = B.\slot$: the caller runs $\textsf{processSlot}$ up to $B.\slot$ first.
Then $L \gets B$, and each vote~$v$ in~$B$ is fed through $\textsf{processVote}$.
A fresh vote updates $\mathsf{ntargets}[i] \gets \max_{\preceq}(\mathsf{ntargets}[i],\, v.\target)$ (well-defined as both arguments lie on~$L$ at height $\sigma.h$, treating~$\bot$ as~$\genesis$); if also $v.\kind = \just$, then $\mathsf{jtargets}[i] \gets v.\target$.
The $P$-gate adds~$i$ to~$P$ iff $v.\mathsf{finalize\_height} = \sigma.h_j$ and $v.\mathsf{finalize\_target} = \sigma.J$; this is independent of freshness.
$\textsf{processBlock}$ never advances~$h$: height advances happen only in $\textsf{processSlot}$, which runs on every slot (including empty ones).
...
\begin{proof}
Reading off Figure~\ref{alg:state-machine}: $s$ only increases (in $\textsf{processSlot}$); $h$ only increases, by~$1$, in the height-advance branches of $\textsf{processHeight}$, which also set $h_j, h_n \gets h$ on their respective updates and $s_h \gets L.\slot$.
...
At a height-advance on chain~$Y$ firing at target~$T$ with pre-state height~$h$, freshness forces $T.\slot \geq s_h$: directly for justification, since every contributing $\just$ vote has target~$T$; for prefix notarization, since the firing~$T$ is some $\mathsf{ntargets}[i]$ value, hence has slot $\geq s_h$.
...
\begin{proof}
By Lemma~\ref{lem:heightprog}, some $B^* \preceq B$ advanced height from~$h$ to~$h+1$ via a quorum~$Q$ at height~$h$ on~$B^*$'s chain; each $i \in Q$ contributed a fresh vote~$a_i$ at height~$h$ (freshness is automatic for $\just$ votes; prefix-notarization entries in $\mathsf{ntargets}$ are also fresh by Figure~\ref{alg:state-machine}).
Finality of~$C$ at~$h$ gives a quorum~$Q_F$ each signing a vote~$b$ with $b.\mathsf{finalize\_height} = h$ and $b.\mathsf{finalize\_target} = C$.

By quorum intersection, $|Q \cap Q_F| \geq n/3$; pick $v \in Q \cap Q_F$ not slashable.
E1 (Definition~\ref{def:slashing}) applied to $v$'s votes $a \in Q$, $b \in Q_F$ with $a.\height = h = b.\mathsf{finalize\_height}$ forces $a.\target = C$; freshness gives $C = a.\target \preceq B^* \preceq B$.
\end{proof}
...
\begin{proof}
\emph{Case $h = h'$.}
Finality of~$C$ gives a quorum~$Q_F$ with $\mathsf{finalize\_height} = h$, $\mathsf{finalize\_target} = C$.
...
Because $\just$ votes bump $\mathsf{ntargets}$, a prefix notarization at height~$h$ on a chain containing a finalized~$C$ may fire at an ancestor $T^* \preceq C$.
...
It maintains a single \emph{root block} $R_s$ together with a cached \emph{cert key} $\mathsf{key}_R$ for the event that last set $R_s$.
Each firing of $\textsf{processHeight}$ (a \emph{cert event}) is compared against $\mathsf{key}_R$ in lex order; whenever its key strictly dominates, $(R_s, \mathsf{key}_R)$ is updated to the new event.
...
When $\textsf{processHeight}$ fires a height advance on a chain (Figure~\ref{alg:state-machine}), we call the triggering event a \emph{cert event}, characterized by:
\begin{itemize}
...
A justification also prefix-notarizes its block (the justification branch sets $(J, N) \gets (T, T)$ jointly), but we classify the combined event as a justification with key $\mathsf{key}(T, h, 1)$.
...
\begin{itemize}
  ...
  \item $\mathsf{key}_R$ is the cached cert key of the event that set $R_s$.
\end{itemize}
The genesis store has $\T = \{\genesis\}$, $F_s = R_s = \genesis$, and $\mathsf{key}_R = (0,\, 1,\, \genesis.\slot,\, \mathsf{hash}(\genesis))$, treating genesis as a bit-$1$ cert event at height~$0$.
\end{definition}

The capital~$\Sigma$ distinguishes the store from the per-chain state~$\sigma$.
We write $\Sigma.\sigma$, $\Sigma.\T$, $\Sigma.F_s$, $\Sigma.R_s$, $\Sigma.\mathsf{key}_R$ for field access, and $\Sigma.\mathsf{key}_R.h$, $\Sigma.\mathsf{key}_R.b$ for the height and justification-bit components of the cached key.

The acceptance routine $\textsf{onBlock}$ (Figure~\ref{alg:store}) admits $B$ only if $B.\parent \in \Sigma.\T$ and $\Sigma.F_s \preceq B$.
It runs $\textsf{processSlot}$ once per slot from $\Sigma.\sigma[B.\parent].s + 1$ up to $B.\slot$ (so any height advance enabled by the accumulated tally fires before~$B$'s votes are folded in), then $\textsf{processBlock}(\cdot, B)$; the resulting post-state becomes $\Sigma.\sigma[B]$.
The post-state then offers two cert-event descriptors to $\textsf{updateForkChoiceRoot}$: the justified checkpoint $(\sigma[B].h_j,\, 1,\, \sigma[B].J)$ and the notarized checkpoint $(\sigma[B].h_n,\, 0,\, \sigma[B].N)$; the bit-$1$ / bit-$0$ split reflects the distinct events (the justification may have fired at a strictly smaller height than the notarization).
Each call to $\textsf{updateForkChoiceRoot}$ first filters out any candidate~$C$ with $C \not\succeq \Sigma.F_s$ and then updates $(\Sigma.R_s,\, \Sigma.\mathsf{key}_R)$ iff the candidate's cert key strictly exceeds the current $\mathsf{key}_R$; this maintains $R_s$ incrementally as the lex-max cert event ever observed on a chain extending the current $F_s$, never recomputed.
Finally, $\textsf{updateFinalized}$ sets $\Sigma.F_s \gets \sigma[B].F$ iff $\sigma[B].F \succ \Sigma.F_s$ and $\sigma[B].F \preceq \Sigma.R_s$.
...
\Function{$\textsf{updateForkChoiceRoot}$}{$\Sigma,\; (h,\, b,\, C)$} \Comment{Filter by $\Sigma.F_s \preceq C$, then running max}
  ...
  \If{$\mathsf{newKey} > \Sigma.\mathsf{key}_R$ and $\Sigma.F_s \preceq C$}
...
\begin{proof}
$F_s$ is updated only by $\textsf{updateFinalized}$, whose guard $F_B \succ \Sigma.F_s$ forces strict extension along~$\preceq$.
\end{proof}
...
\emph{$\textsf{updateForkChoiceRoot}$.}
...
\begin{proof}
Fix the moment when $\textsf{onBlock}(B)$ offered the descriptor $(h_F,\, 1,\, F)$ to $\textsf{updateForkChoiceRoot}$, and let $F_s^0 = \Sigma.F_s$ at that moment.
...
The filter passes, so after the call $\Sigma.\mathsf{key}_R \geq (h_F,\, 1,\, F.\slot,\, \mathsf{hash}(F))$; Lemma~\ref{lem:Rs-key-monotone} preserves this bound thereafter.
Write $h_R \coloneqq \Sigma.\mathsf{key}_R.h$ and $\mathsf{bit}_R \coloneqq \Sigma.\mathsf{key}_R.b$; the bound gives $h_R \geq h_F$.
If $h_R > h_F$, Lemma~\ref{lem:certchain} applied to the cert event $(R_s,\, h_R,\, \mathsf{bit}_R)$ gives $F \prec R_s$.
If $h_R = h_F$, the bound forces $\mathsf{bit}_R = 1$, so $R_s$ was justified at height~$h_F$ on some processed chain, giving a $\just$-quorum~$Q'$ of size $\geq 2n/3$ with target~$R_s$ at height~$h_F$.
Finality of~$F$ at~$h_F$ gives a quorum~$Q_F$ with $\mathsf{finalize\_height} = h_F$ and $\mathsf{finalize\_target} = F$.
By quorum intersection, $|Q' \cap Q_F| \geq n/3$; pick $v \in Q' \cap Q_F$ not slashable.
...
$F_B$ being finalized in~$B$'s post-state requires some ancestor~$B' \preceq B$ to have post-state $(J_{B'},\, h_j^{B'}) = (F_B,\, h_{F_B})$; $B'$ was processed before~$B$ (parent-first), so Lemma~\ref{lem:upgrade} gives $R_s \succeq F_B$, and the $\textsf{updateFinalized}$ guard passes.
\end{proof}
...
\emph{$R_s$.} The multiset~$D$ of descriptors offered to $\textsf{updateForkChoiceRoot}$ is a deterministic function of the processed set: each block contributes its post-state's two descriptors exactly once.
Write $K_{\max} \coloneqq (h_{F_{\max}},\, 1,\, F_{\max}.\slot,\, \mathsf{hash}(F_{\max}))$.
For any $(h, b, C) \in D$ with $h \geq h_{F_{\max}}$, Lemma~\ref{lem:certchain} gives $C \succeq F_{\max}$ (strict when $h > h_{F_{\max}}$); at $h = h_{F_{\max}}$, Remark~\ref{rem:fs-invariant} with $F_{\max}$ justified gives $\Sigma.\mathsf{key}_R \geq K_{\max}$, forcing any competitive descriptor to have bit~$1$, hence a $\just$-quorum~$Q_C$ of size $\geq 2n/3$ targeting~$C$ at height~$h_{F_{\max}}$, while finality of~$F_{\max}$ gives a quorum~$Q_F$ with $\mathsf{finalize\_height} = h_{F_{\max}}$ and $\mathsf{finalize\_target} = F_{\max}$; quorum intersection and E1 (as in Lemma~\ref{lem:upgrade}) force $C = F_{\max}$.
Descriptors with $h < h_{F_{\max}}$ have key strictly below $K_{\max}$ on the leading coordinate, so once $\mathsf{key}_R \geq K_{\max}$ holds they are dominated.
...
This section extends the per-chain state machine of Figure~\ref{alg:state-machine} with a penalty counter and a new transition function $\textsf{processInactivityPenalties}$, invoked each slot by an extended $\textsf{processSlot}$ (Figure~\ref{alg:leak-processslot}); $\textsf{processBlock}$, $\textsf{processHeight}$, and $\textsf{processVote}$ are unchanged.
...
Augment the per-chain state tuple with one non-negative counter $\sigma.\mathsf{penalties}[1..n]$, initialized to~$0$ at genesis; no other field is added or modified.
Figure~\ref{alg:leak-processslot} replaces $\textsf{processSlot}$ from Figure~\ref{alg:state-machine} with a version that snapshots the pre-advance state and invokes $\textsf{processInactivityPenalties}$ after $\textsf{processHeight}$.
...
$\textsf{processInactivityPenalties}$ checks three independent guards; any subset may fire at a given slot.
...
\emph{Layer~2 target.} Fires when the justification branch of $\textsf{processHeight}$ did not fire (else $J$ would strictly advance by Lemma~\ref{lem:slotmono}).
$T^*$ is the plurality target across $\sigma_{\mathrm{pre}}.\mathsf{jtargets}$ (deterministic tiebreak), or $\bot$ if no $\just$ votes have been registered; the second disjunct of the guard bumps non-voters when $T^* = \bot$ (in which case all~$n$ validators are bumped).
Since no $2n/3$-quorum on a single target formed, the plurality count is $< 2n/3$, so $|\{i : \sigma_{\mathrm{pre}}.\mathsf{jtargets}[i] \neq T^* \vee \sigma_{\mathrm{pre}}.\mathsf{jtargets}[i] = \bot\}| > n/3$.
...
The finality branch of $\textsf{processHeight}$, when it fires, sets $\sigma_{\mathrm{post}}.F = \sigma_{\mathrm{pre}}.J \succ \sigma_{\mathrm{pre}}.F$, so this guard is skipped automatically.

\paragraph{Canonical-freshness.}
A vote~$v$ processed at the current slot on canonical chain~$X$ at state-height $h_c = \sigma_{\mathrm{post}}.h$ bumps $\sigma.\mathsf{ntargets}[v.\validator]$ iff $v$ is \emph{canonical-fresh}:
\[
...
Fix a chain and write $\sigma_{\mathrm{pre}}^{(t)}, \sigma_{\mathrm{post}}^{(t)}$ for its pre/post-$\textsf{processHeight}$ snapshots at slot~$t$.
...
The finality branch of $\textsf{processHeight}$ did not fire (else $F$ would advance), so $|\sigma_{\mathrm{pre}}^{(t)}.P| < 2n/3$ and $|\{i : i \notin \sigma_{\mathrm{pre}}^{(t)}.P\}| > n/3$.

\emph{Stable-stuck.} The Layer~2 target guard fires at~$t$.
The justification branch did not fire, so the plurality count on $\sigma_{\mathrm{pre}}^{(t)}.\mathsf{jtargets}$ is $< 2n/3$ (counting $T^* = \bot$ as~$0$); hence the bumped set has size $> n/3$.
...
Let $V$ be the set of votes $i$ has previously cast, and write $\mathsf{currentSlot}$ for the node's protocol time (a slot counter shared across chains).
At each voting opportunity, $i$ constructs its vote by $\textsf{constructVote}(V,\, \Sigma)$, defined in Figure~\ref{alg:voting-rule}.
...
\Function{$\textsf{constructVote}$}{$V,\; \Sigma$}
  \State $X \gets \textsf{getHead}(\Sigma)$
  \State $h_c \gets \sigma[X].h$; \;\; $h_j \gets \sigma[X].h_j$; \;\; $J \gets \sigma[X].J$
  \vspace{3pt}
  \State \textbf{-- Target selection --}
  \If{$\exists\, v \in V$ with $v.\mathsf{finalize\_height} = h_c$ and $v.\mathsf{finalize\_target} \neq \bot$}
    \State $T_c \gets v.\mathsf{finalize\_target}$ \Comment{$i$ is locked at $h_c$}
  \Else
  ...
  \If{$\not \exists v \in V$ s.t. $v.\height = h_c$} \Comment{first vote at $h_c$: cast $\just$ (R1)}
    ...
    \EndIf
    \State \Return $(i,\; h_c,\; \mathsf{currentSlot},\; T_c,\; \just,\; f_h,\; f_t)$
  \Else \Comment{already voted at $h_c$: cast $\notar$ (R2)}
    \State \Return $(i,\; h_c,\; \mathsf{currentSlot},\; T_c,\; \notar,\; \bot,\; \bot)$
  \EndIf
\EndFunction
...
Fairness is local to a single honest validator~$i$: we fix~$i$, its local store~$\Sigma$, and the canonical chain $X = \textsf{getHead}(\Sigma)$.
...
Height advances reset $\mathsf{jtargets}$ and $\mathsf{ntargets}$ (Figure~\ref{alg:state-machine}), so Layer-2 state at the next height is independent of~$i$'s prior votes and no ``lock'' issue propagates.
...
Unless $\geq n/3$ validators are slashable: if $\Sigma.\mathsf{key}_R.b = 0$, then no justification at height $\Sigma.\mathsf{key}_R.h$ has fired on any processed chain.
\end{lemma}

\begin{proof}
Write $h_R \coloneqq \Sigma.\mathsf{key}_R.h$ and suppose some justification at~$h_R$ fired on a processed chain, producing $J_\ast$ justified at~$h_R$.
$\textsf{onBlock}$ offered the descriptor $(h_R,\, 1,\, J_\ast)$ to $\textsf{updateForkChoiceRoot}$ at that time, with key $(h_R,\, 1,\, J_\ast.\slot,\, \mathsf{hash}(J_\ast))$.
...
When $F_s^0$ was set, the cert event $(F_s^0,\, h_{F_s^0},\, 1)$ had already raised the cache height to $\geq h_{F_s^0}$ (the $\textsf{updateFinalized}$ guard $F_B \preceq R_s$ ensures the cache covered it), and Lemma~\ref{lem:Rs-key-monotone} preserves $h_{F_s^0} \leq h_R$.
We show $F_s^0 \preceq J_\ast$, so the filter in $\textsf{updateForkChoiceRoot}$ passes.
If $h_R > h_{F_s^0}$, Lemma~\ref{lem:certchain} applied to $F_s^0$ and the cert event at $J_\ast$ gives $F_s^0 \prec J_\ast$.
If $h_R = h_{F_s^0}$, finality of $F_s^0$ at~$h_R$ gives a finalize-quorum $Q_F$ with $\mathsf{finalize\_height} = h_R$ and $\mathsf{finalize\_target} = F_s^0$, and $J_\ast$ being justified at~$h_R$ gives a $\just$-quorum~$Q'$ targeting~$J_\ast$ at~$h_R$; quorum intersection and E1 (as in Lemma~\ref{lem:upgrade}) force $F_s^0 = J_\ast$.
Either way $F_s^0 \preceq J_\ast$, so $\textsf{updateForkChoiceRoot}$ raises the cache to $\Sigma.\mathsf{key}_R \geq (h_R,\, 1,\, J_\ast.\slot,\, \mathsf{hash}(J_\ast))$ at that moment; Lemma~\ref{lem:Rs-key-monotone} preserves this bound.
At the present moment $\Sigma.\mathsf{key}_R.h = h_R$, so lex-comparison with the first coordinate fixed forces $\Sigma.\mathsf{key}_R.b \geq 1$, contradicting $\mathsf{key}_R.b = 0$.
\end{proof}

\begin{lemma}[Finalize vote implies justification]\label{lem:no-finalize-without-just}
If~$i$ has cast a vote~$v$ with $v.\mathsf{finalize\_height} = h'$ and $v.\mathsf{finalize\_target} = T_f \neq \bot$, then~$T_f$ was justified at height~$h'$ on some processed chain by the time~$v$ was cast.
\end{lemma}

\begin{proof}
Only (R1) produces $\mathsf{finalize\_height} \neq \bot$, so~$v$ came from (R1) at an earlier moment with~$i$'s local store~$\Sigma_0$; by the (R1) finalize-field conditional, $(f_h, f_t) = (h_j, J) = (h', T_f)$ read from~$i$'s then-canonical $X_0 = \textsf{getHead}(\Sigma_0)$, so $\sigma[X_0].h_j = h'$ and $\sigma[X_0].J = T_f$.
A state has $(h_j, J) = (h', T_f)$ only after the justification branch of $\textsf{processHeight}$ fired on that chain with target~$T_f$ at pre-state height~$h'$, i.e.\ some ancestor $B' \preceq X_0$ triggered justification of~$T_f$ at height~$h'$.
\end{proof}

\begin{lemma}[Canonical-fresh honest vote]\label{lem:canonical-fresh-honest-vote}
Unless $\geq n/3$ validators are slashable, and assuming $f < n/3$ and all honest validators follow Definition~\ref{def:voting-rule}: let~$i$ be honest with local store~$\Sigma$, $X = \textsf{getHead}(\Sigma)$, and $h_c = \sigma[X].h$.
...
Every state-height advance on~$X$ fires a cert event whose descriptor is offered to $\textsf{updateForkChoiceRoot}$, raising $\mathsf{key}_R.h$ if it exceeds the cached value; hence no advance from $h_R + 1$ to $h_R + 2$ has fired on~$X$, so $h_c \in \{h_R,\, h_R + 1\}$.
By Definition~\ref{def:voting-rule}, $v.\height = h_c$ and $v.\target = T_c$, where $T_c = T \preceq X$ with $\sigma[T].h = h_c$ in the unlocked case, and $T_c = L$ where~$L$ is a lock target from a prior (R1) vote of~$i$ with $\mathsf{finalize\_height} = h_c$ in the locked case.

\emph{Case $h_c = h_R + 1$.}
A lock at~$h_c$ would, by Lemma~\ref{lem:no-finalize-without-just}, require some~$J_\ast$ justified at~$h_c$ on a processed chain.
The filter-passing argument of Lemma~\ref{lem:root-h-notarize-only} applies (with $h_R$ there replaced by~$h_c$, using $h_{F_s^0} \leq h_R < h_c$), so $\textsf{updateForkChoiceRoot}$ raised the cache to $\Sigma.\mathsf{key}_R.h \geq h_c = h_R + 1$ and Lemma~\ref{lem:Rs-key-monotone} preserves this, contradicting $\mathsf{key}_R.h = h_R$.
So~$i$ is unlocked at~$h_c$ and $T_c = T \preceq X$ with $\sigma[T].h = h_c$.

\emph{Case $h_c = h_R$ with $\mathsf{bit}_R = 0$.}
...
\emph{Case $h_c = h_R$ with $\mathsf{bit}_R = 1$.}
If~$i$ is unlocked, $T_c = T \preceq X$ directly.
Otherwise~$i$ is locked at~$h_c$ with target~$L$; Lemma~\ref{lem:no-finalize-without-just} gives $L$ justified at~$h_c$ on some processed chain, and $\mathsf{bit}_R = 1$ gives $R_s$ justified at~$h_R = h_c$ on some processed chain.
...
Unless $\geq n/3$ validators are slashable, and assuming $f < n/3$ and all honest validators follow Definition~\ref{def:voting-rule}: let~$i$ be an honest validator, $\Sigma$ its local store, $X = \textsf{getHead}(\Sigma)$ its canonical chain, and $h_c = \Sigma.\sigma[X].h$ the current canonical state-height.
Let $v$ be the vote~$i$ produces at the current slot.
...
So~$v$ is canonical-fresh on~$X'$, and processing~$v$ sets $\mathsf{ntargets}[i] \neq \bot$.
$\mathsf{ntargets}$ resets only at a height advance, which does not fire while state-height stays at~$h_c$, so $\mathsf{ntargets}[i] \neq \bot$ persists and Layer~1 does not bump~$i$.
\end{proof}

The $f < n/3$ hypothesis is used only in Lemma~\ref{lem:canonical-fresh-honest-vote} via Lemma~\ref{lem:just-unique-height}, to collapse the lock target~$L$ onto $R_s$ in the $h_c = h_R$, $\mathsf{bit}_R = 1$ case.
...
\begin{enumerate}
  ...
  \item no honest validator has cast a $\just$ vote at height $> R_s.h$ at any time strictly before $\mathsf{vote}(t+1)$.
\end{enumerate}
\end{lemma}
...
\begin{enumerate}
  ...
  \item $(R_s,\, R_s.h,\, R_s.\slot) \in C_J$;
  \item no honest validator has cast a $\just$ vote at height $> R_s.h$ at any time strictly before $\mathsf{vote}(t+2)$.
\end{enumerate}
\end{lemma}
...
\begin{enumerate}
  ...
  \item $\textsf{getHead}$ is deterministic;
  \item $\mathsf{start}(t) - \Delta \geq \GST$;
  \item $\mathsf{syncer}(t)$, $\mathsf{syncer}(t+1)$, and $\mathsf{syncer}(t+2)$ are honest; let $\Sigma^\star_t, \Sigma^\star_{t+1}, \Sigma^\star_{t+2}$ be the common stores, $R_s^t \coloneqq \Sigma^\star_t.R_s$ and $R_s^{t+1} \coloneqq \Sigma^\star_{t+1}.R_s$ the common roots at $\mathsf{vote}(t)$ and $\mathsf{vote}(t+1)$ respectively, $H \coloneqq \textsf{getHead}(\Sigma^\star_t)$, and $H^{t+1} \coloneqq \textsf{getHead}(\Sigma^\star_{t+1})$;
  \item $H.h = R_s^t.h$;
  % \item no honest has cast any vote at height $> R_s^t.h$ at any time strictly before $\mathsf{vote}(t)$;
  \item at $\mathsf{start}(t+1)$, $\textsf{getHead}$ applied to $\mathsf{syncer}(t+1)$'s store returns a chain $Y \succeq H$ such that every vote cast at $\mathsf{vote}(t)$ by an honest validator with target $\preceq H$ is included in~$Y$;
  \item at $\mathsf{start}(t+2)$, $\textsf{getHead}$ applied to $\mathsf{syncer}(t+2)$'s store returns a chain $X \succeq H^{t+1}$ such that every vote cast at $\mathsf{vote}(t+1)$ by an honest validator with target $\preceq H^{t+1}$ is included in~$X$;
  \item by $\mathsf{start}(t+3) - \Delta$, in every honest validator's view, there exists a chain $X' \succeq X$ such that every vote cast at $\mathsf{vote}(t+2)$ by an honest validator with target $\preceq H^{(t+2)}$ is included in~$X'$, where $H^{(t+2)} \coloneqq \textsf{getHead}(\Sigma^\star_{t+2})$.
\end{enumerate}
Then at $\mathsf{vote}(t+3)$, every honest validator has:
\begin{enumerate}
  \item the same $R_s$, with $R_s.h = H^{t+1}.h + 1$;
  \item $(R_s,\, R_s.h,\, R_s.\slot) \in C_J$;
  \item no honest validator has cast a $\just$ vote at height $> R_s.h$ at any time strictly before $\mathsf{vote}(t+3)$.
\end{enumerate}
\end{lemma}

\begin{proof}
We invoke \Cref{rev:lem:healing-advance} after renaming its slot parameter $t$ to $t+1$, so that its $H$ becomes our $H^{t+1}$, its common stores become $\Sigma^\star_{t+1}, \Sigma^\star_{t+2}$, and its conclusion at $\mathsf{vote}(t+2)$ becomes ours at $\mathsf{vote}(t+3)$.
Under this renaming, hypotheses (1)--(4) and (7)--(8) of the present lemma map to hypotheses~(1)--(4), (6), (7) of \Cref{rev:lem:healing-advance} directly (hypothesis~(3) via $\mathsf{start}(t+1) \geq \mathsf{start}(t) + 2\Delta$).
It remains to establish the renamed hypothesis~(5): no honest has cast a vote at height $> H^{t+1}.h$ strictly before $\mathsf{vote}(t+1)$.
We do this in two steps: Step~1 bounds all honest vote heights by $R_s^t.h + 1$, and Step~2 shows $H^{t+1}.h = R_s^t.h + 1$.

\emph{Step 1: no honest has cast a vote at height $> R_s^t.h + 1$ strictly before $\mathsf{vote}(t+1)$.}
By \Cref{rev:ass:view-merge} + honest $\mathsf{syncer}(t)$ + determinism, every honest at $\mathsf{vote}(t)$ shares $\Sigma^\star_t$ and hence $R_s^t$, so \Cref{rev:lem:Rs-height-monotone} gives $R_s.h \leq R_s^t.h$ in every honest view at every time $\tau \leq \mathsf{vote}(t)$.
...
Therefore no honest vote cast at $\tau \leq \mathsf{vote}(t)$ has height $> R_s^t.h + 1$, and since no voting event occurs in $(\mathsf{vote}(t), \mathsf{vote}(t+1))$, the claim extends to all of $\tau < \mathsf{vote}(t+1)$.

\emph{Step 2: $H^{t+1}.h = R_s^t.h + 1$.}
By hypothesis~(5), $h_c^{(t)} = H.h = R_s^t.h$ in every honest view at $\mathsf{vote}(t)$.
\Cref{rev:lem:canonical-fresh-honest-vote} then makes every honest slot-$t$ vote (either kind) target a common $T \preceq H$; both kinds update $\mathsf{ntargets}[i]$, so on~$Y$ the prefix walk at state-height $R_s^t.h$ accumulates $\geq 2n/3$ to some $B^\star \succeq T$, firing prefix notarization and inserting $(B^\star, R_s^t.h, B^\star.\slot) \in C_N$.
By \Cref{rev:ass:view-merge} + honest $\mathsf{syncer}(t+1)$ + determinism, $\Sigma^\star_{t+1} = \Sigma_{\mathsf{syncer}(t+1)}$ at $\mathsf{start}(t+1)$ and its canonical is~$Y$ (hypothesis~(6)), hence $\sigma[H^{t+1}].h = R_s^t.h + 1$.

Steps~1 and~2 together give ``no honest has cast a vote at height $> H^{t+1}.h$ strictly before $\mathsf{vote}(t+1)$'', which is the renamed hypothesis~(5).
\Cref{rev:lem:healing-advance} therefore applies and yields exactly the claimed conclusion at $\mathsf{vote}(t+3)$.
\end{proof}
