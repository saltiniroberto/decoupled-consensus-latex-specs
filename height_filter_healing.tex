...
\newcommand{\timeoutParticipation}{\mathsf{timeouts}}
...
set~\(P\) contains validators whose finality fields were accepted while this
pair was the latest unfinalized justification.  After it is finalized, the
set is ignored.  A newer justification replaces \((h_j,J)\) and resets~\(P\).
...
Figure~\ref{alg:state-machine} gives the same rules as pseudocode.  The round
schedule supplies the paper-only Boolean \(\mathit{close}\) to
\(\mathsf{process\_block}\); it is true only for the block transition that
records the end-of-round result.  All timely attestations for that close have
already been included.  The remaining function and field names follow the
...
\(\mathsf{has\_timeout\_quorum}\) implement the checks stated above.  The
abstract model uses the deciding block as the next height's target.  Calling
the close rule after each block is useful as a safety-proof interpretation,
but the round bounds in this paper use exactly one scheduled close per round.
...
\Statex \hspace{1em}\(L\gets\mathsf{genesis},\ s\gets0,\ h\gets1,\
  \currentHeightStartSlot\gets0,\ \currentHeightTarget\gets\mathsf{genesis}\)
\Statex \hspace{1em}\(\currentHeightNonjustifiable\gets\mathsf{false},\
  \targetParticipation\gets\mathsf{false}^{V_1},\
  \timeoutParticipation\gets\mathsf{false}^{V_1}\)
\Statex \hspace{1em}\((h_j,J)\gets(0,\mathsf{genesis}),\
  (h_F,F)\gets(0,\mathsf{genesis}),\ P\gets\varnothing\)
\vspace{3pt}

\Function{process\_block}{$\sigma,B,\mathit{close}$}
  \State \textbf{assert} \(B.\mathsf{parent}=\sigma.L\) and
    \(B.\mathsf{slot}>\sigma.s\)
  \State \(\sigma.L\gets B;\quad \sigma.s\gets B.\mathsf{slot}\)
...
\Function{process\_attestation}{$\sigma,a$}
  ...
  \ForAll{\(i\in\mathsf{get\_attesting\_indices}(\sigma,a)\)}
    \If{\(i\in V_{\sigma.h_j}\), \(\sigma.h_j>\sigma.h_F\),
        \(\sigma.F\preceq\sigma.J\),
        \(d.\mathsf{finality\_height}=\sigma.h_j\), and
        \(d.\mathsf{finality\_target}=\sigma.J\)}
      \State \(\sigma.P\gets\sigma.P\cup\{i\}\)
    \EndIf
    \If{\(i\in V_{\sigma.h}\) and \(\mathit{viable}\)}
      \State \(\sigma.\timeoutParticipation[i]\gets\mathsf{true}\)
      \If{\(\neg\mathit{timeout}\)}
...
\Function{process\_justification\_and\_finalization}{$\sigma$}
  \If{\(\mathsf{has\_new\_finalization}(\sigma)\)}
    \State \((\sigma.h_F,\sigma.F)\gets(\sigma.h_j,\sigma.J)\)
  \EndIf
...
\Function{advance\_height}{$\sigma,\mathit{justify\_target}=\bot$}
  \If{\(\mathit{justify\_target}\neq\bot\)}
    \State \((\sigma.h_j,\sigma.J)\gets
      (\sigma.h,\mathit{justify\_target});\quad \sigma.P\gets\varnothing\)
  \EndIf
  \State \(\sigma.h\gets\sigma.h+1\)
  \State \(\sigma.\currentHeightStartSlot\gets\sigma.s;\quad
    \sigma.\currentHeightTarget\gets\sigma.L\)
  \State \(\sigma.\currentHeightNonjustifiable\gets
    (\sigma.h-\sigma.h_F>D_{\rm debt})\mathbin{\wedge}(K\mid\sigma.h)\)
  \State \(\sigma.\targetParticipation\gets\mathsf{false}^{V_{\sigma.h}}\)
  \State \(\sigma.\timeoutParticipation\gets\mathsf{false}^{V_{\sigma.h}}\)
  \State \Return \(\sigma\)
\EndFunction
...
validator weights for relevant heights, the
...
\Statex \hspace{1em}\(S.\mathsf{blocks}\gets\{\mathsf{genesis}\},\
...
\Function{derive\_block\_states}{$S$}
  \State \(\sigma[\mathsf{genesis}]\gets\sigma_{\rm genesis};\quad
    \mathcal A\gets\{\mathsf{genesis}\}\)
  \Repeat
    \State \(\mathit{changed}\gets\mathsf{false}\)
    \ForAll{\(\mathit{signed\_block}\in S.\mathsf{blocks}\) whose message
             \(B\notin\mathcal A\) has \(B.\mathsf{parent}\in\mathcal A\)}
      \State \(\sigma'\gets\Call{process\_block}{
        \sigma[B.\mathsf{parent}],B,\mathsf{closes\_round}(B)}\)
...
\Function{complete\_step}{$S,I$}
  ...
  \State \(X\gets\{x:\mathsf{timely}(x,I)\in S.\mathsf{timely}\text{ and every
    dependency }z\text{ has }\mathsf{timely}(z,I)\in S.\mathsf{timely}\}\)
  \State \(D\gets\) the completed records used by step \(I\)
  \State \(y\gets\mathsf{execute\_step}(I,X,D)\)
  \State \(S.\mathsf{completed\_steps}\gets
    S.\mathsf{completed\_steps}\cup\{(I,X,D,y)\}\)
  \State \Return \(S\)
\EndFunction
\vspace{2pt}
...
\Function{derive\_fork\_choice}{$S$}
  ...
  \State \((h_F,F)\gets\) greatest compatible finalized pair in
    \(\{(\sigma[B].h_F,\sigma[B].F):B\in\mathcal A\}\)
  \State \(\mathcal T\gets\{B\in\mathcal A:F\preceq B\}\)
  \State \(\mathcal J\gets
    \{(\sigma[B].h_j,\sigma[B].J):B\in\mathcal T,\ F\preceq\sigma[B].J\}\)
  \State \((h_j,J)\gets\arg\max_{(h,C)\in\mathcal J}(h,\mathsf{hash}(C))\)
  \State \(h_{\max}\gets\max\{\sigma[B].h:B\in\mathcal T\}\)
  \State \(\mathcal T'\gets\{B\in\mathcal T:\exists\text{ leaf }L\in\mathcal T,
    B\preceq L\text{ and }\sigma[L].h\geq h_{\max}-1\}\)
  \State \(R\gets J\) if \(h_{\max}=h_j+1\), else \(R\gets F\)
  \State \Return \(\Sigma=(\sigma,\mathcal T,h_F,F,h_j,J,h_{\max},\mathcal T',R)\)
\EndFunction
\vspace{2pt}
...
current height and finalized state.  Figure~\ref{alg:state-machine} sets the
new target to the block that performs the abstract height transition.  An
executable profile may record the same unique first block only once its root
can be represented without a circular state-root dependency.  It must also
define the scheduled block transition for which
\(\mathsf{closes\_round}(B)\) is true.  A valid vote that refers to an unknown
