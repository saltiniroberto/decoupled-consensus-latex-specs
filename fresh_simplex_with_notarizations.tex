...
A \emph{non-finality period} is a maximal interval $[t_1, t_T]$ of consecutive slots processed on~$X$ such that, at every slot~$t$ in the interval,
\[
  \sigma_{\mathrm{pre}}^{(t)}.F \prec \sigma_{\mathrm{pre}}^{(t)}.J
  \quad\text{and}\quad
  \sigma_{\mathrm{post}}^{(t)}.F = \sigma_{\mathrm{pre}}^{(t)}.F.
\]
Equivalently, $F$ is stuck at some fixed value $F_0 \prec \sigma.J$ throughout the interval: a pending-finality gap is open at every slot's pre-state, and the finality branch of $\textsf{processHeight}$ does not fire anywhere in the interval.
In particular, the \emph{transition slot} at which $\sigma_{\mathrm{pre}}.F = \sigma_{\mathrm{pre}}.J$ first becomes $\sigma_{\mathrm{pre}}.F \prec \sigma_{\mathrm{pre}}.J$ (i.e., a $J$-advance that opens the gap) lies \emph{outside} the period: its pre-state has $\sigma_{\mathrm{pre}}.F = \sigma_{\mathrm{pre}}.J$, not~$\prec$.
\end{definition}

\begin{theorem}[Non-finality period penalty bound]\label{thm:tightness}
Over any non-finality period of length $T \geq 1$ on the canonical chain, the total penalty increment across all validators assessed by $\textsf{processInactivityPenalties}$ is strictly greater than $T \cdot n/3$.
\end{theorem}

\begin{proof}
Fix a non-finality period $[t_1, t_T]$ with $T \geq 1$, and let $t \in [t_1, t_T]$.
By Definition~\ref{def:non-finality-period}, $\sigma_{\mathrm{pre}}^{(t)}.F \prec \sigma_{\mathrm{pre}}^{(t)}.J$ and $\sigma_{\mathrm{post}}^{(t)}.F = \sigma_{\mathrm{pre}}^{(t)}.F$, so the Layer~2 finalize guard in Figure~\ref{alg:leak-processslot} fires at~$t$.
...
Layer~2 finalize therefore charges strictly more than $n/3$ validators at slot~$t$.
Summing over the $T$ slots of the period, and noting that Layer~1 and the Layer~2 target guard can only add non-negative charges on top, the total penalty increment across all validators is strictly greater than $T \cdot n/3$.

The bound is purely state-logic: the proof reads $\sigma_{\mathrm{pre}}^{(t)}$ and $\sigma_{\mathrm{post}}^{(t)}$ on the canonical chain and counts Layer~2 finalize charges directly from Figure~\ref{alg:leak-processslot}, with no hypothesis on synchrony, on the honest voting rule, or on the size of the slashable set.
\end{proof}
