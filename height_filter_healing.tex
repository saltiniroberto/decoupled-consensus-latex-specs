...
\newcommand{\heightClass}{\mathsf{height\_class}}
...
schedule supplies the Boolean \(\mathit{close}\), which is true only for the
block transition that records the end-of-round result.  All timely votes for
that close have already been included before the function is called.  The
abstract model uses that block as the next height's target.  Calling the close
rule after each block is useful as a safety-proof interpretation, but the
round bounds in this paper use exactly one scheduled close per round.
...
\Statex \hspace{1em}\(\heightClass\gets\mathsf{ordinary},\
...
\Function{state\_transition}{$\sigma,B,\mathit{close}$}
  \State \textbf{assert} \(B.\mathsf{parent}=\sigma.L\) and
    \(B.\mathsf{slot}>\sigma.s\)
  \State \(\sigma.L\gets B;\quad \sigma.s\gets B.\mathsf{slot}\)
  \ForAll{valid signed messages \(m\in B.\mathsf{messages}\)}
    \State \(\sigma\gets\Call{process\_message}{\sigma,m,B}\)
  \EndFor
  \If{\(\mathit{close}\)}
    \State \(\sigma\gets\Call{process\_round\_close}{\sigma}\)
  \EndIf
  \State \Return \(\sigma\)
\EndFunction
\vspace{3pt}

\Function{process\_message}{$\sigma,m,B$}
  \State \(i\gets m.\mathsf{validator}\)
  \If{\(i\in V_{\sigma.h}\) and
       \(m.d=(\mathsf{target},\sigma.h,\sigma.\currentHeightTarget)\)}
    \If{\(\sigma.\currentHeightTarget\neq\bot\) and
         \(\sigma.\currentHeightTarget\prec B\)}
      \State \(\sigma.\targetParticipation[i]\gets\mathsf{true}\)
      \State \(\sigma.\timeoutParticipation[i]\gets\mathsf{true}\)
    \EndIf
  \ElsIf{\(i\in V_{\sigma.h}\) and
          \(m.d=(\mathsf{timeout},\sigma.h)\)}
    \State \(\sigma.\timeoutParticipation[i]\gets\mathsf{true}\)
  \EndIf
  \If{\(i\in V_{\sigma.h_j}\), \(\sigma.h_j>\sigma.h_F\),
      \(\sigma.F\preceq\sigma.J\), and \(m.\phi=(\sigma.h_j,\sigma.J)\)}
...
\Function{process\_round\_close}{$\sigma$}
  \If{\(\sigma.h_j>\sigma.h_F\), \(\sigma.F\preceq\sigma.J\), and
       \(w_{\sigma.h_j}(\sigma.P)\geq q_{\sigma.h_j}\)}
    \State \((\sigma.h_F,\sigma.F)\gets(\sigma.h_j,\sigma.J)\)
  \EndIf
  \State \(Q_T\gets\{i\in V_{\sigma.h}:\sigma.\targetParticipation[i]\}\)
  \If{\(\sigma.\heightClass=\mathsf{ordinary}\),
       \(\sigma.\currentHeightTarget\neq\bot\), and
       \(w_{\sigma.h}(Q_T)\geq q_{\sigma.h}\)}
    \State \((\sigma.h_j,\sigma.J)\gets
      (\sigma.h,\sigma.\currentHeightTarget);\quad \sigma.P\gets\varnothing\)
    \State \Return \(\Call{enter\_next\_height}{\sigma}\)
  \EndIf
  \State \(Q_{\rm timeout}\gets
    \{i\in V_{\sigma.h}:\sigma.\timeoutParticipation[i]\}\)
  \If{\(w_{\sigma.h}(Q_{\rm timeout})\geq q_{\sigma.h}\)}
    \State \Return \(\Call{enter\_next\_height}{\sigma}\)
  \EndIf
  \State \Return \(\sigma\)
\EndFunction
...
\Function{enter\_next\_height}{$\sigma$}
  \State \(\sigma.h\gets\sigma.h+1\)
  \State \(\sigma.\currentHeightStartSlot\gets\sigma.s;\quad
    \sigma.\currentHeightTarget\gets\sigma.L\)
  \If{\(\sigma.h-\sigma.h_F>D_{\rm debt}\) and \(K\mid\sigma.h\)}
    \State \(\sigma.\heightClass\gets\mathsf{nonjustifiable}\)
  \Else
    \State \(\sigma.\heightClass\gets\mathsf{ordinary}\)
  \EndIf
...
\[
  S=(\mathcal B,\mathcal M,\mathcal R,\Theta,\mathcal C,t),
\]
with:
\begin{itemize}
  \item received blocks \(\mathcal B\), including blocks whose parent is not
    yet known and finalized-conflicting blocks retained as evidence;
  \item signed FG and head messages \(\mathcal M\), including messages
    extracted from received blocks;
  \item pointer proposals \(\mathcal R\), each containing a round and either
    a root or an explicit empty pointer;
  \item timely facts \(\Theta\), where \(\mathsf{timely}(x,I)\) records that object \(x\) was
    received before the deadline of scheduled step~\(I\);
  \item completed-step records \(\mathcal C\), each containing the step, its
    exact input, the preceding records on which it depended, and its fixed
    result; and
  \item the local round clock \(t\).
\end{itemize}
...
either copy.  If these inputs contain two distinct pointers signed by the round's
...
Figure~\ref{alg:store} gives the store operations.  An object with an unknown
referenced block remains in its raw set.  It becomes usable by an uncompleted
step when the block and ancestry are known and were timely for that step; it
never changes a completed step.
The derived state map is rebuilt after a merge.  It processes each block from
its own parent state and its own included messages, and never combines sibling
...
\Statex \hspace{1em}\(\mathcal B\gets\{\mathsf{genesis}\},\
  \mathcal M\gets\mathcal R\gets\Theta\gets\mathcal C\gets\varnothing,\
  \ t\gets0\)
\vspace{3pt}

\Function{add\_object}{$S,x$}
  \If{\(x\) is a block}
    \State \(S.\mathcal B\gets S.\mathcal B\cup\{x\}\)
    \State \(S.\mathcal M\gets S.\mathcal M\cup x.\mathsf{messages}\)
    \State \(O\gets\{x\}\cup x.\mathsf{messages}\)
  \ElsIf{\(x\) is a signed message}
    \State \(S.\mathcal M\gets S.\mathcal M\cup\{x\};\quad O\gets\{x\}\)
  \Else \Comment{Pointer proposal, including an explicit empty pointer}
    \State \(S.\mathcal R\gets S.\mathcal R\cup\{x\};\quad O\gets\{x\}\)
  \EndIf
  \ForAll{\(y\in O\) and scheduled steps \(I\) whose input or validation uses \(y\)}
    \If{\(S.t<\mathsf{deadline}(I)\)}
      \State \(S.\Theta\gets S.\Theta\cup\{\mathsf{timely}(y,I)\}\)
    \EndIf
  \EndFor
  \State \Return \(S\) \Comment{Unknown references remain in the raw sets}
\EndFunction
\vspace{3pt}
...
\Function{derive\_block\_states}{$S$}
  ...
  \Repeat
    ...
    \ForAll{\(B\in S.\mathcal B\setminus\mathcal A\) with
             \(B.\mathsf{parent}\in\mathcal A\)}
      \State \(\sigma'\gets\Call{state\_transition}{
        \sigma[B.\mathsf{parent}],B,\mathsf{closes\_round}(B)}\)
      \If{\(B\) is valid and its authenticated post-state equals \(\sigma'\)}
...
\Function{complete\_step}{$S,I$}
  \If{\(S.\mathcal C\) already contains a record for \(I\) in this history}
    \State \Return \(S\) \Comment{Never recompute a completed step}
  \EndIf
  \State \textbf{assert} \(S.t\geq\mathsf{deadline}(I)\) and every required
    preceding step has exactly one history-compatible result
  \State \(X\gets\{x:\mathsf{timely}(x,I)\in S.\Theta\text{ and every
    validation dependency }z\text{ has }\mathsf{timely}(z,I)\in S.\Theta\}\)
  \State \(D\gets\) the completed records used by step \(I\)
  \State \(y\gets\mathsf{execute\_step}(I,X,D)\)
  \State \(S.\mathcal C\gets S.\mathcal C\cup\{(I,X,D,y)\}\)
  \State \Return \(S\)
\EndFunction
\vspace{3pt}
...
\Function{merge\_stores}{$S_1,S_2$}
  ...
  \State \Return \(S\) with
  \Statex \hspace{1em}\(S.\mathcal B=S_1.\mathcal B\cup S_2.\mathcal B,\quad
    S.\mathcal M=S_1.\mathcal M\cup S_2.\mathcal M,\quad
    S.\mathcal R=S_1.\mathcal R\cup S_2.\mathcal R\)
  \Statex \hspace{1em}\(S.\Theta=S_1.\Theta\cup S_2.\Theta,\quad
    S.\mathcal C=S_1.\mathcal C\cup S_2.\mathcal C,\quad
    S.t=\max(S_1.t,S_2.t)\)
