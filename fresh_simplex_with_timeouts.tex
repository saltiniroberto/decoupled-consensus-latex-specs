...
Only \emph{justified} cert events (height-advances fired by \processHeight's justification branch) are compared against $\key_R$ in lex order; whenever such an event's key strictly dominates, $(R, \key_R)$ is updated.
The key packages height, the justified-bit (always~$1$ for events that reach~$R$), the cert event's slot component, and a block hash, so that among justified events at the same height the more recent (higher vote-slot) one wins.
...
\end{itemize}
The genesis store has $\T = \{\genesis\}$, $F_s = R = \genesis$, and $\key_R = (0,\, 1,\, 0,\, \hash(\genesis))$, treating genesis as a bit-$1$ cert event at height~$0$ with slot component~$0$.
\end{definition}

The capital~$\Sigma$ distinguishes the store from the per-chain state~$\sigma$.
We write $\Sigma.\sigma$, $\Sigma.\T$, $\Sigma.F_s$, $\Sigma.R$, $\Sigma.\key_R$ for field access, and $\Sigma.\key_R.h$, $\Sigma.\key_R.b$ for the height and justification-bit components of the cached key.
...
The post-state then offers the justified-checkpoint descriptor $(\sigma[B].h_j,\, 1,\, \sigma[B].s_j,\, \sigma[B].J)$ to \updateForkChoiceRoot; notarization-only events do not update~$R$.
Each call to \updateForkChoiceRoot first filters out any candidate~$C$ with $C \not\succeq \Sigma.F_s$ and then updates $(\Sigma.R,\, \Sigma.\key_R)$ iff the candidate's cert key strictly exceeds the current $\key_R$; this maintains $R$ incrementally as the lex-max cert event ever observed on a chain extending the current $F_s$, never recomputed.
Finally, \updateFinalized sets $\Sigma.F_s \gets \sigma[B].F$ iff $\sigma[B].F \succ \Sigma.F_s$ and $\sigma[B].F \preceq \Sigma.R$.
...
\State \textbf{Genesis store} $\Sigma$:\ $\Sigma.\T \gets \{\genesis\},\ \Sigma.F_s \gets \genesis,\ \Sigma.R \gets \genesis,\ \Sigma.\key_R \gets (0,\, 1,\, 0,\, \hash(\genesis)),$
...
\Function{\onBlock}{$\Sigma,\; B$}
  ...
  \State $\Sigma \gets \updateForkChoiceRoot(\Sigma,\; (\sigma'.h_j,\; 1,\; \sigma'.s_j,\; \sigma'.J))$ \Comment{Only justified checkpoints update $R$}
...
\Function{\updateFinalized}{$\Sigma,\; F_B$}
  \If{$F_B \succ \Sigma.F_s$ and $F_B \preceq \Sigma.R$}
...
The $F_s$-filter never discards a key-dominant event: \Cref{thm:fleqr} below shows that $F_s$ advances only to blocks $\preceq R$, so every cert event whose key could beat $R$ remains on the $F_s$-chain.
...
\begin{theorem}[$F_s \preceq R$]\label{thm:fleqr}
The store maintains $F_s \preceq R$ at all times.
\end{theorem}

\begin{proof}
By induction on store operations.
At genesis, $F_s = R = \genesis$, so $F_s \preceq R$.
Assume the invariant before a call to either mutator in \Cref{alg:store}.

\emph{\updateForkChoiceRoot.}
$F_s$ is untouched, and $R$ either stays or is replaced by a candidate~$C$ that passed the $C \succeq \Sigma.F_s$ filter; either way $R \succeq F_s$.

\emph{\updateFinalized.}
$R$ is untouched, and the guard $F_B \preceq \Sigma.R$ ensures $F_s \gets F_B \preceq R$.
\end{proof}
...
Once a node sets $F_s = F$, $\getConfirmed(\Sigma, \Omega)$ returns a block descending from~$F$ at all future times, for every~$\Omega$.
\end{theorem}

\begin{proof}
By \Cref{thm:finperm} and~\Cref{thm:fleqr}, $F \preceq F_s \preceq R$, and $\getConfirmed(\Sigma, \Omega)$ returns a descendant of~$R$ regardless of~$\Omega$.
\end{proof}
...
$R \succeq F_s^0 \succeq F$ already holds by \Cref{thm:fleqr}; \Cref{thm:finperm} preserves $F_s \succeq F_s^0$ and \Cref{thm:fleqr} preserves $R \succeq F_s$ at all future times, so $R \succeq F$ persists.
...
If $h_R > h_F$, \Cref{lem:certchain} applied to the cert event $(R,\, h_R,\, \cdot,\, 1)$ gives $F \prec R$.
If $h_R = h_F$, $R$ was justified at height~$h_F$ on some processed chain, giving a \just-quorum~$Q'$ of size $\geq 2n/3$ with target~$R$ at height~$h_F$.
Finality of~$F$ at~$h_F$ gives a quorum~$Q_F$ with $\finalizeHeight = h_F$ and $\finalizeTarget = F$.
By quorum intersection, $|Q' \cap Q_F| \geq n/3$; pick $v \in Q' \cap Q_F$ not slashable.
E1 (\Cref{def:slashing}) applied to $v$'s votes in~$Q', Q_F$ forces $R = F$.
\end{proof}
...
\emph{$R$.} The multiset~$D$ of descriptors offered to \updateForkChoiceRoot is a deterministic function of the processed set: each block contributes one justified-checkpoint descriptor exactly once.
...
Hence $R$'s final value is the lex-max over $\{(h, 1, s, C) \in D : h \geq h_{F_{\max}}\}$, a deterministic function of the processed set.
\end{proof}

\begin{corollary}[Cross-node agreement]\label{cor:agreement}
Unless $\geq n/3$ validators are slashable: two nodes that have processed the same set of blocks have the same $(\sigma,\, \T,\, F_s,\, R,\, \key_R)$.
\end{corollary}
...
By \Cref{rem:fs-invariant}, $\Sigma.\key_R.b = 1$, i.e., $R$ is justified at height~$h_R$ on some processed chain.
$X$ descends from~$R$ (\Cref{alg:store}), so state-height monotonicity (\Cref{lem:slotmono}) gives $h_c \geq h_R$ and $\sigma[R].h = h_R$.
...
\emph{Locked case, $h_c = h_R$.}
\Cref{lem:no-finalize-without-just} gives $L$ justified at~$h_c$ on some processed chain; $\Sigma.\key_R.b = 1$ gives $R$ justified at~$h_R = h_c$ on some processed chain.
\Cref{lem:just-unique-height} then forces $L = R$, so $T_c = L = R \preceq X$ with $\sigma[T_c].h = \sigma[R].h = h_c$.
\end{proof}
